...
// ماژولِ مشترکِ قفلِ سیگنال — «همان منطقِ کلاینتِ سایت» (منبعِ واحدِ حقیقت).
// harness این را اعمال می‌کند تا رفتارِ end-to-end واقعیِ سایت (با قفل) را بسنجد،
// نه فقط سیگنالِ خامِ سرور. فایل یک ES module است (export) و مستقیماً import می‌شود.
import { computeLatched } from '../public/static/signal_latch.js'
