...
// ----------------------------------------------------------------------------
// 🩹 fetchJSON — تنها دروازهٔ شبکهٔ این فایل. (رفعِ باگِ «نوار روی ۹ کارت می‌ماند»)
// ----------------------------------------------------------------------------
// تشخیصِ باگ: همهٔ فراخوانی‌های `fetch` در این فایل **بدونِ مهلت** بودند. مرورگر
// برای یک درخواستِ بی‌پاسخ هیچ سقفِ زمانیِ کوتاهی ندارد (ده‌ها ثانیه تا دقایق).
// در مسیرِ `refreshAll` نتیجه‌اش کشنده بود:
//
//     await Promise.allSettled(active.map(a => refreshOneAsset(a.id)))
//     boot.finish()
//
// `allSettled` منتظرِ **کندترین** عضو می‌ماند. اگر فقط *یکی* از ۹ درخواست معلق
// می‌ماند، `boot.finish()` هرگز اجرا نمی‌شد و نوار تا ابد روی «در حال دریافتِ
// تحلیلِ ۹ کارت…» می‌ایستاد — عیناً همان چیزی که گزارش شد. توجه: `try/catch`
// این را نمی‌گرفت، چون درخواستِ معلق **خطا نمی‌دهد**؛ فقط هیچ‌وقت تمام نمی‌شود.
// یک کارتِ کند کلِ صفحه را گروگان می‌گرفت.
//
// راهِ حل: هیچ درخواستی حقِ بی‌نهایت ماندن ندارد. `AbortController` تضمین می‌کند
// هر فراخوانی یا موفق می‌شود یا در مهلتِ معین **صریحاً شکست می‌خورد** — و شکست،
// برخلافِ معلق ماندن، در UI قابلِ نمایش است. کاربر «خطا در این کارت» را می‌بیند
// و بقیهٔ سایت بالا می‌آید؛ به‌جای صفحه‌ای که تا ابد در حالِ بارگذاری است.
async function fetchJSON(url, { timeoutMs = 12000, ...init } = {}) {
  ...
  try {
    ...
    // پیامِ `AbortError` مرورگر برای کاربرِ فارسی‌زبان بی‌معناست؛ ترجمه‌اش می‌کنیم.
    if (e && e.name === 'AbortError') throw new Error('پاسخی از سرور نرسید (مهلت تمام شد)')
    throw e
  } finally {
    clearTimeout(timer)
  }
...
// ----------------------------------------------------------------------------
// 🐕 سگِ نگهبانِ نوار — آخرین خطِ دفاع.
// ----------------------------------------------------------------------------
// مهلت‌ها و زمان‌بندِ تازه باید مانعِ گیر کردنِ نوار شوند. ولی «باید» تضمین نیست:
// باگِ اصلی هم دقیقاً از همین جنس بود — مسیری که کسی تصور نمی‌کرد تمام نشود.
// درسِ آن باگ این است که نوارِ پیشرفت نباید به درستیِ منطقِ بالادست **وابسته**
// باشد؛ باید مستقلاً تضمین کند که تمام می‌شود.
//
// این نگهبان با هیچ‌کدام از رفع‌های دیگر هم‌پوشانی ندارد: آن‌ها *علت* را می‌بندند،
// این *نشانه* را. اگر فردا لایه‌ای تازه مسیرِ نویی بسازد که تمام نشود، کاربر باز
// هم صفحهٔ قابلِ استفاده می‌بیند به‌جای نوارِ جاودانه — و پیام می‌گوید کدام کارت‌ها
// نیامده‌اند، که خودش سرنخِ دیباگ است.
setTimeout(() => {
  if (boot.finished) return
  const missing = Math.max(0, boot.total - boot.done)
  boot.fail(missing
    ? `${missing} کارت پاسخ نداد — بقیه نمایش داده شد`
    : 'سرور پاسخ نداد — دوباره تلاش کنید')
  // صفحه باید قابلِ استفاده بماند حتی اگر هیچ کارتی نیامده باشد.
  const el = document.getElementById('boot-progress')
  if (el) { el.style.transition = 'opacity .6s ease'; setTimeout(() => { el.style.opacity = '0.6' }, 400) }
}, 45000)
      ...
      <div class="mb-2 flex items-center gap-2 rounded-md ${d._fading ? 'bg-amber-500/15 border-amber-500/40' : 'bg-slate-800/70 border-slate-600/60'} border px-2.5 py-1.5 text-[11px]">
        ...
        <span class="${d._fading ? 'text-amber-200' : 'text-slate-300'}">
          ${d._fading
            ...
            : 'این پیشنهادِ ورود قفل شده و با نوسانِ کوچکِ قیمت پس‌وپیش نمی‌شود (نقاطِ ورود/TP/SL پیشنهادی پابرجا می‌مانند تا معامله را ثبت کنی).'}
        </span>
      </div>` : ''
  return `
...
// ============================================================================
// ----------------------------------------------------------------------------
// 🩹 زمان‌بندیِ ضدِ-انباشت (رفعِ باگِ «کم‌کم کند می‌شود و آخرش بالا نمی‌آید»)
// ----------------------------------------------------------------------------
// تشخیص: کدِ پیشین `setInterval(refreshAll, REFRESH_MS)` بود. `setInterval` هیچ
// اهمیتی نمی‌دهد که دورِ قبلی تمام شده یا نه — سرِ وقت شلیک می‌کند. اگر یک دور
// بیشتر از REFRESH_MS طول می‌کشید (در بازارِ بسته/منبعِ کند تقریباً حتمی)، دورِ
// بعدی **روی آن سوار می‌شد**. هر دور ۹ درخواست ⇒ ۹ درخواستِ تازه روی ۹ درخواستِ
// هنوز-تمام-نشده. سرور کندتر ⇒ دورها طولانی‌تر ⇒ هم‌پوشانیِ بیشتر. این یک حلقهٔ
// بازخوردِ مثبت است؛ خروجی‌اش دقیقاً «اول سریع، بعد کند، بعد هرگز» است.
//
// راهِ حل — سه لایه، هر کدام یک نقصِ متفاوت را می‌بندد:
//
//   ۱) زنجیرهٔ setTimeout به‌جای setInterval: دورِ بعدی **پس از پایانِ** دورِ
//      فعلی زمان‌بندی می‌شود. هم‌پوشانی از نظرِ ساختاری ناممکن می‌شود، نه صرفاً
//      بعید. (قفلِ بولی هم کار می‌کرد ولی دور را «رد» می‌کند؛ این‌جا دور فقط
//      عقب می‌افتد و هیچ به‌روزرسانی‌ای گم نمی‌شود.)
//
//   ۲) عقب‌نشینیِ تطبیقی: اگر دوری کند بود، فاصله تا دورِ بعد بلندتر می‌شود.
//      وقتی سرور زیرِ فشار است، فشار آوردنِ بیشتر بدترین کارِ ممکن است. سایت
//      به‌جای اصرار، خودش را با سرعتِ واقعیِ سرور تطبیق می‌دهد.
//
//   ۳) توقف در پس‌زمینه: اگر تب دیده نمی‌شود، پُلینگ متوقف می‌شود. مرورگر
//      تایمرهای تبِ پنهان را throttle می‌کند ولی fetchها را نه؛ نتیجه‌اش تبی
//      بود که ساعت‌ها در پس‌زمینه بی‌صدا به سرور فشار می‌آورد. کاربری که سایت
//      را در یک تب باز می‌گذارد، نباید دلیلِ کند شدنش باشد.
//
// این هر سه، رفتارِ «بازگشت به حالتِ سالم» را می‌سازند: سایتِ کند خودش را ترمیم
// می‌کند به‌جای اینکه خودش را به مرگ برساند.

// یک زمان‌بندِ خود-زنجیره‌ایِ عمومی. چون همین الگو برای هر پُلینگی لازم است،
// یک‌بار نوشته می‌شود و همهٔ پُلینگ‌ها از آن استفاده می‌کنند (ماژولاریتیِ ROS2:
// افزودنِ پُلینگِ تازه در آینده = یک فراخوانی، نه تکرارِ دوبارهٔ این منطق).
function startPoller(name, fn, baseMs, { maxMs = baseMs * 8, pauseWhenHidden = true } = {}) {
  let timer = null
  let stopped = false
  let delay = baseMs
  ...
  async function tick() {
    if (stopped) return
    // تبِ پنهان ⇒ کاری نکن، فقط دوباره سر بزن. (بدونِ fetch ⇒ بدونِ بارِ سرور.)
    if (pauseWhenHidden && document.hidden) {
      timer = setTimeout(tick, Math.min(baseMs * 2, 10000))
      return
    }
    const t0 = Date.now()
    try {
      await fn()
      // موفق و سریع ⇒ به‌تدریج به ریتمِ پایه برگرد (نه پرشِ ناگهانی).
      const took = Date.now() - t0
      delay = took > baseMs ? Math.min(maxMs, Math.round(delay * 1.5))
                            : Math.max(baseMs, Math.round(delay * 0.7))
    } catch (e) {
      // خطا ⇒ عقب‌نشینی. سرورِ خطادار را بمباران نکن.
      delay = Math.min(maxMs, Math.round(delay * 1.8))
      console.warn(`[poller:${name}]`, e && e.message)
    }
    if (!stopped) timer = setTimeout(tick, delay)
  }

  // وقتی کاربر به تب برمی‌گردد، فوراً یک دورِ تازه بزن (نه اینکه منتظرِ تایمر بماند)
  // تا دادهٔ روی صفحه بلافاصله تازه شود.
  if (pauseWhenHidden) {
    document.addEventListener('visibilitychange', () => {
      if (!document.hidden && !stopped) {
        clearTimeout(timer)
        delay = baseMs          // بازگشت به تب = شروعِ تازه، بدونِ جریمهٔ عقب‌نشینیِ قبلی
...
refreshAll()
startPoller('decisions', refreshAll, REFRESH_MS)
// پُلینگِ سریعِ قیمت (هر ۲ ثانیه) — سبک، فقط قیمت
startPoller('spots', refreshSpots, SPOT_MS)
