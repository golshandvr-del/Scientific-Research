...
const checks = {
  ...
  // ④ قیدِ (الف): S1911 هرگز primary نیست ⇒ کاربر یک تصمیمِ اصلی می‌بیند و این
  //    لایه در otherLayers می‌نشیند. این **اصلِ** قیدِ ممیزی است.
  neverPrimary: primaryS1911 === 0,
  // ⑤ و در هر رویداد، S1911 عملاً در otherLayers دیده شده (نه غایب) ⇒ ثابت می‌کند
  //    ④ به‌خاطرِ «اجرا نشدنِ لایه» سبز نشده، بلکه لایه اجرا شده و آگاهانه پایین نشسته.
  presentInOtherLayers: s1911InOthers === nPick,
  // ⑥ تلاقی با ابرمجموعه واقعاً رخ می‌دهد: هر ۸۲ رویدادِ S1911 داخلِ S965 است، پس
  //    S965 باید در هر رویداد **حاضر** باشد (primary یا در otherLayers). این
  //    اندازه‌گیریِ همپوشانی را از مسیرِ زندهٔ سایت هم تأیید می‌کند.
  //    ⚠️ عمداً «S965 == primary» نمی‌سنجیم: S950 در آرایه بالاتر از S965 است و
  //    وقتی همزمان ENTRY بدهد، طبقِ طراحیِ کارت اولویت می‌گیرد — که ربطی به S1911
  //    ندارد. اجرای اولِ همین اسکریپت این را نشان داد (۳ رویداد primary=S950).
  supersetPresentEveryEvent: rows.every(r => r.primary === 'S965' || r.others.includes('S965')),
}
