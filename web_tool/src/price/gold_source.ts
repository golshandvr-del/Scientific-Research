...
// ---------------------------------------------------------------------------
// 🚀 ادغامِ بازه‌های H1 روی یک درخواستِ ابَرمجموعه (رفعِ کندیِ لود — User Note)
// ---------------------------------------------------------------------------
// مسئله (اندازه‌گیری‌شده، نه حدس): کلیدِ کش `gold:${interval}:${range}` بود، پس
// بازه‌های مختلفِ **همان** interval سه کلیدِ جدا می‌ساختند و de-dup میانشان کار
// نمی‌کرد. کارت‌های H1 تا D1 همه از interval=1h تغذیه می‌شوند:
//     gold:1h:3mo  ← H1                 ≈ ۱۲۵ KB
//     gold:1h:1y   ← H4 · H8            ≈ ۴۸۸ KB
//     gold:1h:2y   ← H6 · H12 · D1      ≈ ۱۰۰۰ KB
// ⇒ هر لودِ سرد ≈ **۱.۶ MB فقط برای H1**، در حالی که `2y` هر دو بازهٔ دیگر را
//   **در خود دارد**. یعنی ~۶۱۳ KB از آن، دادهٔ تکراری بود. روی سندباکسِ نزدیک
//   به Yahoo این ۱.۷ ثانیه است، ولی روی اتصالِ کاربر همین چند ده ثانیه می‌شود —
//   و این همان «خیلی دیر لود می‌شود» است.
//
// راه‌حل: یک کلیدِ مشترکِ ابَرمجموعه (`gold:1h:2y`) و ساختِ بازه‌های کوتاه‌تر با
// **برشِ** همان آرایه. سه کارتِ اولِ H1 دیگر هیچ درخواستِ شبکه‌ای نمی‌زنند و از
// همان یک fetch (که single-flight هم هست) تغذیه می‌شوند ⇒ ۳ درخواست ⇒ **۱**.
//
// ⚠️ چرا این از نظرِ دادهٔ تصمیم امن است — تأییدِ تجربی پیش از نوشتنِ کد:
//   `1h/1y` و `1h/3mo` را با `1h/2y` مقایسه کردم:
//     · 1y : ۵۶۹۵ کندلِ مشترک = **۱۰۰.۰٪** پوشش
//     · 3mo: ۱۴۵۸ کندلِ مشترک = **۹۹.۹٪** پوشش
//     · اختلافِ close: **فقط ۱ کندل** — و آن دقیقاً **آخرین کندلِ در حالِ
//       تشکیل** است (۴۴۱۹.۶۰ در برابرِ ۴۴۱۸.۷۰)، چون دو درخواست چند
//       میلی‌ثانیه فاصله داشتند. هیچ کندلِ **بسته‌ای** اختلاف نداشت.
//   همهٔ لایه‌های این سایت روی کندلِ **بسته** تصمیم می‌گیرند (i = n−1)، پس این
//   تفاوت به هیچ تصمیمی نمی‌رسد. اگر روزی لایه‌ای بخواهد کندلِ نابسته را بخواند،
//   آن‌وقت این ادغام باید بازبینی شود — دلیلش همین‌جا نوشته شد.
//
// ⚠️ مرزِ محافظه‌کاری: این ادغام **فقط** برای interval=1h فعال است، چون تنها
//   همین interval است که چند بازهٔ هم‌پوشان دارد و بالای ۹۹.۹٪ سازگاری سنجیده
//   شد. بازه‌های ۵m/۱۵m/۳۰m دست‌نخورده می‌مانند (نه هم‌پوشانی دارند، نه اندازه‌گیری
//   شده‌اند) ⇒ صفر ریسکِ رگرسیون برای کارت‌های M5/M15/M30.
const H1_SUPERSET_RANGE = '2y'
// چند کندلِ H1 در هر بازه لازم است (سخاوتمندانه — کفِ واقعی را index.tsx می‌سنجد).
const H1_RANGE_BARS: Record<string, number> = { '1d': 30, '5d': 130, '1mo': 560, '3mo': 1600, '6mo': 3200, '1y': 6200 }
...
export async function fetchGold(interval: string, range: string): Promise<{ candles: Candle[]; meta: any }> {
  // 🚀 مسیرِ ادغام: بازهٔ کوتاه‌ترِ H1 ⇒ از ابَرمجموعهٔ ۲ساله برش بزن (صفر درخواستِ نو).
  if (interval === '1h' && range !== H1_SUPERSET_RANGE && H1_RANGE_BARS[range]) {
    const sup = await cachedFetch(`gold:1h:${H1_SUPERSET_RANGE}`,
      () => _fetchGoldRaw('1h', H1_SUPERSET_RANGE), { freshMs: 30_000, staleMs: 600_000 })
    const want = H1_RANGE_BARS[range]
    // اگر ابَرمجموعه به‌اندازهٔ کافی عمیق بود ⇒ برش. وگرنه همان کلِ آرایه
    // (هرگز کمتر از درخواست نمی‌دهیم؛ بیشتر بودن برای هیچ لایه‌ای مضر نیست چون
    //  همه پنجرهٔ خودشان را از انتهای آرایه می‌گیرند).
