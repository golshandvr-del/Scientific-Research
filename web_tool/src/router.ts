...
import { computeTripleSMA, DEFAULT_TRIPLE_SMA } from './triple_sma_pullback'
...
export interface RouterDecision {
  ...
  // --- دروازهٔ زمانی (برای شمارشِ معکوسِ زنده + نمایشِ وقتِ ایران) ---
  // لایه‌های زمان-محور (S139 Overnight، S140⁺⁺ Monday، S141 Turn-of-Month، S73 EURUSD)
  // این فیلد را پر می‌کنند تا فرانت‌اند بتواند «چند ساعت:دقیقه:ثانیه تا فعال‌شدنِ سیگنال»
  // را زنده نمایش دهد و ساعتِ UTC را به وقتِ ایران (UTC+3:30) ترجمه کند.
  timeGate?: {
    layerCode: string           // کدِ لایه (S139/S140⁺⁺/S141/S73)
    label: string               // نامِ فارسیِ پنجره
    entryHoursUtc: number[]     // ساعاتِ ورودِ UTC (مثلِ [18,19,20])
    activeDaysUtc?: number[]    // روزهای فعالِ هفته getUTCDay: 0=یکشنبه..1=دوشنبه (اگر روز-محور)
    dayOfMonthNote?: string     // توضیحِ قیدِ روزِ ماه (برای Turn-of-Month)
    // 🕒 باگِ User Note #۳: روزهای مجازِ *ماه* (۱..۳۱) برای countdownِ دقیق. اگر پر شود،
    //   فرانت‌اند شمارشِ معکوس را فقط تا نزدیک‌ترین «روزِ ماهِ مجاز + ساعتِ ورود» می‌دهد.
    activeDaysOfMonth?: number[]
    windowOpen: boolean         // آیا پنجره هم‌اکنون باز است (داخلِ ساعت/روزِ مجاز)
    // ساعتِ پایانِ پنجرهٔ ورودِ UTC (انحصاری): وقتی پنجره باز است، فرانت‌اند یک
    // «شمارشِ معکوس تا پایانِ فرصتِ ورود» (به وقتِ ایران) نمایش می‌دهد. اگر تعریف
    // نشده باشد، پیش‌فرض = max(entryHoursUtc)+1 (پایانِ آخرین کندلِ ساعتیِ ورود).
    endHourUtc?: number
  }
...
export function decide(a: AnalysisResult, close: number[],
  ...
  // ========================================================================
  // لایهٔ «Monday Week-Start Drift» — S140⁺⁺ (M5، دوشنبه ۱۸-۲۰ UTC)
  // ------------------------------------------------------------------------
  // قانونِ شمارهٔ ۱: فقط «سودِ خالصِ بیشتر» مهم است. طلا در عصرِ دوشنبه (۱۸–۲۰ UTC)
  // درایوِ صعودیِ ابتدای هفته دارد (اثرِ روزِ هفته؛ دوشنبه t=+6.11 قوی‌ترین روز).
  // ارتقا به M5 با TP/SL بازتنظیم‌شده (SL100/TP200) و حذفِ ساعتِ زیان‌دهِ ۲۱ (S190–S195):
    ...
    if (mo.state === 'ENTRY') {
      ...
      return {
        ...
        sizing: {
          ...
          note: `استراتژیِ S140⁺⁺ (زمان-محورِ روز×ساعت روی M5 — بدونِ اندیکاتور). دوشنبه ساعتِ ۲۱:۳۰/۲۲:۳۰/۲۳:۳۰ به وقتِ ایران، ورودِ open کندلِ بعد. ` +
            `ارتقا از M15 به M5 با TP/SL بازتنظیم‌شده و حذفِ ساعتِ زیان‌دهِ ۰۰:۳۰ به وقتِ ایران ⇒ WR ۴۴.۵٪، اثرِ افزایشی +$1,553 (تأییدشده روی M15).`,
          lots: lots ?? undefined,
          riskDollars: rd,
          capital, riskPct,
          capitalNote: `با سرمایهٔ ${capital.toLocaleString('en-US')}$ و ریسکِ ${riskPct}% ` +
