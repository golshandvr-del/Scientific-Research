...
/**
 * اندازهٔ براکت — **عیناً** قاعدهٔ رانرِ حکم، نه یک تقریبِ مشابه.
 *
 * مرجع: `strategies/s382_williamsr_momentum.py` خطوطِ ۹۴–۹۸ و ۱۸۶–۱۹۰، که
 * رانرِ S547 (`tools/s547_preholiday_runner.py`) هندسه‌اش را از آن وام می‌گیرد:
 *
 *     a = atr(df)                                  # ewm(alpha=1/100, adjust=False) روی TR
 *     sl_abs = float(np.nanmedian(a.to_numpy())) * 1.5
 *
 * یعنی دو مرحله، به همین ترتیب: ① TR با **EWMA** صاف می‌شود (α=۱/۱۰۰)،
 * ② میانه روی **کلِ سریِ موجود** گرفته می‌شود — نه روی پنجرهٔ اخیر.
 *
 * ⚠️ چرا این نکته حیاتی است و چطور لو رفت: نسخهٔ اولِ من میانهٔ **TR خام** را
 * روی **۱۰۰ کندلِ آخر** می‌گرفت، با این استدلالِ به‌ظاهر منطقی که «براکت باید
 * با مقیاسِ فعلیِ بازار هم‌خوان باشد». آزمونِ سطحِ کارت نشان داد نتیجه روی هر
 * سه کارت ۳.۷×–۴.۲× بزرگ‌تر از سند است (M30: ۱۵۲ پیپ به‌جای ۴۱). دلیلش
 * ساختاری است: طلا در ۲۰۲۶ چند برابرِ میانگینِ ۱۵.۶ ساله نوسان دارد، پس
 * «پنجرهٔ اخیر» عددی کاملاً متفاوت با «میانهٔ کل تاریخ» می‌دهد.
 *
 * و این یک اختلافِ زیبایی‌شناختی نیست: SL چهار برابر یعنی **ریسکِ هر معامله
 * چهار برابرِ چیزی است که RQS2 سنجیده**. سایت سبز می‌ماند، کارت سیگنال می‌دهد،
 * و کاربر فکر می‌کند لایه‌ای با maxDD ۳.۱٪ را معامله می‌کند در حالی که هندسهٔ
 * زیرِ پایش چیزِ دیگری است. حکم فقط برای همان هندسه صادر شده بود.
 *
 * نکتهٔ دوم (چرا «کلِ سری» و نه پنجره): چون عددِ سند یک **ثابتِ سراسری** است
 * که یک‌بار از کلِ تاریخ محاسبه می‌شود و در همهٔ ۱۴۶ معامله یکسان می‌ماند. هر
 * نسخهٔ «متحرک» عملاً یک پارامترِ آزادِ نو وارد می‌کند که سند آن را نسنجیده.
 */
export function medianAtr(
  ...
  // ② EWMA با α=۱/n و adjust=False — معادلِ دقیقِ `tr.ewm(alpha=1/p, adjust=False).mean()`
  //    (pandas مقدارِ اولیه را خودِ نخستین نمونه می‌گیرد؛ همین‌جا هم.)
  const alpha = 1 / n
  const ew: number[] = new Array(trs.length)
  let acc = trs[0]
  ew[0] = acc
  ...
  // ③ میانه روی **کلِ** سریِ صاف‌شده (معادلِ np.nanmedian روی کلِ آرایه)
