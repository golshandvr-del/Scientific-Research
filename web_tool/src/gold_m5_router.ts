...
export function decideGoldM5(a: AnalysisResult, close: number[],
  ...
  if (open && high && low && times && times.length === close.length) {
    ...
    if (le.inWindow) {
      ...
      // شفافیتِ User Note: تشخیصِ صریحِ «رالیِ قویِ climactic». اگر روند صعودیِ قوی
      // در جریان است (رشتهٔ بلندِ کندلِ صعودی) اما به‌خاطرِ بزرگ و انفجاری‌بودنِ
      // کندل‌ها ورودِ Late-Entry تأیید نمی‌شود، به‌جای پیامِ گنگِ «منتظرِ ۴ کندل»،
      // صراحتاً وضعیت و علتِ عدمِ ورود را توضیح می‌دهیم.
      if (le.climaxBlocked) {
        const climaxConf: Confirmation[] = [
          ...
          { label: 'ورودِ کم‌ریسک در این لحظه', met: false,
        ...
        return {
          ...
          reason: `روند را می‌بینم: طلا یک صعودِ قویِ پیوسته دارد و به پایانِ ماه هم نزدیکیم. ` +
            `دلیلِ اینکه سیگنالِ خرید نمی‌دهم این نیست که روند را ندیده‌ام — بلکه این حرکت ` +
            `بیش‌ازحد سریع و انفجاری (climactic) است. ورود وسطِ چنین جهشِ تندی ریسکِ بالای ` +
            `برگشتِ ناگهانی دارد و در آزمون‌ها ورودِ امنی نبوده. منطقِ درست: صبر تا حرکت کمی ` +
            `آرام شود یا یک پولبک بدهد؛ آن‌وقت ورود بسیار کم‌ریسک‌تر است. پس فعلاً وارد نمی‌شوم.`,
  ...
  return {
    ...
    reason: `روند را می‌بینم و صعودی است — این را درست تشخیص داده‌ام. اما این لایه یک اسکالپِ ` +
      `«پولبک‌خری» است: فقط وقتی قیمت وسطِ روند کمی برمی‌گردد پایین (RSI زیرِ ${RSI_ENTRY}) خرید می‌کند. ` +
      `الان قیمت گران است (RSI(21)=${rsi.toFixed(1)}، بالای ${RSI_APPROACH})؛ خریدن در اوجِ حرکت ریسکِ ` +
      `برگشت دارد. پس نه به‌خاطرِ ندیدنِ روند، بلکه به‌خاطرِ نبودنِ نقطهٔ ورودِ کم‌ریسک صبر می‌کنم ` +
      `تا یک پولبک بدهد.`,
    indicators,
  }
}
