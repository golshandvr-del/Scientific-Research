...
export function analyze(c: Candle[]): AnalysisResult {
  ...
  // 6b) ★ ماشهٔ شتابِ MACD (کشفِ اثبات‌شدهٔ S88 — پاسخِ User Note «فهمِ رفتارِ چارت»)
  //   عبورِ macd_hist از زیرِ صفر به بالای صفر، هم‌زمان با رژیمِ صعودی، بهترین لحظهٔ
  //   ورودِ LONG است: روی ۱۵۰k کندل PF=۱.۲۴، Sharpe=۲.۵۸، هر دو نیمهٔ داده مثبت.
  //   این «شروعِ مجددِ شتابِ صعودی هم‌سو با روند» است — نه پیش‌بینیِ کورکورانهٔ تک‌کندل
  //   (که در S85 اثبات شد شکست می‌خورد). وزنِ بالا فقط وقتی رژیم صعودی است.
  const mhPrev = macdHist[i - 1]
  const macdTrigger = regimeOk && mh > 0 && mhPrev <= 0
  add('macd_accel_trigger', macdTrigger ? 1 : 0, macdTrigger ? 0.30 : 0,
    macdTrigger
      ? '★ ماشهٔ شتابِ صعودی فعال شد (MACD از منفی به مثبت در رژیمِ صعودی — لبهٔ S88)'
      : 'ماشهٔ شتابِ صعودی فعال نیست')
  ...
  if (regimeOk) {
    ...
    activeBrain = 'bear'
    direction = 'NONE'
    noEntryReason = `رژیم نزولی تشخیص داده شد (قیمت < EMA50 < EMA200). چرا سایت SHORT نمی‌دهد؟ ` +
      `دو تحقیقِ مستقل این را اثبات کرد: (۱) S83 نشان داد جریانِ SHORTِ دستی لبهٔ سودِ خالص ` +
      `ندارد؛ (۲) S88 — پاسخ به همین شکایتِ شما — نشان داد حتی SHORT با تشخیصِ درستِ رژیمِ ` +
      `نزولی روی ۱۵۰k کندل ضرر می‌دهد (net=−۲٬۰۸۳$, PF=۰.۹۳). طلا داراییِ با بایاسِ صعودیِ ` +
      `ساختاری است؛ short کردنش ذاتاً بازنده است (قانونِ L53). پس مغزِ طلا فقط Long است. ` +
      `منتظرِ بازگشتِ رژیم صعودی + ماشهٔ MACD بمانید.`
  } else {
