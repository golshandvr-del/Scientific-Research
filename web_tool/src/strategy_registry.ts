...
// ⭐ S1520 ⭐نو — «سقفِ تازهٔ ۹۰-کندلی × کندلِ مطلع» (Informed Fresh High) · XAUUSD-H8
//    RQS2=90.6 · هر ۱۱ دروازه سبز · n=147 · WR=59.86% · lift=+19.13pp · z=3.83 · PF=2.31
//    تنشِ n_trials=50 ⇒ 90.0 (کاهشِ ناچیزِ ۰.۶ ⇒ حکم به بودجهٔ چندگانگی حساس نیست)
//    سند: results/S1520_InformedFreshHigh_Xauusd_H8_rqs2_91_ACCEPT.md
//    پیش‌ثبت: results/S1520_PREREG_InformedFreshHigh_Xauusd_H8H4H12.md
//
//    🔒 قانونِ MTF — سه کارت پیش‌ثبت شد، **فقط یکی** ACCEPT گرفت:
//       · H8  → RQS2 **90.6** ⇒ **وصل** (همین‌جا)
//       · H4  → RQS2 **16.7 REJECT** ⇒ وصل نمی‌شود. دلیلش عددی و آموزنده است:
//         بازوی **مخالف** (counter +10.93pp) از بازویِ گیت‌شده (+3.64pp) **بهتر**
//         بود ⇒ روی H4 کندلِ پُربدنهٔ سقف‌شکن «اورشوتِ درون‌روزی» است نه اطلاع.
//       · H12 → RQS2 **27.1** اثبات‌نشده (n ناکافی برای دروازهٔ توان) ⇒ وصل نمی‌شود.
//       این سه‌گانه در results/_s1520_parity/ به‌صورت **اجراپذیر** نگه داشته شده و
//       آزمونِ برابری، H4/H12 را هم پورت می‌کند تا ثابت شود غیبتشان یک **تصمیمِ
//       داوری** است، نه یک باگِ خاموشِ پورت.
//
//    ⚠️ قیدهای پرتفوی (اعداد از بخشِ ۴ سندِ ACCEPT، همان‌جا سنجیده شده‌اند):
//      · **S526 = والدِ همین رویداد** با ۸۲.۳٪ ورودِ مشترک (۱۲۱ از ۱۴۷).
//        قانون S605 ⇒ S1520 **جانشینِ** S526 است، نه رقیبش: همان رویداد، خالص‌تر
//        (n ۲۰۰→۱۴۷، WR ۵۵.۵→۵۹.۹، PF ۱.۹۴→۲.۳۱، z ۳.۲۳→۳.۸۳). S526 روی سایت
//        وصل **نیست** ⇒ تعارضی وجود ندارد؛ اگر روزی وصل شود **باید** یکی حذف گردد.
//      · **S382-H4 لایو**: هم‌پوشانی ۵۷.۱٪. اما بخشِ **غیرمشترک** هنوز lift
//        +۱۳.۲۳pp دارد (n=۶۳، WR ۵۳.۹۷) ⇒ ارزشِ مستقلِ واقعی دارد و حذف نمی‌شود؛
//        تنها هشدارِ حجم مشترک لازم است (کارتِ دیگری است، خودکار جمع نمی‌شود).
//    ماژولار (ros2-مانند): فایلِ کاملاً مستقل informed_fresh_high_s1520.ts؛ افزودنش
//      فقط یک ورودی در CARD_LAYERS['XAUUSD-H8'] می‌خواهد و هیچ لایهٔ دیگری را
//      دست نمی‌زند — پنج ساکنِ فعلیِ کارت (S950/S965/S770/S966/S607) بی‌تغییر می‌مانند.
import { decideS1520, S1520_CFG } from './informed_fresh_high_s1520'
// ⭐⭐ S589 ⭐نو — «سقفِ تازهٔ ۹۰-کندلی × تأییدِ حجمِ نسبیِ هم‌اسلات» · **دو کارت**
//    (Volume-Confirmed Fresh High) — نخستین لایهٔ سایت که از **بُعدِ پنجمِ داده
//    (حجم)** استفاده می‌کند؛ هیچ‌یک از لایه‌های وصل‌شدهٔ پیشین آن را لمس نکرده بود.
//    · XAUUSD-H8 ⇒ RQS2 **88.3** (تنشِ n_trials=50 ⇒ 86.9) · n=159 · WR 56.60٪
//      lift +15.87pp · z=3.16 · PF 2.02 · maxDD ۲.۱٪ · SL/TP = 179.67/269.50 pip
//    · XAUUSD-H4 ⇒ RQS2 **86.3** · n=322 · WR 50.93٪ · lift +9.86pp · z=3.29
//      PF 1.60 · SL/TP = 122.85/184.28 pip  ← هندسهٔ **خودش**، نه قرضی از H8
//    · XAUUSD-H12 ⇒ **REJECT 25.1** ⇒ عامدانه وصل **نمی‌شود** (قانونِ MTF در
//      جهتِ عکسش: تعمیم بدونِ شاهد ممنوع). دلیلِ مکانیکی هم روشن است: نرخِ
//      عبورِ گیت روی H12 **۷۵.۴٪** است، بالای سقفِ ۷۵٪ِ قانونِ S529 ⇒ گیت
//      آن‌جا چیزی را غربال نمی‌کند. شاهدِ منفی **اجراشدنی** است
//      (results/_s589_parity/XAUUSD_H12.json) و آزمونِ پریتی به‌صورتِ assert
//      نگه می‌داردش، پس افزودنِ H12 در آینده تست را می‌شکند نه اینکه منتشر شود.
//
//    ⭐ **چرا این لایه اطلاعِ نو می‌آورد (و نه فقط یک فیلترِ دیگر):** بازوی
//      مکملِ کم‌حجم روی **H4** lift = **−۰.۱۷pp** گرفت — یعنی سقفِ تازه بدونِ
//      تأییدِ حجم روی آن کارت **صفرِ مطلق** لبه دارد؛ با آن ACCEPT می‌شود. پس
//      حجم اینجا «فیلترِ کیفیت» نیست، **خودِ حاملِ اطلاعات** است (Easley–O'Hara).
//      قانونِ نوِ نقشه: «شکستِ سقف روی طلا حجم‌محور است؛ شکستِ کم‌حجم نویز است.»
//
//    ⚠️⚠️ **ممیزیِ شاهدِ کاذب — پیش از سیم‌کشی انجام شد، نه بعد از آن.**
//      ابزار: tools/s589_false_witness_audit.py · سند: results/_s589/false_witness.json
//      (ابزار با بازتولیدِ اعدادِ منتشرشدهٔ §۴ سند اعتبارسنجی شد.)
//      · H8 در برابرِ **S1520 که روی همین کارت وصل است**: jaccard **۰.۵۲۵** ·
//        ۶۳.۸٪ از رویدادهای S589 · size_ratio ۰.۸۵ ⇒ زیرِ آستانهٔ ۰.۶۰ ⇒
//        **شاهدِ کاذب نیست**. ولی فاصله فقط ۰.۰۷۵ است و اندازه‌ها هم‌مقیاس:
//        دو گیتِ **متعامد** (حجم در برابرِ بدنهٔ کندل) روی **یک رویدادِ پایهٔ
//        مشترک** ⇒ ENTRYِ هم‌زمان **یک** فرصت است و **یک** پوزیشن می‌خواهد.
//      · H4 در برابرِ **S382 (تنها ساکنِ آن کارت)**: jaccard **۰.۱۴۱** ·
//        size_ratio ۰.۳۳ ⇒ قاطعانه مستقل (دو خانوادهٔ واقعاً متفاوت).
//      · هر دو کارت در برابرِ **S526**: ۱۰۰٪ زیرمجموعهٔ ساختاری (ذاتی — S589
//        همان رویداد + گیت است). S526 وصل **نیست** ⇒ امروز تعارضی نیست؛
//        قیدِ ماندگار: اگر وصل شود **باید** یکی از این دو حذف گردد.
//      ⇒ حکم: **CLEAR-WITH-CONSTRAINTS** روی هر دو کارت.
//
//    ⚠️ BUG-DATASETDRIFT (کشفِ گیتِ سلامت، نه حدس): حکمِ H4 روی
//      `data/XAUUSD_H4.csv` صادر شده که بالادست با کامیتِ `a8cd44cc` **حذف**
//      شده است؛ `data/mt5_full/XAUUSD_H4.csv.gz` نسخهٔ بعدی است (۲۳٬۸۵۴ کندل
//      در برابرِ ۲۳٬۷۵۵) و آن ۹۹ کندلِ اضافه میانهٔ ATR100 و در نتیجه براکتِ
//      منجمد را جابه‌جا می‌کند. فایلِ دورانِ حکم از تاریخِ گیت بازیابی شد و هر
//      ۵ سنجهٔ H4 **عیناً** بازتولید شد ⇒ اعدادِ بالا از همان نسخه‌اند.
//
//    ماژولار (ros2-مانند): فایلِ کاملاً مستقل volume_fresh_high_s589.ts؛ افزودنش
//      فقط دو ورودی در CARD_LAYERS می‌خواهد و هیچ لایهٔ دیگری را دست نمی‌زند —
//      هفت ساکنِ کارتِ H8 و ساکنِ یگانهٔ H4 بی‌تغییر می‌مانند.
//    پریتیِ پورت **GREEN** روی هر سه کارت (۲۶۰/۴۸۸/۱۸۷ سیگنال بیت‌به‌بیت،
//      max|Δrvol|=5e−7، کنترلِ منفیِ گیت، شاهدِ منفیِ MTF): web_tool/tools/_parity_s589.mts
//    سند: results/S589_VolumeConfirmedFreshHigh_Xauusd_H8H4_rqs2_88_ACCEPT.md
import { decideS589, S589_CFG } from './volume_fresh_high_s589'
// ⭐ S1516 ⭐نو — «کفِ تازهٔ تقویم‌همتا (۳۰ روزِ معاملاتی)» · XAUUSD-**H6 و H4**
//    H6(L=120) RQS2=87.1 · n=244 · WR=54.51% · lift=+13.64pp · z=3.24 · PF=1.853
//    H4(L=180) RQS2=84.1 · n=316 · WR=51.58% · lift=+10.51pp · z=3.27 · PF=1.622
//    H3(L=240) REJECT 16.5 ⇒ **عامدانه وصل نمی‌شود** (دیوارِ اسپرد؛ شاهدِ منفی
//      در هارنسِ پریتی اجرا می‌شود تا این تصمیمِ حکمی از سکوتِ پورت تفکیک شود).
//    ⚠️ ممیزیِ شاهدِ کاذب **پیش از** سیم‌کشی: در برابرِ ساکنانِ زندهٔ هر دو کارت
//      پاک است (H6: S919 0.004 · S955 0.013 · S607 0.007 · H4: S382 0.048 ·
//      S589 **0.094** · S547 0.0). موردِ حساس S589 بود — هم‌خانوادهٔ مفهومی
//      (رکوردِ تازه، سمتِ سقف) و هم‌اندازه (size_ratio 0.96) ⇒ دقیقاً شکلِ
//      پروندهٔ S404/S408؛ ولی عدد گفت هم‌اندازه‌اند و مجزا (۸۷ کندلِ مشترک).
//      🔒 قیدِ ماندگار: در برابرِ **والدش S1511 (L=90)** jaccard=**0.809** ⇒
//      شاهدِ کاذبِ واقعی. S1511 وصل نیست؛ اگر وصل شود «یکی، نه هر دو».
//    برابریِ پورت (بیت‌به‌بیت روی کلِ ۱۵.۶ سال — ۴۱۳ و ۵۱۸ رویداد، پنج سریِ
...
// ⭐⭐ S607 — «شوکِ انگل با دو گیتِ متعامد» (روند × رژیمِ σ) · **سه کارت**:
//    XAUUSD-**D1** (خام، بی‌گیت) + XAUUSD-**H8**-DUAL + XAUUSD-**H6**-DUAL
//    RQS2 = **83.1** (تنش ۸۲.۹) · هر ۱۱ دروازهٔ H0..H10 روی **استخرِ اتحادِ
//    سه کارت** سبز · n=۳۸۴ · WR=۶۳.۸۰٪ · lift=+۱۵.۹۸pp
//    قاعده: σ²_t = 0.94σ²_{t−1} + 0.06r²_{t−1} (IGARCH/RiskMetrics، λ منجمد از
//      S840) ⇒ z_t = r_t/σ_t؛ ورودِ **follow** وقتی |z| ≥ 2.618 روی کندلِ بسته.
//      هندسه: SL = slK×ATR34 وایلدر، TP = max(rr×SL, SL) — صفر پارامترِ نو.
//    دو گیتِ **متعامد** (فقط روی H8/H6 — منبعِ عمقِ آماری):
//      · گیتِ روندِ علّی (ارثی از S604): علامتِ close[i−1] − close[i−1−K] باید
//        هم‌جهتِ شوک باشد؛ K = ۶۰ روز × barsPerDay ⇒ H8: ۱۸۰ کندل، H6: ۲۴۰.
//        ⚠️ هر دو سرِ مقایسه **قبل** از کندلِ شوک‌اند ⇒ صفر نگاهِ آینده.
//      · گیتِ رژیمِ آرامِ σ (ارثی از S605/S606): σ[i] ÷ میانهٔ متحرکِ σ روی W=۲۳۳
//        کندلِ **shift(1)**‌شده باید ≤ ۱.۰ باشد ⇒ شوک در آرامش، نه در طوفان.
//    D1 عضوِ **خام** است (والدِ ACCEPTِ S840 بدونِ گیت) — این نامتقارنی عمدی و
//      از استخرِ رسمی ارث‌بری‌شده است، نه فراموشی.
//    ⛔ **XAUUSD-H12 عمداً وصل نمی‌شود**: انتخاب‌گرِ رسمیِ S607 آن را با حاشیهٔ
//      ۰.۱۵ حذف کرد. آزمونِ `web_tool/tools/_parity_s607.mts` این حذف را به یک
//      **شاهدِ منفیِ اجراشدنی** بدل کرده (assert می‌کند H12 در S607_CFG نیست).
//    پریتی: TS ↔ پایتون سبز روی هر سه کارت (سری‌ها، سیگنال‌ها، هندسهٔ SL/TP) و
//      n_dual هر کارت بیت‌به‌بیت برابرِ results/_s607_dual_gate/census.json.
//    سند: results/S607_EngleShockDualGatePool_Xauusd_D1H8H6_rqs2_83.1_ACCEPT.md
import { decideS607, S607_CFG } from './engle_dual_gate_s607'
...
// ⭐⭐ S1911 — شوکِ مطلع در آرامش (H8) — کندل‌های ورودیِ ctx باید H8 باشند
//    (index.tsx آن‌ها را با aggregateCandles(H1, 8) می‌سازد، عینِ S965/S966/S950).
// ⚠️ کفِ داده: این لایه **سنگین‌ترین** نیازِ دادهٔ کارتِ H8 را دارد — نه به‌خاطرِ
//    ATR21 بلکه به‌خاطرِ گیتِ آرامش: میانهٔ σ روی پنجرهٔ بستهٔ ۲۳۳ کندلی و خودِ σ
//    پس از ۵۰ کندل شروع می‌شود ⇒ کفِ ۲۸۵ کندلِ H8. اگر فیدِ سایت کوتاه‌تر باشد،
//    گاردِ minBars در computeS1911 صادقانه «دادهٔ ناکافی» می‌دهد و هرگز سیگنالِ
//    جعلی نمی‌سازد. پریتی ثابت کرد پورت به warm-up وابسته نیست: با پنجرهٔ ۳۰۰۰
//    کندلی به همان رویدادهای کلِ ۱۱٬۹۷۸ کندل رسید.
const s1911Layer = (cfg: typeof S1911_CFG[string]): LayerFn => (ctx) => decideS1911(cfg, ctx.a, ctx.candles, ctx.capital, ctx.riskPct)
// ⭐⭐ S955 — «جهشِ هم‌راستا در رژیمِ آرام» = رویدادِ S950 ∧ گیتِ آرامشِ σ.
//    سه کارتِ ACCEPT با **یک قاعدهٔ منجمد و صفر تیونِ per-card**:
//      H8 87.7 (n=112) · H12 89.4 (n=76) · H6 84.9 (n=149)
//    کندل‌های ورودیِ ctx باید هم‌تایم‌فریمِ کارت باشند — index.tsx آن‌ها را با
//    aggregateCandles(H1, 8/12/6) می‌سازد، دقیقاً همان مسیری که ساکنانِ فعلیِ
//    این سه کارت (S950/S1911/S965 · S800 · S919/S607) از آن تغذیه می‌کنند
//    ⇒ S955 **هیچ مسیرِ دادهٔ نویی نمی‌خواهد** و صفر ریسکِ رگرسیون می‌آورد.
// ⚠️ کفِ داده: هم‌ردهٔ S1911 سنگین است (۲۸۵ کندل: میانهٔ σ روی پنجرهٔ بستهٔ ۲۳۳
//    + آغازِ σ پس از ۵۰). روی فیدِ واقعی اندازه‌گیری شد و هر سه کارت جا دارند:
//    H8 (range=1y) ≈۹۰۴ کندل · H12 (2y) ≈۱۲۱۰ · H6 (2y) ≈۲۴۲۰ ⇒ حاشیهٔ ۳.۲×
//    تا ۸.۵×. پس هیچ‌یک از این سه کارت به سکوتِ دائمی نمی‌افتد. اگر روزی فید
//    کوتاه شد، گاردِ minBars در computeS955 صادقانه «دادهٔ ناکافی» می‌دهد.
const s955Layer = (cfg: typeof S955_CFG[string]): LayerFn => (ctx) => decideS955(cfg, ctx.a, ctx.candles, ctx.capital, ctx.riskPct)
...
// ⭐⭐ S607 — شوکِ انگل با دو گیتِ متعامد (D1 خام · H8-DUAL · H6-DUAL) — استخرِ سه‌عضوی.
//    کندل‌های ورودیِ ctx باید هم‌تایم‌فریمِ کارت باشند: index.tsx با
//    aggregateCandles(H1, 24) برای D1، ×8 برای H8 و ×6 برای H6 می‌سازد — دقیقاً
//    همان مرزهایی که کارت‌های S800/S770/S919 پیش‌تر تثبیت کرده‌اند، پس S607
//    **هیچ مسیرِ دادهٔ نویی نمی‌خواهد** و صفر ریسکِ رگرسیون به کارت‌ها می‌آورد.
// ⚠️ کفِ دادهٔ این لایه، متفاوت در هر کارت (خودِ ماژول در computeS607 گارد دارد):
//    · D1  ⇒ max(ATR34+2, بذرِ σ=51)                     = **۵۱ کندل**
//    · H8  ⇒ max(…, درفتِ K=180 ⇒ ۱۸۲, رژیمِ W=233 ⇒ ۲۳۴) = **۲۳۴ کندل**
//    · H6  ⇒ max(…, درفتِ K=240 ⇒ ۲۴۲, رژیمِ W=233 ⇒ ۲۳۴) = **۲۴۲ کندل**
//    عمقِ فیدِ زندهٔ فعلی (GOLD_TF): D1 ≈۷۲۰ (حاشیهٔ ۱۴×) · H8 ≈۷۸۰ (۳.۳×) ·
//    H6 ≈۲۴۱۶ (۱۰×) ⇒ هر سه کارت با حاشیهٔ راحت روشن می‌شوند.
//    ⚠️ ولی کفِ کارتِ H8 در index.tsx فعلاً **۱۱۰** است (ارثی از S950 که ۹۱+۲
//    کندل می‌خواست) — عددی کمتر از نیازِ ۲۳۴ِ این لایه. این **باگ نیست**، چون
//    آن کف فقط تصمیم می‌گیرد «آیا کارت تحلیل شود»، و فیدِ واقعی ۷۸۰ کندل
//    می‌دهد؛ در سناریوی نادرِ فیدِ ناقص هم گاردِ خودِ ماژول صادقانه «دادهٔ
//    ناکافی» برمی‌گرداند و هرگز سیگنالِ جعلی نمی‌سازد (دو خطِ دفاعی، همان
//    الگویی که S966 روی همین کارت پذیرفت).
const s607Layer = (cfg: typeof S607_CFG[string]): LayerFn => (ctx) => decideS607(cfg, ctx.a, ctx.candles, ctx.capital, ctx.riskPct)
// ⭐ S1520 — سقفِ تازهٔ ۹۰-کندلی × کندلِ مطلع (H8) — همان مسیرِ کندلِ H8 (H1×8)
//    که پنج ساکنِ فعلیِ کارت از آن تغذیه می‌کنند ⇒ هیچ منبعِ دادهٔ تازه‌ای لازم
//    نیست و ریسکِ ناهم‌ترازیِ تایم‌فریم صفر است.
//    ⚠️ نکتهٔ عملیاتیِ سنجیده‌شده: این لایه به **۱۹۰ کندلِ H8** نیاز دارد
//       (lookback ۹۰ برای سقف + ۱۰۰ برای ATRِ وایلدر) ⇒ حدودِ ۱۵۲۰ کندلِ H1.
//       اگر فید کوتاه‌تر باشد، گاردِ خودِ ماژول صادقانه «دادهٔ ناکافی» برمی‌گرداند
//       و هرگز سیگنالِ جعلی نمی‌سازد. سنگین‌ترین نیازِ دادهٔ کارتِ H8 است، پس
//       گامِ بعدی این را روی فیدِ **واقعی** بررسی می‌کند نه روی حدس.
const s1520Layer = (cfg: typeof S1520_CFG[string]): LayerFn => (ctx) => decideS1520(cfg, ctx.a, ctx.candles, ctx.capital, ctx.riskPct)
// ⭐⭐ S589 — سقفِ تازهٔ ۹۰-کندلی × تأییدِ حجمِ هم‌اسلات (H8 **و** H4) — دو حکمِ
//    مستقلِ تک-کارتی (نه استخری)، پس هر دو کارت طبقِ قانونِ MTF وصل می‌شوند.
//    کندل‌های ورودیِ ctx باید هم‌تایم‌فریمِ کارت باشند: index.tsx با
//    aggregateCandles(H1, 8) برای H8 و ×4 برای H4 می‌سازد — دقیقاً همان مسیری
//    که ساکنانِ فعلیِ این دو کارت (S1520/S965/… و S382) از آن تغذیه می‌کنند
//    ⇒ S589 **هیچ مسیرِ دادهٔ نویی نمی‌خواهد** و صفر ریسکِ رگرسیون می‌آورد.
//
// 🔴 **تنها نیازِ نوِ این لایه: ستونِ `volume`.** همهٔ لایه‌های وصل‌شدهٔ پیشین
//    فقط OHLC می‌خواستند، پس این نخستین بار است که سایت به بُعدِ پنجمِ داده
//    تکیه می‌کند. سه چیز اندازه‌گیری شد (نه فرض):
//      ① `Candle.volume` در `indicators.ts` وجود دارد و `_fetchGoldRaw`
//         آن را از Yahoo می‌خواند (`q.volume?.[i] ?? 0`).
//      ② `aggregateCandles` حجم را **جمع** می‌زند (نه میانگین/دورریز) ⇒
//         حجمِ کندلِ تجمیعیِ H8/H4 درست ساخته می‌شود.
//      ③ روی فیدِ واقعی: از ۱۷٬۳۹۷ کندلِ ساعتیِ خام ۷۶.۱٪ حجمِ ناصفر دارند،
//         و پس از تجمیع **۱۰۰٪** کندل‌های H8 (۱۹۳۹ کندل) و ۹۹.۹٪ کندل‌های
//         H4 حجمِ ناصفر دارند ⇒ گیت روی فیدِ زنده قابلِ ارزیابی است.
//    ⚠️ و اگر روزی نبود: `computeS589` صادقانه سکوت می‌کند (ENTRY نمی‌دهد و
//       در جدول می‌نویسد «حجمِ فید ناکافی») — چون سقفِ تازهٔ **بی‌تأییدِ حجم**
//       روی H4 لبهٔ اندازه‌گیری‌شدهٔ ۰.۱۷pp **منفی** دارد. دو خطِ دفاعی، همان
//       الگویی که S966/S607 روی همین کارت‌ها پذیرفتند.
//
// ⚠️ کفِ دادهٔ این لایه: `lookback + atrP` = **۱۹۰ کندل** روی هر دو کارت
//    (سقفِ غلتانِ ۹۰ + هم‌گراییِ ATR100) ⇒ ~۱۵۲۰ کندلِ H1 برای H8 و ~۷۶۰ برای H4.
//    قیدِ گرم‌شدنِ گیتِ حجم از این کوچک‌تر است (۲۰ رخدادِ هم‌اسلات ⇒ ۶۰ کندلِ H8
...
// ---------------------------------------------------------------------------
// 🔎 CARD_LAYER_CODES — کدِ لایه‌های هر کارت، به همان **ترتیبِ اولویتِ واقعی**
// ---------------------------------------------------------------------------
// برای بخشِ «info» کارت‌ها (User Note). چرا این جدولِ جدا لازم است و نمی‌شود
// کدها را از خودِ `CARD_LAYERS` خواند: عناصرِ آن آرایه **توابعِ بسته‌شده**اند
// (closure)؛ کدِ استراتژی داخلِ بدنهٔ `decideSxxx` ساخته می‌شود و فقط وقتی
// وجود پیدا می‌کند که لایه اجرا شود **و سیگنال بدهد**. برای لایهٔ ساکت هیچ
// شیئی نیست که کدش را از آن بپرسیم.
//
// ⚠️ این جدول باید **همیشه** با CARD_LAYERS هم‌گام بماند. تکیه بر «یادمان
//    باشد» کافی نیست، پس یک آزمونِ هم‌گامی نوشته شد که هر دو سمت را می‌شمارد:
//        web_tool/tools/_probe_layer_catalog.mts
//    اگر کسی لایه‌ای اضافه/حذف کند و این‌جا را به‌روز نکند، آن آزمون قرمز
//    می‌شود. **ترتیب هم معنادار است**: ترتیبِ این آرایه باید عیناً ترتیبِ
//    CARD_LAYERS باشد، چون کاربر در info می‌خواند «کدام لایه اولویتِ بالاتری
//    دارد» — و ترتیبِ اشتباه یعنی اطلاعاتِ غلط، نه صرفاً نامرتبی.
//
// نکته: کلیدِ `S333` روی کارتِ M30 عمداً همین‌جا هم S333 است (نه S431) چون
// این جدول **شناسهٔ تابعِ وصل‌شده** را نگه می‌دارد؛ نامِ نمایشیِ S431 در
// layer_catalog.ts نشسته. اگر این دو را یکی می‌کردم، آزمونِ هم‌گامی دیگر
...
// ---------------------------------------------------------------------------
// 🛡️ قیدِ «شاهدِ کاذبِ بین‌کارتی» — وقتی یک لایه روی چند تایم‌فریم هم‌رویداد است.
// ---------------------------------------------------------------------------
// `FALSE_WITNESS_PAIRS` بالا یک حالت را پوشش می‌دهد: دو لایهٔ **هم‌کارت**. ولی
// حالتِ دومی هم هست که آن مکانیزم ساختاراً نمی‌بیند، چون `dropFalseWitnesses`
// فقط `ctx.cardId` جاری را در اختیار دارد و هر کارت جداگانه رندر می‌شود:
//
//     یک لایهٔ واحد که روی **چند کارت** ACCEPT گرفته، ولی رویدادش روزانه است
//     ⇒ همان n روی همهٔ تایم‌فریم‌ها ⇒ کارت‌ها سه *بدیل* اند، نه سه شاهد.
//
// نمونهٔ محرزِ همین استقرار: S547 روی M15/M30/H4 هر سه ACCEPT است (۸۴.۷ / ۸۹.۳ /
// ۸۱.۹) ولی n هر سه دقیقاً **۱۴۶** است — چون رویداد «روزِ پیش‌تعطیلات» است و
// روز، مستقل از تایم‌فریم، یکی است. سندِ ACCEPT §۸ صریح است: «H4 هم‌رویداد با
// M30 است (همان ۱۴۶ روز) ⇒ جانشین/بدیل، نه کارتِ مستقل؛ هم‌زمان معامله نشوند».
//
// اگر این قید ثبت نشود، کاربر در یک روزِ P سه کادرِ سبز روی سه کارت می‌بیند و
// طبیعی است که نتیجه بگیرد سه شاهدِ مستقل دارد ⇒ سه برابر ریسک روی **یک** رویداد،
// بدونِ هیچ اطلاعِ تازه. این دقیقاً همان «شاهدِ کاذب» است، فقط در بُعدِ تایم‌فریم
// به‌جای بُعدِ لایه. مکانیزمِ نمایش نمی‌تواند حلش کند؛ فقط اندازه‌گیریِ قبلی
// (۱۴۶ = ۱۴۶ = ۱۴۶ در جدولِ سند) می‌تواند.
//
// راهِ حل، هم‌سو با فلسفهٔ جدولِ بالا: قید در **داده** ثبت می‌شود نه در نثر.
// کارتِ `primary` نگه داشته می‌شود و بقیه برچسبِ «بدیل» می‌گیرند — حذف نمی‌شوند،
// چون کاربرِ کارتِ M15 باید بتواند لایه‌اش را ببیند؛ فقط باید بداند که این همان
// رویدادِ کارتِ M30 است با براکتِ متفاوت، نه یک تأییدِ دوم.
interface CrossCardAlternate {
  code: string             // کدِ لایه
  cards: string[]          // کارت‌هایی که همان رویداد را می‌بینند
  primaryCard: string      // کارتِ مرجع (بالاترین RQS2) — «اگر یکی، این یکی»
  sharedEventNote: string
}
const CROSS_CARD_ALTERNATES: CrossCardAlternate[] = [
  {
    code: 'S547',
    cards: ['XAUUSD-M15', 'XAUUSD-M30', 'XAUUSD-H4'],
    primaryCard: 'XAUUSD-M30',
    sharedEventNote:
      'هر سه کارت همان ۱۴۶ روزِ پیش‌تعطیلات را معامله می‌کنند (n مستقل از تایم‌فریم است). '
      + 'قلهٔ حکم M30 است (RQS2 ۸۹.۳ · z=۴.۰۰)؛ M15 (۸۴.۷) و H4 (۸۱.۹) بدیل‌اند، نه شاهدِ دوم. '
      + 'سندِ ACCEPT §۸ + قانونِ S605: هم‌زمان معامله نشوند.',
  },
]

/**
 * اگر لایهٔ فعالِ این کارت عضوِ یک خانوادهٔ هم‌رویدادِ بین‌کارتی باشد، نشانه‌گذاری
 * می‌شود. هیچ حذفی انجام نمی‌شود — هدف فقط این است که کاربر «شاهدِ دوم» را با
 * «همان شاهد از پنجرهٔ دیگر» اشتباه نگیرد.
 */
function markCrossCardAlternates(d: RouterDecision, cardId: string): void {
