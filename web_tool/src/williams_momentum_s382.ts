...
export function computeS382(candles: Candle[], cfg: S382Config): RawSignal {
  ...
  // ⚠️ حالتِ «رژیمِ نوسانِ خارج از بازهٔ آزموده‌شده» — باید به کاربر **گفته** شود.
  //    اندازه‌گیریِ واقعی روی همین داده (۲۳٬۷۵۵ کندل، ۱۵.۷ سال):
  //      • میانهٔ ATR(100) = ۸.۱۹۰ ⇒ SL = ۱۲۲.۹ pip ✔ عیناً عددِ قفل‌شدهٔ سند
  //      • ۷۱.۱٪ تاریخ داخلِ بندِ [۰.۶۰×, ۱.۶۰×] ⇒ بند در اکثرِ مواقع بی‌اثر است
  //      • ۱۶.۴٪ بالای بند، ۱۲.۵٪ زیرِ بند
  //    مسئله: اگر ATR زنده بسیار بالاتر از بند باشد، بستنِ SL روی سقفِ بند یعنی
  //    استاپی **تنگ‌تر از نوسانِ جاریِ بازار** ⇒ خروجِ تصادفی. جهتِ دیگر (ATR
  //    بسیار پایین) استاپی **پهن‌تر از لازم** می‌دهد ⇒ ریسکِ دلاریِ بیش از حد.
  //    در هر دو حالت، حکمِ اندازه‌گیری‌شده (RQS2=۸۳.۵) دیگر عیناً منتقل نمی‌شود.
  //    پس بند را نگه می‌داریم (تا از هندسهٔ آزموده‌شده دور نشویم) ولی وضعیت را
  //    **صریحاً گزارش** می‌کنیم؛ پنهان‌کردنش کاربر را به معامله‌ای می‌کشاند که
  //    گمان می‌کند آزموده شده و نیست.
  let geomClamp: 'in-band' | 'clamped-high' | 'clamped-low' = 'in-band'
  let atrRatio = 1.0
  ...
  if (crossedUp) {
    reason =
      ...
      (geomClamp === 'clamped-high'
        ? ` ⚠️ توجه: نوسانِ جاری ${atrRatio.toFixed(2)}× بالاتر از نوسانی است که این ` +
          `لایه در آن آزموده شد. حد ضرر روی سقفِ مجاز (${slPipShow} pip) بسته شد، ` +
          `که از نوسانِ فعلیِ بازار **تنگ‌تر** است ⇒ احتمالِ خروجِ تصادفی بالاتر از ` +
          `آنچه در بک‌تست دیده شد. حجم را کوچک‌تر بگیرید یا منتظرِ آرام‌شدنِ بازار بمانید.`
        : (geomClamp === 'clamped-low'
          ? ` ⚠️ توجه: نوسانِ جاری ${atrRatio.toFixed(2)}× پایین‌ترِ نوسانِ آزموده‌شده است. ` +
            `حد ضرر روی کفِ مجاز (${slPipShow} pip) بسته شد، که از نوسانِ فعلی **پهن‌تر** ` +
            `است ⇒ ریسکِ دلاریِ هر معامله بیش از حالتِ آزموده‌شده.`
          : ''))
  } else if (approaching) {
    reason =
