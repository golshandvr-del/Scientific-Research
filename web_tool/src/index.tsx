...
app.post('/api/trade/advice', async (c) => {
  try {
    ...
    if (meta_asset.isGold) {
      ...
      // ⚠️ S1520 کفِ H8 را از ۸۸۰ به **۱۵۲۰** بالا برد. دلیلش اندازه‌گیری است نه
      //    احتیاط: کفِ ۸۸۰ فقط ۱۱۰ کندلِ H8 است و برای S950 (گرم‌شدنِ ۹۱ کندل)
      //    نوشته شده بود، اما S1520 به **۱۹۰ کندلِ H8** نیاز دارد (۹۰ برای سقفِ
      //    غلتانِ تازه + ۱۰۰ برای بازگشتِ ATRِ وایلدر) ⇒ سنگین‌ترین مصرف‌کنندهٔ
      //    این کارت. با کفِ قدیمی، در حالتِ **تخریب‌شدهٔ** فید (مثلاً ۱۵۰ کندلِ H8
      //    پس از قطعیِ منبع) نگهبان عبور می‌داد (۱۵۰>۱۱۰) و کارت رندر می‌شد، ولی
      //    S1520 در سکوت گرسنه می‌ماند و همیشه خنثی برمی‌گشت — حالتی که از UI
      //    قابلِ تفکیک از «سیگنالی ندارم» نیست (گران‌ترین نوعِ باگِ سیم‌کشی).
      //    فیدِ زنده اکنون ۵۶۹۵ کندلِ H1 = ۸۰۳ کندلِ H8 می‌دهد (حاشیهٔ ۴.۲۳×
      //    نیازِ لایه و ۱.۸۹× همین کفِ نو) ⇒ کفِ ۱۵۲۰ کارتِ سالم را رد نمی‌کند.
      //    ابزارِ بازاندازه‌گیری: web_tool/tools/_feed_s1520.mts
      const minBars = aggF === 4 ? 240 : aggF === 6 ? 1560 : aggF === 8 ? 1520
...
const GOLD_TF: Record<string, { interval: string; range: string; gap: number }> = {
  ...
  'XAUUSD-H6': { interval: '1h',  range: '2y',  gap: 3600 },  // H6 از تجمیعِ H1×6
...
async function decideAsset(a: typeof ASSETS[number], capital = 10000, riskPct = 1.0) {
  if (a.isGold) {
    ...
    // ⚠️ H6 (S919): **عمیق‌ترین کفِ سایت.** گیتِ قراردادِ کینزی به
    //   close[t−1−240] با t=n−2 نگاه می‌کند ⇒ کفِ ریاضیِ ۲۴۳ کندل. کفِ ۲۶۰
    //   انتخاب شد (حاشیهٔ ۱۷ کندل ≈ ۴ روز) تا در تعطیلی‌ها لایه به‌جای
    //   سیگنالِ ناقص، صادقانه «دادهٔ ناکافی» بدهد. خودِ ماژول هم گاردِ مستقل
    //   دارد (دو خطِ دفاعی) — این کف فقط جلوی هزینهٔ بی‌فایدهٔ تحلیل را می‌گیرد.
