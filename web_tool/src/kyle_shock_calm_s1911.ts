...
export function decideS1911(
  ...
  const meta: DecideMeta = {
    ...
    filters: [
      ...
      `🆕 گیتِ آرامشِ رژیم: σ_t ÷ میانهٔ σ(${cfg.calmW} کندلِ **بسته**) ≤ ${cfg.regMax} ` +
        `(RiskMetrics λ=${cfg.lam}) — پایهٔ منجمدِ S605/S606، علّی`,
      `تفکیک‌گرِ اصلی: همان شوک در **طوفان** ورود نمی‌دهد ⇒ RQS2 از ۸۲.۲ به ۹۳.۹`,
      `هندسهٔ نامتقارنِ TP>SL (${cfg.kSl}/${cfg.kTp}) — ارثی از S965 · ~۵ معامله در سال`,
      `⚠️ زیرمجموعهٔ ۱۰۰٪ِ S965 (۸۲ از ۱۴۴ · jaccard ۰.۵۷) ⇒ فیلترِ کیفیت، ` +
        `نه شاهدِ مستقل ⇒ سایزِ مشترک در تلاقی`,
    ],
  }
