...
export function decideS560(
  ...
  if (active) {
    ...
    reason =
      ...
      `کندلِ ماقبلِ مرزِ روز هم با وقفه‌ای بزرگ‌تر از حدِ مجاز آمده، یعنی ` +
      `«بستهٔ روزِ قبل» یک قیمتِ **کهنه** است و گپِ ${gapAbs.toFixed(2)}$ی محاسبه‌شده ` +
      `مصنوعِ شکافِ داده است، نه رفتارِ واقعیِ بازار. معامله روی چنین گپی ` +
      `بی‌معنا است. (اندازهٔ اثرِ این گارد روی کلِ تاریخ: تنها ۳ سیگنال از ۴۲۳ ` +
      `= ۰.۷۱٪، هر سه در ناحیهٔ معیوبِ ژوئنِ ۲۰۱۳ ⇒ حکمِ آماری دست‌نخورده است.)`
  } else if (s.gapUsd >= 0) {
    reason =
