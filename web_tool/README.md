# سایتِ دستیارِ تصمیمِ چند-دارایی 🧭

> ## ⚠️ معیارِ موفقیت (User Note 2) — مرجع: [`../PARADIGM.md`](../PARADIGM.md)
> معیارِ موفقیتِ کلِ محصول **فقط «سودِ خالصِ بیشتر»** است (نه WR/PF/فرکانس).
> این سایت یک **دستیارِ تصمیمِ دو-دارایی و ۴-حالته** است: برای هر دارایی
> (`XAUUSD`, `EURUSD` — طبقِ User Note به دو داراییِ سوددهْ کاهش یافت) یک کارت با ماشینِ حالتِ
> **خنثی → نزدیک‌شدن به سیگنال → ورود (TP/SL) → مدیریتِ معامله**. پشتِ صحنه یک
> **Routerِ رژیم-محور** (`src/router.ts`) تصمیم می‌گیرد ولی
> کاربر فقط تصمیمِ نهایی را می‌بیند. گذارِ ورود→مدیریت وابسته به ثبتِ معاملهٔ کاربر است.

> ## 🥇 رکوردِ فعلیِ سودِ خالصِ کل: **+۱۲۸٬۳۲۵$** (تعریفِ رسمی = XAUUSD + EURUSD)
> **قانونِ شمارهٔ ۱ (بازتأکید): تنها معیارِ این محصول «سودِ خالصِ بیشتر» است — نه WR، نه PF، نه فرکانس.**
> آخرین چرخه (پاسخِ User Note ها) هدفش **افزایشِ سود از راهِ کاهشِ سیگنال‌های غلط (کاهشِ ضرر)** بود، نه ماشهٔ نو.
> دو **فیلترِ کیفیت** روی لایهٔ `Squeeze→Breakout` (`src/squeeze_breakout.ts`) در سایت پیاده و به کاربر با ذکرِ دلیل نمایش داده می‌شوند:
> - **S136 — «قدرتِ شکست» (`brk_strength=(close−priorHigh)/ATR14 ≥ 0.30`):** شکستِ ضعیف/کاذب رد می‌شود ⇒ رکورد +۱۲۱٬۶۹۴$→**+۱۲۶٬۱۱۸$**.
> - **S138 — «RSI اشباعِ خرید» (`RSI14 ≤ 75`):** شکستِ صعودی در اوجِ اشباع اغلب «شکستِ خسته» است ⇒ رد می‌شود ⇒ رکورد +۱۲۶٬۱۱۸$→**+۱۲۸٬۳۲۵$**.
> وقتی هر یک از این فیلترها شکست را رد کند، کارتِ دارایی حالتِ **«خنثی»** را با توضیحِ صریحِ عددِ شاخص (قدرتِ شکست / RSI۱۴) به کاربر نشان می‌دهد.
> جزئیاتِ علمی: [`../results/SqueezeBreakoutFilter_NetProfit_126118.md`](../results/SqueezeBreakoutFilter_NetProfit_126118.md) و [`../results/SqueezeSecondFilter_RSI_NetProfit_128325.md`](../results/SqueezeSecondFilter_RSI_NetProfit_128325.md).

> ## 🏆 برندهٔ سودِ خالص که در سایت اعمال شده: **S66 (ER=۰.۱۵ روی Adaptive-TP + Kelly Router)**
> سایت اکنون **هر چهار بُعدِ برندهٔ سودِ خالص** را پیاده کرده است (نه فقط S63 پایه):
> **(۱) کجا وارد شویم** (S63/L36 — Router رژیم-محور)، **(۲) با چه حجمی** (S64/L38 —
> حجمِ Kelly رژیم-آگاه)، **(۳) تا کجا سود بگیریم** (S65/L39 — TPِ رژیم-محور)، و
> **(۴) کِی محیط را «روندی» بدانیم** (S66/L40 — آستانهٔ `ER_TREND_THR=0.15`، راهکار A).
> در بک‌تستِ walk-forward روی ۱۵۰٬۰۰۰ کندل، این ترکیب سودِ خالص را از **۲۹۴۹$ به
> ۶۸۲۳$ (+۱۳۱٪)** رساند — با آنکه WR از ۶۲٪ به ۵۰.۷٪ افت کرد و سود/DD به ۹.۸۰ بهبود یافت.
> **این خودْ اثباتِ قانونِ شمارهٔ ۱ است: WR مهم نیست، سودِ خالص مهم است.**

> ### 🧹 بازطراحیِ کامل (User Note)
> نسخهٔ قبلی پرباگ و شلوغ بود (نمودار، بخشِ «وضعیتِ تحقیقِ علمی»، اجرای ONNX در
> مرورگر، انبوهِ اطلاعاتِ غیرمفید). همهٔ این‌ها **حذف شدند**. اکنون UI فقط شاملِ
> کارتِ هر دارایی با **یک باکسِ ماشینِ حالتِ ۴-وضعیتی** + جدولِ اختیاریِ شاخص‌ها
> (برای شفافیت) است. هیچ نمودار یا بخشِ تحقیقی وجود ندارد.

## قابلیت‌ها
- 🧭 **چهار حالتِ تصمیم برای هر دارایی** (Router رژیم-محور، S63/L36):
  - **خنثی:** صراحتاً می‌گوید وارد نمی‌شود + دلیل (رژیمِ رنج، ER پایین، proba کم) با اعداد.
  - **نزدیک‌شدن به سیگنال:** ستاپ در حالِ شکل‌گیری + فهرستِ تأییدهایِ موردِ انتظار.
  - **ورود:** جهت (Long/Short) + TP + SL + R:R + احتمال + دکمهٔ «معامله را ثبت کردم».
    **به‌علاوه اکنون دو اهرمِ سودِ خالص هم نمایش داده می‌شوند** (پایین):
    - 🪙 **حجمِ پیشنهادی (S64/L38):** ضریبِ حجمِ رژیم-آگاه (Kelly) بر اساسِ سطلِ رژیم؛
      سطلِ قویِ `trend_hi` → ~۲× حجمِ پایه، سطلِ مرزی → کاهش‌یافته. در بک‌تست سودِ خالص را ~۸۳٪ بالا برد.
    - 🎯 **هدفِ سودِ رژیم-محور (S65/L39):** ضریبِ TP دیگر ثابت نیست؛ در رژیمِ روندیِ
      کارآمد TPِ دورتر (تا ۲.۰×ATR در Bull، ۲.۶× در Bear) و در رژیمِ کم‌کارا TP نزدیک‌تر.
  - **مدیریتِ معامله:** فقط پس از ثبتِ کاربر؛ سود/زیان (R)، پیشرفت به TP، جمع‌بندیِ اقدام،
    و توصیه‌های زندهٔ SL/TP/بستن (با اعمالِ تک‌کلیکی).
- 🪙 **دو دارایی:** XAUUSD (موتورِ S67 — `router.ts`) و EURUSD (موتورِ نوِ S73 Session-Open Drift — `eurusd_router.ts`) — هر کدام موتورِ تصمیمِ **مستقلِ** خودش. DXY/AUDUSD حذف شدند (لبهٔ سوددهی نداشتند). **سودِ خالصِ کل = XAUUSD + EURUSD = +۴۴٬۴۵۸$.**
- 📡 **داده زنده:** کندلِ M15 از Yahoo Finance (طلا `GC=F` + rebase به spot؛ بقیه مستقیم).
- 🔬 **جدولِ شفافیت (اختیاری/تاشو):** مقادیرِ کلیدیِ تصمیم (روند، ER، ADX، proba، RSI، ATR).
- 💾 **ذخیرهٔ معامله در localStorage** (مقاوم به رفرش)، حذف فقط با دکمهٔ «بستنِ معامله».
- ⏱️ **بروزرسانی خودکار** هر ۳۰ ثانیه.
- 🧹 **حذف‌شده‌ها (بازطراحی):** نمودار شمعی، اجرای ONNX در مرورگر، بخشِ «وضعیتِ تحقیقِ
  علمی»، تحلیلِ MTF/بین‌بازاری/اخبار در UI، و همهٔ اطلاعاتِ اضافه. UI فقط تصمیمِ نهایی است.

## معماری فنی
- **Backend:** Hono روی Cloudflare Pages/Workers (Edge).
- **موتور تحلیل (TypeScript):** بازتولید دقیق ماژول‌های پایتونِ پروژه:
  - `src/indicators.ts` ← `engine/indicators.py` (RSI, ATR, MACD, ADX, Bollinger, Stoch, z-score, slope…)
  - `src/structure.ts` ← `engine/structure.py` (Pivot + سطوح S/R فعال با ادغام و انقضا)
  - `src/features.ts` ← `engine/features.py` (بازتولید دقیق ۵۷ feature ورودی مدل، بدون look-ahead)
  - `src/signal.ts` ← موتور امتیازدهی شفاف برای تفسیرپذیری
  - `src/external.ts` ← MTF (H1/H4/D1) + بین‌بازاری (DXY/TNX) + تقویم اخبار
- **مدل ONNX (مرورگر):** `src/browser/signal_client.ts` (باندل `esbuild` → `public/static/browser-signal.js`)
  با `onnxruntime-web` (WASM) از CDN؛ مدل‌ها در `public/static/models/`.
- **Frontend:** HTML + TailwindCSS (CDN) + Chart.js (candlestick financial) — فارسی/RTL.
- **ابزار اعتبارسنجی:** `tools/export_parity_reference.py` (مرجع پایتون) و `tools/verify_parity.mjs` (آزمون TS↔Python).

### ✅ اجرای واقعی مدل ONNX در مرورگر (نه تقریب)
در نسخه‌ی پیشین، اجرای مدل ML در سرور لبه ممکن نبود و از یک تقریب امتیازدهی استفاده می‌شد.
**اکنون این محدودیت رفع شده است:** هر ۳ فایل مدلِ ensemble ربات
(`xauusd_s14_model_{0,1,2}.onnx`) با کتابخانه‌ی **`onnxruntime-web` (WASM)** مستقیماً در
**مرورگر کاربر** بارگذاری و اجرا می‌شوند.

جریان کار (فایل `src/browser/signal_client.ts` → باندل `public/static/browser-signal.js`):
1. کندل‌های زندهی M15 (~۴۰۰۰–۵۷۰۰ کندل، ۶۰ روز) از `/api/candles` دریافت می‌شوند.
2. `buildFeatures` (فایل `src/features.ts`) دقیقاً معادل `engine/features.py` ۵۷ feature می‌سازد.
3. هر ۳ مدل ONNX روی بردار feature اجرا و میانگینِ احتمال کلاس «برد» گرفته می‌شود.
4. تصمیم LONG/NONE با آستانه‌ی `THR=0.68` + شرط رژیم صعودی — دقیقاً منطق ربات MT5.

**اعتبارسنجی parity:** خروجی TS+ONNX در برابر مرجع پایتون روی ۲۰۰۰ کندل آزمون شد:
- اختلاف feature‌ها: ~`3.6e-6` (فقط خطای گرد‌کردن float32)
- تطابق رژیم: **۱۰۰٪**
- تطابق تصمیم نهایی LONG/NONE: **۹۹.۶۵٪** (تنها ۷ اختلاف در کندل‌هایی که احتمال دقیقاً روی مرز ۰.۶۸ است)

این دیگر یک تقریب نیست — **خودِ مدل ربات** است که در مرورگر اجرا می‌شود.
«موتور امتیازدهی شفاف» همچنان وجود دارد اما فقط برای **تفسیرپذیری** (توضیح سهم عوامل)؛ تصمیم نهایی با مدل واقعی ONNX است.

## API
| مسیر | پارامتر | توضیح |
|------|---------|-------|
| `GET /api/decision` | — | تصمیمِ ۴-حالتهٔ **همهٔ دارایی‌ها** یک‌جا (موازی، مقاوم به خطای هر دارایی) |
| `GET /api/decision/:asset` | `asset` ∈ {XAUUSD,EURUSD} | تصمیمِ یک دارایی |
| `POST /api/trade/advice` | body: `{ asset, trade:{side,entry,tp,sl}, modelProbPct? }` | مدیریتِ معاملهٔ باز (حالت ۴) برای دارایی مشخص |
| `GET /api/health` | — | بررسی سلامت سرویس |
| `GET /api/candles` | `interval`,`range` | (قدیمی/داخلی) کندل‌های خامِ طلا |
| `GET /api/analysis` | `interval`,`range` | (قدیمی/داخلی) تحلیلِ کاملِ طلا |

نمونه: `/api/decision` ، `/api/decision/EURUSD`

> نکته: endpointهای قدیمیِ `/api/mtf`، `/api/intermarket`، `/api/news`، `/api/context`
> و `/api/spot` هنوز در backend موجودند اما دیگر در UI استفاده نمی‌شوند (بازطراحی).

## اجرای محلی (سندباکس)
```bash
cd web_tool
npm install
npm run build
pm2 start ecosystem.config.cjs      # اجرا روی پورت 3000
curl http://localhost:3000/api/health
```

## اجرا روی سرور مجازی / هاست خودتان
این پروژه Cloudflare Pages است، اما روی هر جای دیگری هم به‌سادگی اجرا می‌شود:

**گزینه ۱ — دیپلوی رایگان روی Cloudflare Pages (پیشنهادی):**
```bash
cd web_tool
npm run build
npx wrangler pages deploy dist --project-name xauusd-live-tool
```

**گزینه ۲ — روی VPS/هاست خودتان با Node:**
```bash
cd web_tool
npm install && npm run build
# اجرای دائم با PM2:
pm2 start "npx wrangler pages dev dist --ip 0.0.0.0 --port 3000" --name gold-tool
```
سپس با یک reverse proxy (Nginx/Caddy) روی دامنه‌ی خودتان قرار دهید.

> اگر Yahoo از IP سرور شما محدود شد، می‌توانید در `src/index.tsx` تابع `fetchGold`
> را به یک provider دیگر (مثلاً Twelve Data با کلید رایگان، یا بروکر خودتان) تغییر دهید.

## سلب مسئولیت
این ابزار صرفاً برای **تحقیق علمی** است و **توصیه‌ی مالی نیست**. معامله در بازار با ریسک همراه است.
