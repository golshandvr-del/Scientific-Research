...
> ## ⚠️ جهت‌گیریِ جدید (User Note 2) — مرجع: [`../PARADIGM.md`](../PARADIGM.md)
> معیارِ موفقیتِ کلِ محصول از این پس **فقط «سودِ خالصِ بیشتر»** است (نه WR/PF/فرکانس).
> این ابزار باید به یک **سایتِ دستیارِ تصمیمِ چنددارایی و ۴-حالته** تبدیل شود:
> برای هر دارایی (`XAUUSD`, `DXY`, `EURUSD`, `AUDUSD`, ...) یک بخش با ماشینِ حالتِ
> **خنثی → نزدیک‌شدن به سیگنال → ورود (TP/SL) → مدیریتِ معامله**. پشتِ صحنه یک
> **Routerِ رژیم-محور** ده‌ها استراتژی را اجرا می‌کند ولی کاربر فقط تصمیمِ نهایی
> را می‌بیند. گذارِ ورود→مدیریت وابسته به ثبتِ معاملهٔ کاربر است.
...
- 🤝 **مدیریت معاملهٔ باز کاربر (Trade Advisor):** کاربر معامله‌ای که در حساب دمو باز کرده (جهت long/short + قیمت ورود + TP + SL) را در سایت ثبت می‌کند و سایت آن را **به‌صورت زنده مدیریت** می‌کند:
  ...
  - 🐞 **رفع باگ‌های مدیریت معامله (پاسخ به User Note):**
    1. **حذف پیشنهاد TPِ زیانده:** پیش‌تر ممکن بود در معاملهٔ خرید، هنگام وجود مقاومت
       زیرِ قیمت ورود، سیستم پیشنهاد دهد TP را زیرِ قیمت ورود بیاورید (ضرر تضمینی).
       اکنون پیشنهاد اصلاح TP فقط زمانی داده می‌شود که مقدار جدید همچنان در سمت سود و
       با فاصلهٔ منطقی (≥۰.۵×ATR) از ورود باشد؛ در غیر این‌صورت به‌جای آن **هشدار خطر**
       («مقاومت زیرِ قیمت ورود توست») نمایش داده می‌شود.
    2. **رفع تناقض جمع‌بندی:** «اقدام کلی» دیگر وقتی هشدار فعالی وجود دارد، پیام
       «شرایط پایدار است» نمی‌دهد؛ جمع‌بندی با شدیدترین باکس هشدار هم‌خوان شد.
    3. **اصلاح باکس مدل:** آستانهٔ واقعی مدل (**۶۸٪**، همان `THR` مدل ONNX) اعمال شد؛
       احتمال‌های بالای ۶۸٪ «هم‌سو با معامله» و زیر ۵۰٪ «ضعف» نامیده می‌شوند (بازهٔ
       میانی خنثی). دیگر عدد ۷۱٪ به‌غلط «احتمال پایین» خوانده نمی‌شود و این باکس به‌خاطر
       `regimeOk` سرور به‌صورت همیشگی و گمراه‌کننده ظاهر نمی‌گردد.
