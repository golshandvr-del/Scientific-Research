...
| مسیر | پارامتر | توضیح |
|------|---------|-------|
| `GET /api/assets` | — | متادیتای فوریِ **۹ کارت** + فهرستِ لایه‌های هر کارت (میدانِ `layers`). بدونِ هیچ fetchِ بالادست ⇒ **~۷ms**؛ همین نقطه فازِ ۱ را تغذیه می‌کند |
| `GET /api/decision` | — | تصمیمِ ۴-حالتهٔ **همهٔ ۹ کارت** یک‌جا (موازی، مقاوم به خطای هر کارت) |
| `GET /api/decision/:asset` | `asset` ∈ {XAUUSD-M5, XAUUSD (=M15), XAUUSD-M30, XAUUSD-H1, XAUUSD-H4, XAUUSD-H6, XAUUSD-H8, XAUUSD-H12, XAUUSD-D1} | تصمیمِ یک کارت (از `runCard`) |
