...
def build_null(asset: str, tf: str, n_perm: int = N_PERM, seed: int = SEED,
    ...
    # 🔴 BUG-NULLUNCOND (گامِ ۳۷): تا این گام همین فراخوانی `trail_pip=None`
    # داشت و `be_trigger_pip` نداشت ⇒ خطِ مبنای بی‌قید **بدونِ** تریلینگ
    # اجرا می‌شد در حالی که سیگنال **با** تریلینگ. یعنی همان «تلهٔ ۲»ی که
    # در گامِ ۳۴ برای جای‌گشت‌ها رفع کردم، در همین چند خط بالاتر **باقی
    # مانده بود**. من فقط نمونهٔ اولِ باگ را دیدم و همزادش را ندیدم.
    #
    # و جهتِ آسیب **معکوسِ** فرضِ من بود: با اندازه‌گیری معلوم شد تریلینگ
    # WR را **پایین** می‌آورد نه بالا (۴۸.۳۷٪ بی‌تریلینگ ← ۴۳.۷۳٪ با
    # تریلینگ)، چون استاپِ متحرک معامله را زودتر می‌بندد و بخشی از
    # برنده‌های کوچک را به زیانِ کوچک بدل می‌کند. پس خطِ مبنا مصنوعاً
    # **بالا** رفته بود و مهارتِ لایه به‌غلط **منفی** به نظر می‌رسید:
    # نزدیک بود لایه‌ای سالم را با شاهدِ آلوده اعدام کنم.
