...
def run_one(card, rule_name, sl_k, rr, L, NM, RB, df, atr_med, ps, sig):
    ...
    # `uncond_baseline` سیگنالِ لایه را لازم ندارد؛ فقط هندسه را می‌خواهد،
    # پس روی هر سه گامِ نمونه‌برداری اجرا و **سخت‌ترین** انتخاب می‌شود.
    unc = max(NM.uncond_baseline(L, df, sl_abs, ps, s)[0] or -1e9
              for s in (1, 3, 7))
    # جای‌گشت: همان **تعدادِ** سیگنال، زمان‌بندیِ تصادفی.
    # نکته: `perm_baseline` برای هندسه از `sl_abs` و `L.RR` استفاده می‌کند،
    # پس `L.RR` موقتاً روی rrِ این نامزد قرار می‌گیرد تا هندسهٔ مدلِ صفر
    # **عیناً** هندسهٔ نامزد باشد (شرطِ عدالتِ مقایسه).
    _rr_backup = L.RR
    try:
        L.RR = rr
