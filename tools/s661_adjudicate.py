# -*- coding: utf-8 -*-
"""
s661_adjudicate.py — داور نهایی S661 (استخر منجمد {H6,H8} — یک لمس نیمهٔ دوم)
================================================================================
پیش‌ثبت حاکم: results/S661_PREREG2_FROZEN_POOL_H6H8.md (کامیت e0cd64e4)

گاردها:
  BUG-GEOMDRIFT هندسه فقط از JSONهای اسکن خوانده می‌شود.
  BUG-WRUNITS   نول کانونی «درصد» می‌خواهد (درس مستند S660).
  BUG-PERMK     perm_k = تعداد جای‌گشت‌ها.
  BUG-NULLUNCOND بی‌قید per-side با allow_overlap=True و هندسهٔ خود عضو.
  BUG-PIPGUESS  pip از se.ASSETS.
  BUG-SCOREKEY  کلید rqs2_score.
  BUG-AXIS/SPAN محور مشترک H1-گام روی کل افق استخر؛ close هم‌راستا از H1.
  قید ۲         n<30 ⇒ بدون حکم.
"""
...
from tools.s661_explore import (                          # noqa: E402
    fisher_fast, WARMUP, MAX_HOLD)

SCAN = os.path.join(ROOT, 'results', '_scan_S661')
OUTD = os.path.join(ROOT, 'results', '_s661_final')
os.makedirs(OUTD, exist_ok=True)

ASSET = 'XAUUSD'
MEMBERS = ('H6', 'H8')      # استخر منجمد PREREG-2
P = 9                       # fisher period (منجمد)
THR = 1.5                   # آستانهٔ اشباع (منجمد)
SEED = 20260816
N_PERM = 600
N_TRIALS_FAMILY = 439       # بدهی بلوک S66x: 408+30+1
N_TRIALS_STRICT = 675       # اتحاد فرضی: 644+31


def build_member(tf: str):
    """معاملات عضو روی کل ۱۵.۶ سال + نول کانونی per-side (سه تلهٔ s434)."""
    # هندسه فقط از JSON اسکن (BUG-GEOMDRIFT)
    with open(os.path.join(SCAN, f'explore_{tf}.json')) as f:
        sc = json.load(f)
    sl_pip = float(sc['sl_pip'])
    tp_pip = float(sc['tp_pip'])
    cell = [c for c in sc['cells'] if c['p'] == P and c['thr'] == THR][0]
    ...
    # ── نول کانونی per-side (سه تله + BUG-WRUNITS: درصد) ────────────────
    ...
    for side_name, side_mask in (('long', le), ('short', sh)):
        k_side = int(side_mask.sum())          # تلهٔ ۱
        if k_side == 0:
    ...
    # lift کل‌داده برای شرط هم‌جهتی pool_cards (نه گزینش — فقط قاعدهٔ ماژول)
...
def main():
    t0 = time.time()
    print(f'== S661-POOL {MEMBERS} p={P} thr={THR} seed={SEED} ==', flush=True)
    members = [build_member(tf) for tf in MEMBERS]

    # assert مرز مسیر C: فاصلهٔ نیمه‌های H6 و H8 < ۱۰ روز
    ...
    # محور مشترک H1-گام (ضد BUG-AXIS/BUG-SPAN)
    ...
    # ماسک هولدآوت = مرز مسیر C (نیمهٔ تقویمی — PREREG-2 §۳.۵)
    te = pool['t_entry'].values.astype(np.int64)
    holdout = te >= split_ns
    n_expl = int((~holdout).sum())
    n_oos = int(holdout.sum())
    print(f'[مسیر C] مرز={np.datetime64(split_ns, "ns")} · '
          f'اکتشاف={n_expl} · هولدآوت={n_oos}', flush=True)
    ...
    results = {}
    for tag, ntr in (('family_439', N_TRIALS_FAMILY),
                     ('strict_675', N_TRIALS_STRICT)):
        ...
        print('score   :', r['rqs2_score'], flush=True)       # BUG-SCOREKEY
    ...
    out = dict(layer='S661-POOL', asset=ASSET, members=list(MEMBERS),
               p=P, thr=THR, seed=SEED, n_perm=N_PERM,
               used=[u['card'] for u in res['used']], dropped=res['dropped'],
               n_before=res['n_before'], n_after=res['n_after'],
               member_stats=[dict(card=m['card'], n=m['n'],
                                  lift_full=m['lift'], lift_fh=m['lift_fh'],
                                  sl_pip=m['sl_pip'], src=m['src'])
                             for m in members],
               sl_pip_w=sl_w, tp_pip_w=tp_w,
               pool_n=len(pool), pool_wr=wr_pool,
               split_ns=split_ns, n_explore=n_expl, n_holdout=n_oos,
               null=_clean(null), results=_clean(results),
               elapsed_s=round(time.time() - t0, 1))
    fp = os.path.join(OUTD, 'verdict_POOL_H6H8.json')
    with open(fp, 'w') as f:
