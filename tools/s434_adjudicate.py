...
def run_candidate(asset: str, tf: str) -> dict:
    ...
    return {
        ...
        # 🔴 BUG-NULLTRAIL: این دو کلید **افزوده شدند** چون مدلِ صفر
        # (`tools/s434_null_model.py`) با `run.get('trail')` سراغشان می‌آمد و
        # `None` می‌گرفت. پیامد: جای‌گشت‌ها **بدونِ تریلینگ** اجرا می‌شدند در
        # حالی که سیگنال با تریلینگ اجرا می‌شد ⇒ z تفاوتِ *مدیریتِ معامله* را
        # می‌سنجید نه تفاوتِ *انتخابِ زمان*، و چون تریلینگ WR را بالا می‌برد،
        # خطِ مبنا مصنوعاً **پایین** و z مصنوعاً **بزرگ** می‌شد: یک پاسِ ارزان
        # که هیچ خطایی تولید نمی‌کرد. دقیقاً همان «تلهٔ ۲»ی که خودم در
        # docstringِ مدلِ صفر مستند کردم و ابزارم در آن افتاد.
        'trail': CAND['trail'],
        'be_trigger': CAND['be_trigger'],
    }
