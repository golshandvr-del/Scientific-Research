# -*- coding: utf-8 -*-
"""S1524 — کفِ تازهٔ تقویم‌همتا (افق ۵۴۰ ساعت = ۹۰ کندل H6؛ LB per TF) ⇒ LONG | H4/H8/H12/D1.
پیش‌ثبت: results/S1524_PREREG_CalendarMatchedFreshFloor_Xauusd_H4H8H12D1.md (قبل از این کد).
رویداد S1511 نیوتن: low > max(low[t-LB..t-1])، لبهٔ تازه.
هارنس عیناً S526؛ تنها تغییر: LOOKBACK بر اساس کارت. اصل S526:
...
OUT = 'results/_s1524'
CARDS = ['XAUUSD_H4', 'XAUUSD_H8', 'XAUUSD_H12', 'XAUUSD_D1']
LB_BY_CARD = {'XAUUSD_H4': 135, 'XAUUSD_H8': 68, 'XAUUSD_H12': 45, 'XAUUSD_D1': 23, 'XAUUSD_H6': 90}  # 540h
N_TRIALS = 34
LOOKBACK = 90   # مقدار پیش‌فرض؛ در main به LB_BY_CARD[card] تنظیم می‌شود
...
def main():
    ...
    def load_full(card):
        ...
        assert os.path.exists(path), f'{path} missing — gunzip from data/mt5_full'
        df = pd.read_csv(path)
    ...
    def signals_fresh_high(df):
        lo = df['low']
        prior_max = lo.rolling(LOOKBACK).max().shift(1)
        ff = (lo > prior_max).fillna(False)
        return ff & ~ff.shift(1).fillna(False)
    ...
    print(f'S1524 calendar-matched fresh FLOOR | LB_BY_CARD={LB_BY_CARD} | '
          f'fresh edge only | sl_k={L.SL_K} rr={L.RR} side=long | '
          f'conditioned null (drift>0 space) k={NM.K} n_trials={N_TRIALS}',
          flush=True)
    global LOOKBACK
    for card in cards:
        LOOKBACK = LB_BY_CARD[card]
        print(f'  {card}: LOOKBACK={LOOKBACK} (540h horizon)', flush=True)
        try:
