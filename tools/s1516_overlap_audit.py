# -*- coding: utf-8 -*-
"""S1516 — ممیزی هم‌پوشانی فوری (قانون overlap): S1511-H6 در برابر ACCEPTهای زنده/هم‌خانواده.
...
OUT = 'results/_scan_S1516'; COST = 3.3; LB = 90
...
def fresh_floor(df, L=LB):
    lo = df['low']; ff = _b(lo > lo.rolling(L).max().shift(1)); return ff & ~ff.shift(1, fill_value=False) & drift(df, L)


def fresh_high_L(df, L):
    c = df['close']; nh = _b(c > c.rolling(L).max().shift(1)); return nh & ~nh.shift(1, fill_value=False) & drift(df, L)
...
def main():
    ...
    CARD = sys.argv[1] if len(sys.argv) > 1 else 'H6'; LC = {'H6': 120, 'H4': 180}[CARD]
    h6 = load(f'data/mt5_full/XAUUSD_{CARD}.csv'); h8 = load('data/mt5_full/XAUUSD_H8.csv')
    tr, be = trades_t(L, h6, fresh_floor(h6, LC))
    print(f'S1516-{CARD}(L={LC}): n={len(tr)} be={be:.2f}', flush=True)
    res = dict(card=CARD, L=LC, src='data/mt5_full', be=round(be, 2), all=seg(tr, be))
    h6r = load('data/mt5_full/XAUUSD_H6.csv'); tr1511, _ = trades_t(L, h6r, fresh_floor(h6r, 90)); res['vs_S1511_H6'] = vs_window(tr, be, tr1511)
    h4r = load('data/mt5_full/XAUUSD_H4.csv'); tr1523, _ = trades_t(L, h4r, fresh_high_L(h4r, 180)); res['vs_S1523_H4_180'] = vs_window(tr, be, tr1523)
    ...
    e_idx = np.flatnonzero(fresh_high_L(h6, LC).values)
    rec = np.array([bool(np.any((e_idx <= e) & (e_idx > e - 3))) for e in eb])
    res['filter_S526calL_recent3_same_card'] = dict(with_s526=seg(tr[rec], be), without_s526=seg(tr[~rec], be))
    mid = len(tr) // 2
    ...
    with open(f'{OUT}/overlap_audit_{CARD}.json', 'w') as f: json.dump(res, f, ensure_ascii=False, indent=1)
