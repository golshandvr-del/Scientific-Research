...
def main():
    ...
    for r in rows:
        ...
        #    و **مقدمِ بر همه**: نقطهٔ سربه‌سرِ هندسهٔ براکت. با TP<SL یک ورودِ
        #    بی‌مهارت هم WR بالا می‌دهد؛ مرجعِ ~۵۲٪ لیفت را دقیقاً روی همان
        #    هندسه‌ها متورم می‌کند که اشتباهِ #۸ در آن‌ها زندگی می‌کند (S375).
        own = r.get('own_ref')
        be = r.get('be')
        if be is not None:
            base, ref_src = be, f"be:{r.get('be_src')}"
