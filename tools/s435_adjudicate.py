...
def null_for(df, mask, sl, tp, mh, asset='XAUUSD', n_perm=N_PERM, seed=SEED):
    ...
    return {'long': dict(uncond_wr=wr_unc,
                         ...
                         # 🔴 گامِ ۸۷ — باگِ `BUG-PERMK`: قبلاً اینجا `k` بود،
                         # یعنی **اندازهٔ نمونهٔ** هر جای‌گشت (تعدادِ سیگنالِ
                         # بازو). ولی موتور `perm_k` را **تعدادِ جای‌گشت‌ها**
                         # می‌خواند و با `PERM_K_MIN=500` مقایسه‌اش می‌کند
                         # (engine/rqs2.py:1040). پس مقدارِ درست `pa.size`
                         # است: شمارِ جای‌گشت‌هایی که واقعاً WR برگرداندند.
                         # ⚠️ چرا خطرناک بود: در بازوی `full` مقدارِ اشتباه
                         # ۶۳۲ شد که تصادفاً از ۵۰۰ بزرگ‌تر است ⇒ موتور
                         # `H3` را «معلوم» اعلام کرد **در حالی که فقط ۴۰
                         # جای‌گشت پشتش بود**. یعنی باگ در یک بازو خودش را
                         # پنهان کرد و در بازوی دیگر لو رفت — همان الگویِ
                         # «موفقیتِ خاموش» برای یازدهمین بار در این مأموریت.
                         perm_k=int(pa.size)),
            'short': {}}
