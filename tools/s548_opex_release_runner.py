# -*- coding: utf-8 -*-
"""S548 — رانرِ Gold OPEX Release (رهایی پس از سررسید آپشن GC) روی خانوادهٔ ۱۱-کارتیِ XAUUSD (دادهٔ کامل).

پیش‌ثبت: `results/S548_PREREG_GOLD_OPEX_RELEASE_XAUUSD_MTF.md`
...
وصله‌های مجاز (قفل در پیش‌ثبت):
  ۱) L.signals → آخرین کندلِ روزِ معاملاتیِ قبل از روزِ رهایی R (= روز کاری بعد از سررسید X) (LONG)
  ۲) L.load   → data/full/{card}.csv (gunzip از data/mt5_full؛ H4 تجمیع‌شده از H1 همان منبع)
  ۳) MTF.SEED=20260826، MTF.N_TRIALS=36
"""
...
def market_holidays(y0=2010, y1=2027):
    """تعطیلات NYSE-style (عیناً S547)."""
...
def release_days(y0=2010, y1=2027):
    """R = اولین روز کاری پس از سررسید X آپشن GC.
    X (قاعدهٔ CME): چهارمین روز کاری پیش از آخرین روز کاری ماه؛ اگر جمعه یا روز قبل از تعطیلی بود، یک روز کاری عقب."""
    hol = set(market_holidays(y0, y1))
    def is_bd(d):
    ...
    for y in range(y0, y1 + 1):
        for m in range(1, 13):
            ...
            d = last if is_bd(last) else prev_bd(last)          # آخرین روز کاری ماه
            for _ in range(4):                                    # چهارمین روز کاری پیش از آن
                d = prev_bd(d)
            if d.weekday() == 4 or not is_bd(d + pd.Timedelta(days=1)):
                d = prev_bd(d)
            out.append(next_bd(d))                                # R
...
def make_signals(df):
    """سیگنال روی آخرین کندلِ آخرین روزِ معاملاتیِ پیش از R (ورود در close آن = open روز R)."""
...
def main():
    ...
    print(f'S548 Gold OPEX Release | {len(_PRE)} release days 2010-2027 | side=long | '
