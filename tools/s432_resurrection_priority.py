...
def main():
    ...
    # =======================================================================
    def rank_key(r):
        m, z, c = r['h9_margin'], r.get('z'), r.get('cost')
        if m is None or m <= 0:
            return 0.0
        # سدِ H9 = ۲×هزینه؛ اگر هزینه غایب بود، نرمال‌سازی ممکن نیست ⇒ خام
        bar = (2.0 * c) if (c is not None and c > 0) else None
        m_norm = (m / bar) if bar else m
        z_eff = max(0.0, z if z is not None else 0.0)
        return sqrt(max(0.0, m_norm) * z_eff)
