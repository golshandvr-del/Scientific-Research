...
def session_open_signal(d: dict, hours=(22, 23), mode: str = 'SESSION_OPEN'):
    ...
    prev[0] = -1
    prev[1:] = h[:-1]
    # ⚠️ BUG-HOUREDGE (S434) — نسخهٔ اول لبه را نسبت به **مجموعهٔ** ساعت‌ها
    #   تعریف می‌کرد: `in_h & ~isin(prev, hours)`. روی H1 که ساعتِ ۲۲ و ۲۳ دو
    #   کندلِ **متوالی**اند، کندلِ ۲۳ رد می‌شد چون قبلی‌اش ۲۲ بود ⇒ دو فرصتِ
    #   ورودِ مستقل به **یکی** تبدیل می‌شد. census این را برهنه کرد:
    #   H1 راو=۶٬۸۵۷ در برابر سشن=۳٬۸۶۳ ⇒ ستونِ «یکسان» ❌ شد، درست خلافِ
    #   خاصیتی که در گامِ ۱۹ به کاربر **ادعا** کرده بودم.
    #   خطرِ واقعی: نصف کردنِ n روی هر پنج کارت ⇒ آسیب به دروازهٔ H3 (توان)،
    #   یعنی همان چیزی که این مأموریت می‌کوشد از آن پرهیز کند.
    #   تعریفِ درست: اولین کندلِ **هر ساعتِ هدف** (نه اولین کندلِ سشن) ⇒
    #   شرط `h != prev` به‌جای `~isin(prev, hours)`. روی H1 هر کندل ساعتی
    #   متفاوت از قبلی دارد، پس این تعریف با RAW_HOUR **دقیقاً** یکی می‌شود.
    return in_h & (h != prev)
