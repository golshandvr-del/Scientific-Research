...
def as_dataframe(d: dict) -> pd.DataFrame:
    ...
    """
    # ⚡ `copy=False` (گامِ ۴۸) — کشفِ سنجیده‌شده، نه بهینه‌سازیِ حدسی:
    #   بدونِ آن، pandas هر شش ستون را **کپی** می‌کند، پس روی M1 با پنج
    #   میلیون کندل، ۲۲۸MB دومِ اضافه مصرف می‌شود (۳۲۶MB → ۵۵۴MB) و سندباکسِ
    #   ۹۸۵ مگابایتی در شبیه‌سازی می‌مُرد (M1 دو بار Killed شد).
    #   با آن، `np.shares_memory(df['close'].values, d['close']) is True`
    #   یعنی DataFrame **ارجاع** می‌دهد و مصرف روی ۳۲۶MB می‌مانَد.
    # امنیت: `simulate_trades` فقط می‌خوانَد و ستون‌ها را تغییر نمی‌دهد، پس
    #   اشتراکِ حافظه بی‌خطر است. (اگر روزی نویسنده شد، این must be revisited.)
    return pd.DataFrame({
