# -*- coding: utf-8 -*-
"""S1512 — ممیزی هم‌پوشانی فوری (قانون overlap): S1512-H6 (whole-bar) در برابر ACCEPTهای زنده/هم‌خانواده.
...
OUT = 'results/_scan_S1512'; COST = 3.3; LB = 90
...
def whole_bar(df):
    ...
    wb = _b(lo > lo.rolling(LB).max().shift(1)) & _b(c > c.rolling(LB).max().shift(1))
    return wb & ~wb.shift(1, fill_value=False) & drift(df)
...
def main():
    ...
    tr, be = trades_t(L, h6, whole_bar(h6))
    print(f'S1512-H6: n={len(tr)} (انتظار 237) be={be:.2f}', flush=True)
