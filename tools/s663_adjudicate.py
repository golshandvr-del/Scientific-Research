# -*- coding: utf-8 -*-
"""
s663_adjudicate.py — داور نهایی S663 (استخر منجمد {H3,H6,H8,H12} — یک لمس نیمهٔ دوم)
================================================================================
پیش‌ثبت حاکم: results/S663_PREREG2_FROZEN_POOL_H3H6H8H12.md (کامیت 3d03fc58)
معماری عیناً S661/S662 — فقط سیگنال عوض شده: شوکِ σ_slot W=34 θ=2.0 (follow).
...
from tools.s663_explore import (                          # noqa: E402
    log_ret, sigma_slot, shock_signals, WARMUP, MAX_HOLD)

SCAN = os.path.join(ROOT, 'results', '_scan_S663')
OUTD = os.path.join(ROOT, 'results', '_s663_final')
...
def build_member(tf: str):
    ...
    cell = [c for c in sc['cells'] if c['W'] == W and c['theta'] == THETA
            and c['sigma'] == 'slot'][0]
    ...
    hour = np.ascontiguousarray(d['hour'], dtype=np.int16)
    del d
    ...
    r = log_ret(c)
    sig = sigma_slot(r, hour, W)          # فقط گذشته‌ی همان اسلات
    le, sh, _ = shock_signals(r, sig, THETA, valid)
...
def main():
    t0 = time.time()
    print(f'== S663-POOL {MEMBERS} W={W} theta={THETA} seed={SEED} ==', flush=True)
    members = [build_member(tf) for tf in MEMBERS]

    th = [m['t_half'] for m in members]
    assert max(th) - min(th) < 10 * 86400, f'مرزهای نیمه ناسازگار: {th}'
    ...
    results = {}
    for tag, ntr in (('family_498', N_TRIALS_FAMILY),
                     ('strict_734', N_TRIALS_STRICT)):
    ...
    out = dict(layer='S663-POOL', asset=ASSET, members=list(MEMBERS),
               W=W, theta=THETA, seed=SEED, n_perm=N_PERM,
    ...
    fp = os.path.join(OUTD, 'verdict_POOL_H3H6H8H12.json')
    with open(fp, 'w') as f:
