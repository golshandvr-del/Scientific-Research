# -*- coding: utf-8 -*-
"""S1521 — سقفِ تازهٔ مطلع (پایهٔ منجمد S1520) × گیت رژیمِ آرامِ σ (منجمد S605/S606/S1911، W=233).

پیش‌ثبت: results/S1521_PREREG_InformedFreshHighCalm_Xauusd_H8H12H6.md (کامیت 9831f79c، قبل از این کد).
...
OUT = 'results/_s1521'
CARDS = ['XAUUSD_H8', 'XAUUSD_H12', 'XAUUSD_H6']
N_TRIALS = 23
W_CALM = 233      # منجمد S606/S1911
...
def calm_mask(df):
    ...
    ok = np.isfinite(reg)
    return pd.Series(ok & (reg <= 1.0), index=df.index), pd.Series(ok & (reg > 1.0), index=df.index)


def main():
    mode = sys.argv[1] if len(sys.argv) > 1 and sys.argv[1] in ('calm', 'storm') else 'calm'
    ...
    def signals(df):
        base = fresh_high(df) & (rho(df) >= RHO_THR)   # S1520 منجمد
        calm, storm = calm_mask(df)
        sig = base & (calm if mode == 'calm' else storm)
    ...
    print(f'S1521 informed fresh-high x calm-sigma(W={W_CALM}) [{mode}] | sl_k={L.SL_K} rr={L.RR} '
