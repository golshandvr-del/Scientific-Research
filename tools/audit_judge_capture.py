...
def _identify_by_asset(call: dict):
    ...
    # ⚠️ باگِ نوزدهم (اندازه‌گیری‌شده): وقتی چند تایم‌فریم جا می‌شوند، قیدِ
    #   «بلندترین ایندکسِ سیگنال» به‌تنهایی مبهم است و لایه `INCOMPLETE`
    #   می‌گیرد. `S382` — تنها لایهٔ `ACCEPT`ِ **مستقر در سایت** و همان لایهٔ
    #   کالیبراسیونِ هارنس — دقیقاً این‌طور دفن شد: بلندترین ایندکس ۲۳٬۶۹۸
    #   بود و پنج تایم‌فریمِ طلا (`H1,H4,M15,M30,M5`) جا می‌شدند.
    #
    #   ولی خودِ ضبط یک قیدِ **قطعی** دیگر دارد: اسکریپت‌ها `n_trials` را
    #   برابرِ **طولِ دقیقِ فایلِ داده** اعلام می‌کنند، چون شمارِ جایگشت‌ها
    #   روی همان تعداد کندل ساخته می‌شود. اندازه‌گیریِ `S382`:
    #       n_trials_declared = 23755
    #       XAUUSD_H4  len    = 23755   ⬅️ تطبیقِ دقیق و یکتا
    #   و سازگار است: بلندترین سیگنال (۲۳٬۶۹۸) فقط ۵۷ کندل کمتر از پایانِ
    #   H4 است، یعنی سیگنال‌ها تا انتهای همان فایل پخش شده‌اند.
    #
    #   این تطبیقِ تساویِ دقیق است، نه نزدیک‌ترین‌همسایه؛ و اگر بیش از یک
    #   تایم‌فریم همان طول را داشت (یا هیچ‌کدام) باز هم تصمیم گرفته
    #   نمی‌شود، پس هیچ حدسی وارد داوری نمی‌شود.
    ntd = call.get('n_trials_declared')
    if isinstance(ntd, (int, float)) and ntd > 0:
        exact = [(tf, n) for tf, n in fits if n == int(ntd)]
        if len(exact) == 1:
            return (pair, exact[0][0], 0)
