...
# هندسه — نسبتِ 1:2 حفظ، اندازهٔ مطلق **مقیاس** می‌شود (اشتباهِ رایجِ ۶)
GEOM = {'EURUSD': dict(sl=15.0, tp=30.0, mh=96),
        'XAUUSD': dict(sl=150.0, tp=300.0, mh=96)}
...
def derive_mfe_target(df, mask, mh, asset) -> dict:
    """هدفِ `V3` را از **خودِ داده** می‌گیرد: چندکِ ۷۵٪ِ MFE.
