# -*- coding: utf-8 -*-
"""
RQS2 — معیارِ رسمیِ نسلِ دومِ پذیرشِ لایه‌ها  ·  **نسخهٔ ۲.۴**
================================================================================
سندِ مرجع: `docs/RQS2_SPEC.md` · حسابرسی: `docs/RQS2_AUDIT_FULL_REPORT.md`

نسخهٔ ۲.۴ — رفعِ سه نقصِ آماریِ افشاشده در حسابرسی (اثبات: v24_regression.json)
------------------------------------------------------------------------------
  • F1+F2 (H3): شرطِ ناهمگرای `wr > perm_max` — که حکمش با تعدادِ قرعه K و بذر
    نوسان می‌کرد (۲۲ پاس/۸ رد از ۳۰ اجرا روی یک لایهٔ یکسان) — حذف و با یک
    آمارهٔ **همگرا** جایگزین شد: `p_perm ≤ 0.001` (≡ z≥3.09σ، کمی سخت‌تر از
    z≥۳). دو خط‌کشِ ناسازگارِ پیشین حالا یک خط‌کشِ واحدند.
  • کفِ K از ۱۰ به ۵۰۰: با K<500، `perm_sd` هنوز همگرا نشده ⇒ H3 صراحتاً
    UNKNOWN («K را بالا ببر») نه یک حکمِ قطعیِ ناپایدار.
  • F4 (H10): `n_counter<20` دیگر خودکار UNKNOWN/INCOMPLETE نمی‌دهد؛ یک آزمونِ
    جانشینِ متناسب + WARN جایگزین شد تا لایهٔ **گزینشی** صرفاً به‌خاطرِ
    گزینشی‌بودن از پذیرش محروم نشود.
  اثباتِ ضدِتقلب: آزمونِ بازگشتیِ R1–R4 (`tools/rqs2_v24_regression.py`) —
  هیچ لایهٔ بی‌مهارتی زنده نشد؛ سدِ آماری در واحدِ σ **بالاتر** رفت نه پایین‌تر.
  ⚠️ H5 عمداً دست‌نخورده ماند — حسابرسی ثابت کرد سالم (و حتی سهل‌گیر) است.
...
#   می‌شد که خاصیتِ استراتژی نیست (اثبات: docs/RQS2_AUDIT_FULL_REPORT.md §۲.۳).
#   روی یک لایهٔ *یکسان*، تغییرِ فقط `seed` ⇒ ۲۲ پاس/۸ رد از ۳۰ اجرا.
#   رفع: شرطِ ناهمگرا حذف و با یک آمارهٔ **همگرا** جایگزین شد — p-valueِ یک‌طرفه
#   از همان `z_skill` (که خودش از میانگین/انحرافِ **همگرا**ی جای‌گشت ساخته می‌شود).
#   `p ≤ 0.001 ≡ z ≥ 3.09σ` ⇒ آزمون **کمی سخت‌تر** از `z≥۳` شد، نه نرم‌تر.
#   دو خط‌کشِ ناسازگارِ پیشین (`z≥۳` و `wr>perm_max` که سدِ واقعی‌اش ۳.۹σ بود)
#   حالا **یک** خط‌کشِ واحدند ⇒ ناسازگاریِ اعلام/اجرا رفع شد.
SKILL_P_MAX      = 0.001    # سقفِ p-valueِ همگرا برای H3 (≡ z≥3.09σ)
...
# ============================================================================
#  v2.5 — حکمِ چهارم: POWER-LIMITED (توان‌محدود)   ← تصمیمِ حاکمیتیِ کاربر
# ----------------------------------------------------------------------------
# مسئله‌ای که کاربر درست تشخیص داد: RQS2 دو چیزِ کاملاً متفاوت را با یک برچسبِ
# واحد (REJECT/INCOMPLETE) قاطی می‌کرد:
#     (الف) «لبه وجود ندارد»            ← ردِ واقعی، درست است.
#     (ب) «لبه هست، ولی نمونه برای اثباتِ آماری‌اش کم است» ← ردِ ناعادلانه.
# نمونهٔ زندهٔ (ب): S351 روی XAUUSD-D1 با lift=+۱۴pp عالی بود ولی چون فقط ۷۴
# معامله داشت z=2.6<3.0 ماند؛ و S346 که H8 مرزی داشت. این‌ها لبهٔ اقتصادیِ
# سالم دارند و فقط توانِ آماری کم دارند.
#
# راهِ حل — **بدونِ شل کردنِ هیچ آستانه‌ای** (سپرِ ضدِ تقلبِ #۸ دست‌نخورده):
# گیت‌ها به دو خانواده تقسیم می‌شوند:
#   • گیت‌های اقتصادی  = آیا لبه در حسابِ واقعی پول می‌سازد؟  {H0,H1,H2,H8,H9}
#   • گیت‌های توان‌محور = آیا لبه به‌قدرِ کافی اثبات شده؟      {H3,H7,H10}
# یک لایه POWER-LIMITED است اگر و فقط اگر:
#   ۱) ACCEPT نباشد،
#   ۲) هیچ گیتِ اقتصادی‌ای شکست نخورده باشد (همه True؛ نه False، نه None)،
#   ۳) تنها نقص‌ها در گیت‌های توان‌محور باشد،
#   ۴) جهتِ لبه مثبت و رو به رشد باشد: lift>0 و z_skill>0 (نه فقط کم‌نمونه؛
#      اگر lift منفی بود یعنی لایه غلط است، نه کم‌توان ⇒ REJECT می‌ماند).
# POWER-LIMITED هرگز ACCEPT نیست و به سایت نمی‌رود (پولِ کاربر ریسک نمی‌شود)،
# ولی BURNED هم نیست: به «اتاقِ انتظار» می‌رود تا با دادهٔ بیشتر یا تجمیعِ
# چند-کارتی نمونه‌اش بزرگ شود.
