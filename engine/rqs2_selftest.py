...
def main():
    ...
    NT = 1000                      # n_trials فرضی برای H5

    # ---- T1: فقط رانش — WR=63٪ ولی مبنای بی‌سیگنال هم ۶۰٪ است ----
    t = make_trades(126, 74, SL, TP)                       # WR = 63.0٪
    ok, _ = _run("T1 drift-only (must fail H3)", t,
                 expect_gate={'H3': False}, expect_state='REJECT',
                 tp_pip=TP, null=mk_null(60.0), n_trials=NT, split_bar=1000)
    allok &= ok

    # ---- T2: مهارتِ واقعی — همان WR ولی مبنا ۵۲٪ ----
    ok, _ = _run("T2 real skill (must pass H3)", t,
                 expect_gate={'H3': True},
                 tp_pip=TP, null=mk_null(52.0), n_trials=NT, split_bar=1000)
    allok &= ok

    # ---- T3: نبودِ tp_pip ⇒ H2 نامعلوم، حکم INCOMPLETE ----
    ...
    t4 = make_trades(156, 44, SL, 30.0)                    # WR = 78.0٪
    ok, _ = _run("T4 TP<SL gaming (must fail H2)", t4,
                 expect_gate={'H2': False},
                 tp_pip=30.0, sl_pip=SL, null=mk_null(52.0), n_trials=NT,
                 split_bar=1000)
    allok &= ok
    ...
    ok, _ = _run("T5 no null model (H3/H4/H5=?)", t,
                 ...
                 tp_pip=TP, n_trials=NT, split_bar=1000)
    allok &= ok

    # ---- T6: سمتِ بی‌مهارت — لانگِ ماهر + شورتِ سوارِ رانش ----
    tl = make_trades(70, 30, SL, TP, side='long', start_bar=0, step=20)
    ts = make_trades(56, 44, SL, TP, side='short', start_bar=10, step=20)
    t6 = pd.concat([tl, ts], ignore_index=True).sort_values('exit_bar')
    ok, _ = _run("T6 one skilless side (fail H4)", t6,
                 expect_gate={'H4': False},
                 tp_pip=TP, null=mk_null(52.0, 55.0), n_trials=NT,
                 split_bar=1000)
    allok &= ok

    # ---- T7: خوشه‌ای‌شدنِ تقویمی — همهٔ معاملات در یک بازهٔ زمانی ----
    #   محورِ زمان را طوری می‌سازیم که کلِ معاملات در ربعِ اولِ تقویم بیفتند.
    t7 = make_trades(126, 74, SL, TP, step=10, hold=5)
    bar_time = np.arange(200000, dtype='float64')          # افقِ تقویمیِ بلند
    ok, _ = _run("T7 calendar clustering (fail H6)", t7,
                 expect_gate={'H6': False},
                 tp_pip=TP, bar_time=bar_time, null=mk_null(52.0),
                 n_trials=NT, split_bar=1000)
    allok &= ok

    # ---- T8: معاملاتِ هم‌پوشان ⇒ استقلال نقض ⇒ H0 رد ----
    t8 = make_trades(126, 74, SL, TP, step=2, hold=9)       # hold > step
