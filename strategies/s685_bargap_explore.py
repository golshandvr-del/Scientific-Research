# -*- coding: utf-8 -*-
"""
s685_bargap_explore.py — اکتشافِ S685 فقط روی **نیمهٔ اول** (مسیرِ C)
================================================================================
پیش‌ثبت: results/S685_PREREG_INTRAWEEK_BAR_GAP_DRIFT.md (کامیت پیش از اجرا — قبل از اجرا).

سیگنال: گپِ بین‌کندلیِ منظم |open_t − close_{t−1}| ≥ g×ATR21_{t−1} (Δt == TF دقیقاً؛
بازگشاییِ روز/ویکند حذف) ∧ بدنهٔ پُرِ هم‌جهت (|ρ|≥0.5) ∧ هم‌جهتی با رانشِ ۲۴۰ کندل
⇒ ورود در جهتِ گپ. گریدِ قفل: g∈{0.382,0.618} × rr∈{1,1.5,2} = ۶ سلول.
غربال نسبت به نالِ غیرشرطیِ ارزانِ نیمهٔ اول (درسِ S683).
"""
...
OUT_DIR = os.path.join(ROOT, 'results', '_s685_explore')
...
TF_SEC = {'M1': 60, 'M3': 180, 'M4': 240, 'M5': 300, 'M6': 360, 'M10': 600,
          ...
          'D1': 86400, 'W1': 604800, 'MN1': None}


def bargap_signals(t, o, h, l, c, tf: str, g: float, warm: int = WARM):
    """برمی‌گرداند (long_sig, short_sig, stats) — همه علّی."""
    n = len(c)
    a21 = atr_wilder(h, l, c, 21)
    a_prev = np.concatenate(([np.nan], a21[:-1]))
    gap = np.concatenate(([0.0], o[1:] - c[:-1]))
    dt = np.concatenate(([0], np.diff(t)))
    sec = TF_SEC.get(tf)
    if sec is None:                       # MN1: ماهِ متوالی = 28..31 روز
        regular = (dt >= 27 * 86400) & (dt <= 32 * 86400)
    else:
        regular = dt == sec
    with np.errstate(divide='ignore', invalid='ignore'):
        rho = np.where(h - l > 0, (c - o) / (h - l), 0.0)
        shock = regular & (np.abs(gap) >= g * a_prev)
    body_ok = ((gap > 0) & (rho >= RHO)) | ((gap < 0) & (rho <= -RHO))
    ev = shock & body_ok
    drift = np.zeros(n)
    drift[W_DRIFT + 1:] = c[W_DRIFT:-1] - c[:-W_DRIFT - 1]
    aligned = ev & (np.sign(gap) == np.sign(drift)) & (drift != 0)
    long_sig = aligned & (gap > 0)
    short_sig = aligned & (gap < 0)
    long_sig[:warm] = False
    short_sig[:warm] = False
    n_shock = int(shock[warm:].sum())
    n_ev = int(ev[warm:].sum())
    n_al = int(aligned[warm:].sum())
    stats = dict(n_gap_shock=n_shock, n_event_body=n_ev, n_aligned=n_al,
                 body_pass=round(n_ev / n_shock, 3) if n_shock else None,
                 drift_pass=round(n_al / n_ev, 3) if n_ev else None)
    return long_sig, short_sig, stats
...
def explore(tf: str, asset: str = 'XAUUSD') -> dict:
    ...
    gate_stats = {}
    for g in GS:
        long_sig, short_sig, st = bargap_signals(t_arr, o, h, l, c, tf, g)
        gate_stats[f'g{g}'] = st
        nsig = int(long_sig.sum() + short_sig.sum())
        for rr in RRS:
            tp = round(rr * sl, 1)
            if nsig == 0:
                cells.append(dict(g=g, rr=rr, n=0, skipped='no_sig'))
                continue
            tr = se.simulate_trades(df, long_sig, short_sig, sl, tp,
                                    asset, max_hold=mh, allow_overlap=False)
            if tr is None or len(tr) == 0:
                cells.append(dict(g=g, rr=rr, n=0, skipped='no_trades'))
                continue
            pnl = tr['pnl_pip'].values
            ...
            cells.append(dict(g=g, rr=rr, n=n, n_long=nl, n_sig=nsig,
    ...
    res = dict(asset=asset, tf=tf, src=src, n_full=n_full, n_half=n_half,
               ...
               cost_pip=cost, warm=WARM, rho=RHO, w_drift=W_DRIFT, uncond=unc,
               n_uncond=N_UNCOND, gate_stats=gate_stats,
               grid_cells=len(GS) * len(RRS),
