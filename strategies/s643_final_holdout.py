# -*- coding: utf-8 -*-
"""
S643 — EngulfEvent — داوریِ نهاییِ holdout (مسیرِ C — فقط یک بار اجرا)
=======================================================================
پیش‌ثبت: results/S643_PREREG_ENGULF_EVENT_XAUUSD_PATHC.md (commit 9e31ab27)
منطق عیناً جدولِ §۲ پیش‌ثبت — صفر درجهٔ آزادیِ باز:

  • کارت‌ها: {H3, H4, H6, H8, H12}
  • لانگ: cdl_engulf_bull (پوشای صعودی) → ورود در کندلِ بعد
  • شورت: cdl_engulf_bear (پوشای نزولی) → ورود در کندلِ بعد
...
def run_card(tf, rng):
    ...
    bear = np.asarray(ib.compute('cdl_engulf_bear', df)) != 0   # بانک −100 برمی‌گرداند
...
def main():
    ...
    print('== S643 — داوریِ نهاییِ holdout · Engulf cont · {H3,H4,H6,H8,H12} · '
    ...
    if res is None:
        print('\n[S643] هیچ عضوِ معتبری نماند — استخر تهی. حکم: مرگ در H0.',
              flush=True)
    ...
    print('\n' + R.format_rqs2('S643-ENGULF-POOL', r), flush=True)

    out = dict(prereg='S643_PREREG_ENGULF_EVENT_XAUUSD_PATHC.md@9e31ab27',
