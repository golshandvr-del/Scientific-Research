...
# -*- coding: utf-8 -*-
"""
S1400 — گپ‌فیل H1 با V ثابت روی دادهٔ کامل ۱۵.۶ ساله (data/mt5_full) · XAUUSD H1
==============================================================================
پیش‌ثبت: results/S1400_PREREG_GAP_FILL_H1_VFIXED.md (commit قبل از هر اجرا)
والد: S404 (ACCEPT 96.8, M30) · S408 (ACCEPT 93.8, M15) · S400-H1 (REJECT، X-BAR بدون V)
...
حالت‌ها:
  python3 strategies/s1400_gap_fill_h1_vfixed.py tune
  python3 strategies/s1400_gap_fill_h1_vfixed.py verdict <q> <k_sl>
"""
import sys, os, json
import numpy as np
import pandas as pd
...
TF = 'H1'
SEED = 1400
N_TRIALS_CUM = 298                 # 292 + 6 (پیش‌ثبت §۶)
DATA = 'data/mt5_full/XAUUSD_H1.csv'      # دادهٔ کامل (پیش‌ثبت §۱)
SPLIT_FULL = 45581                         # اولین کندل ≥ 2018-10-25 11:30 UTC (هم‌تاریخ M30)
EXPECT = dict(n=91331, first='2011-01-03 00:00:00', last='2026-08-07 23:00:00')
CKPT = os.path.join(os.path.dirname(__file__), '..', 'results', '_s1400_tune_ckpt.json')
...
def load_full():
    ...
    assert str(df['dt'].iloc[SPLIT_FULL]) == '2018-10-25 12:00:00', "split bar mismatch"
    return df


def run_layer(df, days, atr, q, k_sl, use_v, vflags=None,
              lo_bar=None, hi_bar=None, no_thresh=False, no_dow=False):
    """S1400 روی H1 کامل (V ثابت در تیون؛ در null حذف می‌شود). آستانه/DOW/V = مهارت (در null حذف می‌شوند)."""
...
def mode_verdict(q, k_sl, use_v):
    ...
    print(rqs2.format_rqs2(f'S1400 {TF} q={q}/k_sl={k_sl}/V=fixed', r), flush=True)
    outp = os.path.join(os.path.dirname(__file__), '..', 'results', '_s1400_verdict.json')
