#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""S881 — آزمونِ نهاییِ Path C روی **نیمهٔ دوم** (hold-out) — یک بار، فقط یک بار.

پیش‌ثبت: results/S881_PREREG_SkewFlip_PathC.md (commit 64038264)

ورودی: نامزدِ منتخبِ هر TF از results/_scan_S881/XAUUSD_<TF>.json ('best').
قواعدِ منجمد:
  - θ ثابتِ پیش‌ثبت‌شده است (0.618 یا 1.0) — چندکی نیست، انجمادی لازم ندارد.
  - هندسه: sl/tp پیپِ منجمد از نیمهٔ اول (ATR89 نیمهٔ اول) — بازمحاسبه نمی‌شود.
  ...
  - حکم را موتور صادر می‌کند، نه من.

usage: python3 strategies/s881_verdict.py <TF> [<TF> ...]
خروجی: results/_scan_S881/VERDICT_XAUUSD_<TF>.json
"""
...
from strategies.s881_skew_flip_scan import build_signals
from strategies.s880_entropy_collapse_scan import wr_of

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SCAN = os.path.join(ROOT, 'results', '_scan_S881')
N_TRIALS = 72
N_PERM = 500
...
def null_side(df, side, n_side, sl, tp, hold, rng, n_perm=N_PERM):
    ...
    # WR بی‌قید: کندل‌های معتبر با گامِ نمونه‌برداری (سقف 20k برای سرعت)
...
def judge_tf(tf):
    ...
    p, theta, k, hold = best['p'], best['theta'], best['k'], best['hold']
    sl, tp = best['sl_pip'], best['tp_pip']

    d = fd.load_fast('XAUUSD', tf)
    out['src'] = d['src']
    assert 'mt5_full' in d['src'], f'E-16 trap: wrong data source {d["src"]}'
    df_all = fd.as_dataframe(d)
    half = scan['half_idx']
    # پنجرهٔ گرم‌سازی: p+k+5 کندل قبل از مرز تا S روی اولین کندل‌های OOS تعریف باشد
    warm = p + k + 5
    ...
    ns = n - nl
    # ارزیابیِ پیش‌بینیِ ابطال‌پذیرِ پیش‌ثبت (long باید از short بهتر باشد)
    if nl > 0:
        tl = trades[trades['direction'] == 'long']
        out['oos_wr_long'] = round(100.0 * float((tl['pnl_pip'] > 0).mean()), 2)
    if ns > 0:
        ts = trades[trades['direction'] == 'short']
        out['oos_wr_short'] = round(100.0 * float((ts['pnl_pip'] > 0).mean()), 2)
