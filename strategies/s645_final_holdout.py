# -*- coding: utf-8 -*-
"""
S645 — PsarFlip — داوریِ نهاییِ holdout (مسیرِ C — فقط یک بار اجرا)
=======================================================================
پیش‌ثبت: results/S645_PREREG_PSAR_FLIP_XAUUSD_PATHC.md (commit dd1fb549)
منطق عیناً جدولِ §۲ پیش‌ثبت — صفر درجهٔ آزادیِ باز:

  • کارت‌ها: {H3, H4, H6, H8, H12}
  • لانگ: PSAR(0.02,0.2) از بالای close به زیرِ آن بچرخد (SAR[t-1]≥close[t-1] و SAR[t]<close[t])
  • شورت: آینه — چرخشِ SAR به بالای close
...
def run_card(tf, rng):
    ...
    sar = np.asarray(ib.compute('psar', df), dtype=float)
    cv = df['close'].values.astype(float)
    below = np.nan_to_num(sar < cv, nan=False).astype(bool)
    pbelow = np.roll(below, 1); pbelow[0] = below[0]
    valid = ~np.isnan(sar); pvalid = np.roll(valid, 1); pvalid[0] = False
    bull = below & ~pbelow & valid & pvalid      # SAR flips below close → long
    bear = ~below & pbelow & valid & pvalid      # SAR flips above close → short
...
def main():
    ...
    print('== S645 — داوریِ نهاییِ holdout · PSAR flip · {H3,H4,H6,H8,H12} · '
    ...
    if res is None:
        print('\n[S645] هیچ عضوِ معتبری نماند — استخر تهی. حکم: مرگ در H0.',
              flush=True)
    ...
    print('\n' + R.format_rqs2('S645-PSAR-POOL', r), flush=True)

    out = dict(prereg='S645_PREREG_PSAR_FLIP_XAUUSD_PATHC.md@dd1fb549',
