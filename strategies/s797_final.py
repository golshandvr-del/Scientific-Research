# -*- coding: utf-8 -*-
"""
S797 — داوری رسمی (یک بار) — Shock→Digestion→Continuation — استخر {H6,H8,H12,D1}
طبق strategies/S797_PREREG.md (کامیت 7d40ab75). هیچ پارامتری اینجا جست‌وجو نمی‌شود.
...
LAYER = 797
PIP = 0.10
THETA, PAUSE, KSL, RR = 2.618, 0.5, 1.618, 1.0
MH = {'H6': 20, 'H8': 15, 'H12': 10, 'D1': 5}
MEMBERS = ('H6', 'H8', 'H12', 'D1')
N_TRIALS = 196
K_PERM = 500
HOLDOUT_START = np.datetime64('2018-11-16T00:00:00').astype('datetime64[ns]').astype(np.int64)
...
def atr21(h, l, c, p=21):
    ...
    return np.r_[np.nan, a[:-1]]      # ATR[i-1]: علّی


def member_for(tf, rng):
    d = fd.load_fast('XAUUSD', tf); df = fd.as_dataframe(d)
    assert 'mt5_full' in d['src'], 'E-16 guard'
    o = df['open'].values; h = df['high'].values; l = df['low'].values; c = df['close'].values; n = len(c)
    atr = atr21(h, l, c); rng_ = h - l
    shock = (rng_ >= THETA * atr) & ~np.isnan(atr) & (c != o)
    dirn = np.sign(c - o)
    sh_j = np.r_[False, shock[:-1]]; d_j = np.r_[0.0, dirn[:-1]]
    hi_i = np.r_[np.nan, h[:-1]]; lo_i = np.r_[np.nan, l[:-1]]; rg_i = np.r_[np.nan, rng_[:-1]]
    pause = (rng_ <= PAUSE * rg_i) & (c <= hi_i) & (c >= lo_i)
    atr_j = np.r_[atr[1:], np.nan]          # ATR21 شامل کندل j (لحظهٔ سیگنال)
    valid = ~np.isnan(atr) & ~np.isnan(atr_j)
    sig = sh_j & pause & valid
    ls = sig & (d_j > 0); ss = sig & (d_j < 0)
    sl = np.where(valid, KSL * atr_j / PIP, 0.0); tp = sl * RR
...
def main():
    ...
    if res is None:
        print('[S797] no valid member ⇒ REJECT_BY_RULE'); return
    ...
    print(f'[split] holdout from 2018-11-16T00 | explore={int((~holdout).sum())} oos={int(holdout.sum())}', flush=True)
    ...
    print('\n' + rqs2.format_rqs2('S797-POOL', r), flush=True)
    safe = lambda m: {k: (float(v) if isinstance(v, (int, float, np.floating)) else str(v)) for k, v in m.items()}
    out = dict(layer=LAYER, prereg='strategies/S797_PREREG.md@7d40ab75', rule=dict(theta=THETA, pause=PAUSE, ksl=KSL, rr=RR, mh=MH),
               ...
               n_trials=N_TRIALS, holdout_start='2018-11-16T00:00:00Z', null=null,
               verdict=r['verdict'], rqs2_score=r.get('rqs2_score'), gates=r.get('gates'),
               metrics=safe(r.get('metrics', {})), notes=r.get('notes'))
    with open(os.path.join(HERE, 's797_final_result_POOL.json'), 'w') as f:
        json.dump(out, f, ensure_ascii=False, indent=1, default=str)
    pool.to_json(os.path.join(HERE, 's797_trades_POOL.json'), orient='records')
    print(f"\n[S797 OFFICIAL] {r['verdict']} score={r.get('rqs2_score')}", flush=True)
