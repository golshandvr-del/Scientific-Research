...
# -*- coding: utf-8 -*-
"""
S675 — فاز جست‌وجو (BOS) (فقط نیمهٔ نخست — مسیر C، PREREG در git قبل از این فایل)
...
from strategies.s675_bos_signals import bos_signals, drift_masks  # noqa: E402
...
def run_tf(tf):
    ...
    if 'mt5_full' not in d_src:
        print(f"[S675/{tf}] ⚠️ src={d_src} خارج از mt5_full — E-16 رد شد", flush=True)
        return None
    print(f"[S675/{tf}] src={d_src} n_full={n_full} search_half={n} max_hold={mh}",
          flush=True)
    ...
    for (gate, side), pool in pools.items():
        ...
        for k_sl in K_SLS:
            ...
            for rr in RRS:
                ...
                del tr
    gc.collect()
    print(f"[S675/{tf}] مبناها اندازه‌گیری شد ({len(uncond)} سلول، {time.time()-t0:.0f}s)",
          flush=True)

    rows = []
    cfg_i = 0
    L_raw, S_raw = bos_signals(h, lo, c)
    L_raw &= valid; S_raw &= valid
    print(f"[S675/{tf}] BOS events: long={int(L_raw.sum())} short={int(S_raw.sum())}", flush=True)
    for _ in (0,):
        for gate in GATES:
            if gate == 'aligned':
                Lg = L_raw & drift_up
            ...
            for k_sl in K_SLS:
                ...
                for rr in RRS:
                    ...
                    for side in ('long', 'short', 'both'):
                        ...
                        rows.append(dict(
                            gate=gate, side=side, k_sl=k_sl, rr=rr,
    ...
    print(f"[S675/{tf}] تمام شد: {cfg_i} پیکربندی، {len(rows)} نتیجه، {dt:.0f}s → {path}",
          flush=True)
    if rows:
        r = {k: v for k, v in rows[0].items() if not k.startswith('yearly')}
        print(f"[S675/{tf}] بهترین: {r}", flush=True)
    return out
