...
# -*- coding: utf-8 -*-
"""
S406 — گپ‌فیل + فیلتر نوسان علّی · XAUUSD M15 (پیوند نسخهٔ ACCEPT S404 به M15)
==============================================================================
پیش‌ثبت: results/S406_PREREG_GAP_FILL_M15.md (commit قبل از هر اجرا)
والد: S404 (ACCEPT 96.8, M30) · S400-M15 (مرگ هندسهٔ X-BAR با z=7.5 زنده)

فضای پیش‌ثبت (۸ ترکیب): q∈{60,70} × k_sl∈{1.7,2.0} × V∈{None,ATRq78} · dow≠0
قاعدهٔ برنده (قفل): کمترین maxDD نیمهٔ اول به شرط
  n≥100 · PF>1 · t≥2.5 · maxDD≤4.80% · wr_edge≥2pp · rec≥3 · گره‌گشایی: t.
حالت‌ها:
  python3 strategies/s406_gap_fill_m15.py tune
  python3 strategies/s406_gap_fill_m15.py verdict <q> <k_sl> <V|None>
"""
import sys, os, json
import numpy as np
import pandas as pd
...
TF = 'M15'
SEED = 406
N_TRIALS_CUM = 274                 # 266 + 8 (پیش‌ثبت §۵)
CKPT = os.path.join(os.path.dirname(__file__), '..', 'results', '_s406_tune_ckpt.json')


def run_layer(df, days, atr, q, k_sl, use_v, vflags=None,
              lo_bar=None, hi_bar=None, no_thresh=False, no_dow=False):
    """S406 روی M15. آستانه/DOW/V = مهارت (در null حذف می‌شوند)."""
...
def mode_tune():
    ...
    ok = [r for r in rows if r['n'] >= 100 and r['pf'] > 1 and r['t'] >= 2.5
...
def mode_verdict(q, k_sl, use_v):
    ...
    print(rqs2.format_rqs2(f'S406 {TF} q={q}/k_sl={k_sl}/V={use_v}', r), flush=True)
    outp = os.path.join(os.path.dirname(__file__), '..', 'results', '_s406_verdict.json')
