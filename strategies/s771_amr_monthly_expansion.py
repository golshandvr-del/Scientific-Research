"""S771 — «انبساطِ دامنه نسبت به لنگرِ ماه (AMR)» روی XAUUSD.

پیش‌ثبت: results/S771_PREREG_AMR_MONTHLY_EXPANSION_XAUUSD.md (کامیت 3c9aa0ed).
مسیر چندگانگی: C (holdout). جستجو فقط روی ۶۰٪ نخست؛ حداکثر یک داوری per کارت.

ثابت‌های قفل‌شده (PREREG §۴):
  SEED=20260826 · K_PERM=800 · SPLIT_FRAC=0.60 · AMR_P=13 · ATR_P=100 ·
  SL_K=1.272 · RR=2.058 · THETAS={0.35,0.50,0.65} · HOLDS={16,32}
هندسه از S770 منجمد آمده — هیچ جستجوی مجددی روی SL/RR انجام نمی‌شود.
"""
import argparse
import json
import os
...
HOLDS = (16, 32)
SCAN_DIR = os.path.join(ROOT, 'results', '_scan_S771')
MIN_N_EXPLORE = 30
...
def build_features(df):
    """mfrac = (close − monthOpen)/AMR13 — همه علّی (AMR با shift(1) ماهانه)."""
    t = pd.to_datetime(df['time'], unit='s')
    mon = t.dt.to_period('M').values
    monthly = df.groupby(mon).agg(hi=('high', 'max'), lo=('low', 'min'),
                                  op=('open', 'first'))
    monthly['rng'] = monthly['hi'] - monthly['lo']
    monthly['amr'] = monthly['rng'].rolling(AMR_P).mean().shift(1)
...
def explore_card(tf):
    ...
    df, src = load_card(tf)
    print(f'[S771 explore] tf={tf} src={src} bars={len(df):,}', flush=True)
    ...
    print(f'[S771 explore] {tf} → best={json.dumps(best, ensure_ascii=False)} '
...
def adjudicate_card(tf, n_trials=N_TRIALS):
    ...
    best = ex.get('best')
    if not best:
        print(f'[S771 adjudicate] {tf}: هیچ سلول پایداری نبود ⇒ REJECT-by-rule.',
              flush=True)
    ...
    df, src = load_card(tf)
    print(f'[S771 adjudicate] tf={tf} θ={theta} hold={hold} src={src} '
    ...
    print(f"[S771 adjudicate] {tf} → verdict={r['verdict']} "
