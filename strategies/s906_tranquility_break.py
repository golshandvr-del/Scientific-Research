"""
S906 — Tranquility-Break Displacement · XAUUSD · RQS2 v2.6 · Path C
=====================================================================
اولین شوک (range ≥ θ·ATR21[i−1]) پس از L کندلِ بدونِ شوک؛ جهت = بدنهٔ کندل شوک.
آناتومیک، علّی، ذاتاً یال‌محور. بدون ρ (S965)، بدون درفت (S966)، بدون حجم.

پیش‌ثبت: results/S906_PREREGISTRATION.md (کامیت 11aceca4، قبل از هر آزمون).
گرید منجمد: L∈{34,89} × θ∈{2.058,2.618} × k_sl∈{1.272,2.058} × hold∈{13,34}, RR=1.618 ⇒ N_eff=16.
"""
...
ASSET = 'XAUUSD'
OUT = os.path.join(ROOT, 'results', '_s906')
...
W = 21          # پنجرهٔ ATR (پیش‌ثبت §۲) — علّی (shift 1 در سیگنال)
WARMUP = 91
SPLIT_FRAC = 0.60
K_PERM = 1000
SEED = 905
...
def build_signals(d, L, th, atr_prev):
    """رویداد: شوک در i و هیچ شوکی در [i−L, i−1]. جهت = بدنهٔ کندل شوک."""
    shock = shock_mask(d, th, atr_prev)
    n = len(shock)
    # تعداد شوک در L کندل قبل (بدون خودِ i)
    cs = np.concatenate(([0], np.cumsum(shock.astype(np.int64))))
    idx = np.arange(n)
    lo = np.maximum(idx - L, 0)
    prev_cnt = cs[idx] - cs[lo]           # شوک‌های در [i−L, i−1]
    drought = prev_cnt == 0
    drought[:L] = False
    ev = shock & drought
    o, c = d['open'], d['close']
        ...
        for k_sl in GRID_KSL:
            for rr in (RR_FIXED,):
                for hold in GRID_HOLD:
                    i += 1
                    key = f'L{L}_t{th}_k{k_sl}_rr{rr}_h{hold}'
                    if key in results:
                        continue
...
def phase_final(tf):
    ...
    print(f"FINAL S906 {tf} · locked={locked['best_key']} · "
