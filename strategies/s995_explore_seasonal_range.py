# -*- coding: utf-8 -*-
"""
S995 — شوکِ دامنهٔ فصل‌زدوده × ماندگاریِ کایل (S965 در زیستگاهِ چگال) — «فقط نیمهٔ نخست»
==========================================================================================
S965 (ACCEPT 86، H6/H8): رویداد range ≥ 2.618×ATR21 و retention ρ ≥ 0.618 → follow. در H3 رد شد (lift +2.9).
فرض لاگرانژ: در TFهای پایین، ATR21 «فصلِ ساعتی» را نمی‌بیند — کندلِ بازشدنِ لندن همیشه بزرگ است و شوکِ دروغین می‌سازد.
نرمال‌سازی به «میانهٔ دامنهٔ همان ساعتِ روز در ۲۰ نشستِ قبل» (S994 روی حجم؛ اینجا روی دامنه — بکر در repo):
   rr_t = (high−low)_t / median(range همان slot، ۲۰ نشست، علّی)
لبه: rr ≥ θ∈{2.5, 3.5}؛ retention ρ∈{0.618, 0.8}؛ follow/fade؛ TF∈{M30,H1,H2}؛ rr∈{1.0,1.5}.
"""
import sys, os, json
...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'research', 's995_explore'); os.makedirs(OUT, exist_ok=True)
...
def seasonal_rr(df, ndays=20):
    ...
    v = pd.Series((df['high'].values-df['low'].values).astype(float))
...
with open(os.path.join(OUT,'seasonal_range.json'),'w') as f: json.dump(res,f,indent=1)
print("saved -> seasonal_range.json")
