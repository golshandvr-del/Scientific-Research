# -*- coding: utf-8 -*-
"""
s162_final_weaklayer_upgrade_summary.py
================================================================================
...
--------------------------------------------------------------------------------
این اسکریپتِ جمع‌بندیِ نهاییِ نشست است. پیکربندیِ بهینهٔ تثبیت‌شدهٔ هر لایهٔ ضعیف
(از S157–S161) را یک‌جا اجرا می‌کند و اثرِ خالصِ کل را می‌سنجد.

پیکربندیِ نهاییِ برنده (پاسخِ User Note «WR<40 را به ≥40 برسان با فیلتر/تأیید»):
  • S140 Monday:  فیلترِ تأییدِ امتیازی score≥3 از ۶     ⇒ WR 39.7→42.1 ، net +7,655→+9,920 (+2,265) ✅
  • S142 Mid-Mo:  TPِ تطبیقیِ رژیم-آگاه (score≥3⇒TP500 وگرنه TP150) ⇒ WR 35.5→41.8 ، net +21,012→+20,817 (−195) ✅
  • S143 EURUSD:  فیلترِ امتیازی score≥2 + TP40           ⇒ WR 34.0→43.6 ، net +3,934→+4,605 (+671) ✅
  • S81  Swing:   دست‌نخورده — WR-پایین ساختاری است (R:R≈۱۰:۱)؛ تحمیلِ WR≥40 سود را
                  −9,531$ نابود می‌کند ⇒ نقضِ مستقیمِ قانونِ #۱. صادقانه دست‌نخورده می‌ماند.
  • S91  Scalp:   خارج از این تغییرات (سودِ ناچیزِ +317$، محدودیتِ دادهٔ M5).

اثرِ خالصِ سه لایهٔ بهبودیافته (S140+S142+S143): Δ = +2,265 −195 +671 = **+$2,741**.
================================================================================
"""
import os, sys, json
...
def main():
    print("=" * 100)
    print("S162 — جمع‌بندیِ نهاییِ ارتقای لایه‌های ضعیف (WR<40٪ → ≥40٪)")
    print("قانونِ #۱: سودِ خالص (XAUUSD+EURUSD) اولویتِ مطلق دارد.")
    ...
    rows.append(('S140 Monday', 'فیلترِ امتیازی score≥3/6', base, imp, 'XAUUSD'))

    # S142 baseline + بهبود (TPِ تطبیقی: score≥3⇒TP500 وگرنه TP150)
    ...
    rows.append(('S142 Mid-Month', 'TPِ تطبیقی (score≥3⇒TP500 وگرنه TP150)', base, imp, 'XAUUSD'))
    ...
    rows.append(('S143 EURUSD Mid-Month', 'فیلترِ امتیازی score≥2 + TP40', base, imp, 'EURUSD'))

    # ---------- S81 (دست‌نخورده، فقط گزارش) ----------
    ...
    rows.append(('S81 Swing-Pullback', 'دست‌نخورده (WR-پایینِ ساختاری، R:R≈۱۰:۱)', base81, base81, 'XAUUSD_M30'))

    # ---------- جدول ----------
    print("\n" + "=" * 108)
    print(f"{'لایه':26s}{'baseWR':>8}{'newWR':>8}{'baseNet':>12}{'newNet':>12}{'ΔNet':>11}  پیکربندی")
    print("-" * 108)
    total_delta = 0.0
    out = []
    for name, cfg, b, imp, asset in rows:
        d = imp['net'] - b['net']
        if 'دست‌نخورده' not in cfg:
            total_delta += d
        print(f"{name:26s}{b['wr']:>7.1f}%{imp['wr']:>7.1f}%{b['net']:>+12,.0f}{imp['net']:>+12,.0f}{d:>+11,.0f}  {cfg}")
        out.append(dict(layer=name, config=cfg, asset=asset,
                        base_wr=b['wr'], new_wr=imp['wr'], base_net=b['net'],
                        new_net=imp['net'], delta=d, base_n=b['n'], new_n=imp['n'],
                        base_pf=b['pf'], new_pf=imp['pf']))
    print("-" * 108)
    print(f"{'اثرِ خالصِ سه لایهٔ بهبودیافته (S140+S142+S143)':60s}{total_delta:>+12,.0f}")
    print("=" * 108)
    print("\nنتیجه: هر ۳ لایهٔ بهبودیافته اکنون WR≥40٪ دارند و مجموعِ سودِ خالصشان "
          f"{total_delta:+,.0f}$ افزایش یافت.")
    print("S81 صادقانه دست‌نخورده ماند (تحمیلِ WR≥40 آن −9,531$ سود را نابود می‌کرد ⇒ نقضِ قانونِ #۱).")

    summary = dict(improved_layers=out, total_delta_3layers=total_delta,
                   note='S81 دست‌نخورده؛ S91 خارج از این نشست.',
                   record_before=241487, record_after=241487 + total_delta)
    with open(os.path.join(RESULTS, '_s162_final_weaklayer_upgrade.json'), 'w') as f:
        json.dump(summary, f, ensure_ascii=False, indent=2, default=float)
    print(f"\nرکوردِ رسمی: +$241,487 → +${241487 + total_delta:,.0f} (Δ{total_delta:+,.0f})")
    print("✅ ذخیره شد: results/_s162_final_weaklayer_upgrade.json")
    return summary
