# -*- coding: utf-8 -*-
"""
S651 — آزمونِ نهایی روی نیمهٔ دومِ دست‌نخورده (مسیرِ C) — قفل‌شده طبق PREREG
=============================================================================
⛔ طبقِ `research/S651_PREREG.md` (کامیت bf939fcc) و **پیش از اجرا** کامیت
می‌شود. هیچ پارامتری این‌جا انتخاب نمی‌شود. نیمهٔ دوم هر TF یک‌بار لمس می‌شود.

روش (عیناً الگوی داوریِ S650 — فقط سیگنال و جدولِ قفل متفاوت):
  • معاملات با موتورِ رسمی `engine.scalp_engine.simulate_trades`
  • مدلِ صفر: K=600 زیرمجموعه از رخدادهای غیرشرطیِ هم‌هندسه، بذر 651651
...
from strategies.s651_sign_imbalance_explore import (       # noqa: E402
    signals, GEO_ATR_P, GEO_SL_K, GEO_RR, GEO_HOLD)
from tools import s434_fast_data as fd                     # noqa: E402

OUT = os.path.join(ROOT, 'results', '_scan_S651')
ASSET = 'XAUUSD'
SEED = 651651            # منجمد (PREREG)
PERM_K = 600             # منجمد
N_TRIALS = 17            # منجمد
SPLIT_FRAC = 0.70        # منجمد

# جدولِ قفل‌شدهٔ PREREG — TF → (W, θ). تغییرش = نقضِ پیش‌ثبت.
LOCKED = {
    'M1': (55, 0.618), 'M3': (34, 0.764), 'M4': (34, 0.764),
    'M5': (34, 0.764), 'M6': (21, 0.764), 'M10': (21, 0.764),
    'M12': (21, 0.764), 'M15': (34, 0.618), 'M20': (13, 0.618),
    'M30': (13, 0.764), 'H1': (8, 0.764), 'H2': (8, 0.764),
    'H3': (13, 0.764), 'H6': (21, 0.618), 'H8': (8, 0.764),
    'H12': (13, 0.764), 'D1': (8, 0.618),
}
...
def judge_tf(tf):
    ...
    print(f"\n{'='*88}\n=== S651 FINAL :: {ASSET} {tf} — locked W={w} θ={th} "
          f"| test=second half ({len(df):,} bars)\n    src={src}", flush=True)

    warmup = max(2 * 55, 4 * GEO_ATR_P, 200)
    ...
    if tr is None or len(tr) == 0:
        return dict(layer='S651', tf=tf, verdict='REJECT (no trades)',
                    locked=dict(w=w, theta=th), src=src)
    ...
    print(rqs2.format_rqs2(f'S651_{tf}', res), flush=True)

    out = dict(layer='S651', tf=tf, asset=ASSET, src=src,
               locked=dict(w=w, theta=th, atr_p=GEO_ATR_P, sl_k=GEO_SL_K,
...
def _save_push(tf, out):
    ...
    try:
        ...
        r = subprocess.run(
            ['git', 'commit', '-m',
             f"S651 FINAL {tf}: {out.get('verdict', '?')} "
             f"(hold-out second half, per PREREG bf939fcc)"],
...
def main():
    ...
    print(f"S651 FINAL TEST — path C — locked per PREREG bf939fcc — TFs: {tfs}",
          flush=True)
    for tf in tfs:
        fp = os.path.join(OUT, f'final_{tf}.json')
        ...
        try:
            ...
            out = dict(layer='S651', tf=tf, verdict='ERROR', error=str(e))
            print(f"    ✖ {tf} ERROR: {e}", flush=True)
        _save_push(tf, out)
    print("\nS651 FINAL — DONE", flush=True)
