# -*- coding: utf-8 -*-
"""
S854 — Directional-Change Overshoot (زمان درونی) — XAUUSD فقط
================================================================================
پیش‌ثبت : results/S854_PREREG_DIRECTIONAL_CHANGE_OVERSHOOT.md  (commit 186d6c9c)
داور    : engine/rqs2.compute_rqs2 — حکم موتور عیناً گزارش می‌شود
مسیر C  : اکتشاف فقط ۶۰٪ نخست؛ یک قضاوت با split_bar. n_trials = 24.

فرضیه (ماندلبرو): در زمان درونی رویدادها (Mandelbrot & Taylor 1967)،
پس از تأیید جهت‌گردانی با آستانه δ=d·ATR34 (منجمد per-leg)،
فاز overshoot به‌طور میانگین هم‌اندازه‌ی δ ادامه دارد
(Glattfelder-Dupuis-Olsen 2011) ⇒ سواری بر جهت جدید.

خانواده‌ی مستقل — CUSUM در S85x بسته شد. درس‌ها: caps 50k/1M، np.bool_.
"""
import sys
import os
import json
...
ASSET = 'XAUUSD'
D_THRESH = (1.618, 2.618, 4.236)
K_SL = (1.272, 1.618)
RRS = (1.0, 1.618)
ATR_P = 34
MAX_HOLD = 34
SPLIT_FRAC = 0.60
N_TRIALS = 24                    # 12 ترکیب × 2 سمت
N_PERM = 600
MAX_UNCOND = 50_000
...
EXPLORE_MIN_N = 30
SL_FLOOR_PIP = 5.0
OUT = 'results/_scan_S854'


def dc_signals(close, atr, d, warmup):
    """ماشین Directional-Change: δ_ref منجمد در بارِ ثبت اکسترمم هر پا.
    تأیید DC نزولی ⇒ SHORT؛ تأیید DC صعودی ⇒ BUY. علّیت سخت."""
    n = len(close)
    long_sig = np.zeros(n, dtype=np.bool_)
    short_sig = np.zeros(n, dtype=np.bool_)
    state = 1                      # +1=UP، -1=DOWN (شروع UP فرضی)
    ext = close[warmup]
    a0 = atr[warmup]
    delta = d * (a0 if (a0 == a0 and a0 > 0.0) else 0.0)
    for i in range(warmup + 1, n):
        if delta <= 0.0:           # هنوز ATR معتبر نداریم
            a0 = atr[i]
            if a0 == a0 and a0 > 0.0:
                ext = close[i]
                delta = d * a0
        ...
        if state == 1:
            if close[i] > ext:
                ...
                a0 = atr[i]
                if a0 == a0 and a0 > 0.0:
                    delta = d * a0          # δ منجمد در بار اکسترمم جدید
            elif close[i] <= ext - delta:
                short_sig[i] = True         # تأیید DC نزولی
                state = -1
                ext = close[i]
                a0 = atr[i]
                delta = d * (a0 if (a0 == a0 and a0 > 0.0) else 0.0)
            ...
            if close[i] < ext:
                ...
                long_sig[i] = True          # تأیید DC صعودی
                state = 1
                ext = close[i]
                a0 = atr[i]
                delta = d * (a0 if (a0 == a0 and a0 > 0.0) else 0.0)
...
try:
    ...
    dc_signals = njit(cache=True)(dc_signals)
except Exception:
    pass
...
def run_tf(tf, verbose=True):
    print(f"\n{'='*90}\n=== S854 DC Overshoot :: XAUUSD-{tf} ===", flush=True)
    ...
    sig_cache = {}
    for dth in D_THRESH:
        ls, ss = dc_signals(close, atr, dth, warmup)
        sig_cache[dth] = (ls, ss)
        for k in K_SL:
            for rr in RRS:
                st = combo_trades(df, ls, ss, atr, k, rr,
                                  warmup, hi_explore, pip)
                row = dict(d=dth, k=k, rr=rr,
                           n=(st['n'] if st else 0),
                ...
                grid.append(row)
                if verbose:
                    print(f"    explore d={dth:<6} k={k:<6} rr={rr:<6} "
    ...
    win = elig[0]
    d_w, k_w, rr_w = win['d'], win['k'], win['rr']
    print(f"\n    ★ winner (exploration only): d={d_w} k={k_w} rr={rr_w}  "
    ...
    out = dict(tf=tf, src=src, bars=n, split_bar=split,
               ...
               judged=dict(d=d_w, k=k_w, rr=rr_w,
