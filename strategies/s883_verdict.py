#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""S883 — آزمونِ نهاییِ Path C روی **نیمهٔ دوم** (hold-out) — یک بار، فقط یک بار.

پیش‌ثبت: results/S883_PREREG_NRBreakout_PathC.md (commit bb9ec536)
هیچ چندک منجمدی وجود ندارد (رویداد آناتومیک نسبی)؛ فقط sl/tp منجمد از نیمهٔ اول.
null کانونی K=500 per-side؛ compute_rqs2 با هر ۵ ورودی؛ n_trials=36؛ seed=883.
usage: python3 strategies/s883_verdict.py <TF> [<TF> ...]
"""
...
SCAN = os.path.join(ROOT, 'results', '_scan_S883')
N_TRIALS = 36
N_PERM = 500
...
def judge_tf(tf):
    ...
    m, kc, hold = best['m'], best['kc'], best['hold']
