#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""S884 — آزمونِ نهاییِ Path C روی **نیمهٔ دوم** (hold-out) — یک بار، فقط یک بار.

پیش‌ثبت: results/S884_PREREG_DormancyBreak_PathC.md (commit a1b1d7c3)
رویداد آناتومیک نسبی (قلهٔ/کف تازهٔ W-کندلی + خشکسالی ≥Dmin)؛ هیچ چندک منجمدی
وجود ندارد؛ فقط sl/tp منجمد از نیمهٔ اول. محور side از نامزد IS منجمد است.
null کانونی K=500 per-side؛ compute_rqs2 با هر ۵ ورودی؛ n_trials=48؛ seed=884.
usage: python3 strategies/s884_verdict.py <TF> [<TF> ...]
"""
...
SCAN = os.path.join(ROOT, 'results', '_scan_S884')
N_TRIALS = 48
N_PERM = 500
...
def judge_tf(tf):
    ...
    W, Dmin, side, hold = best['W'], best['Dmin'], best['side'], best['hold']
