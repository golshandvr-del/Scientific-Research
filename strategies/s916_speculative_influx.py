# -*- coding: utf-8 -*-
"""S916 — «هجوم سفته‌بازان»: شوک حجم تیک ۵۵باره + بدنهٔ گاوی + هم‌جهتی دریفت · XAUUSD · Long-only

پیش‌ثبت: results/S916_PREREG_speculative_influx.md (کامیت bfe701e5)
مسیر C: کارت اصلی H2، جست‌وجو فقط در ۶۰٪ نخست، تست نهایی ۱ اجرا (n_trials=1).

سیگنال (منجمد — آناتومیک، صفر پارامترِ توزیعی — درس S882):
  شوک حجم: volume[t] > max(volume[t-55..t-1])
  بدنهٔ گاوی: close[t] > open[t]
  هم‌جهتی دریفت: close[t] > close[t-21]
  ورود کندل بعد (بدون نگاه به آینده). فقط LONG.

هندسه: TP=q60(MFE_train) · SL=q30(MAE_train) · TP>=SL · max_hold=34

اجرا:
  python3 strategies/s916_speculative_influx.py --phase train --tf H2
  python3 strategies/s916_speculative_influx.py --phase holdout --tf H2 --n_trials 1
  python3 strategies/s916_speculative_influx.py --phase family
"""
...
SEED = 916
TREND_LB = 21                    # منجمد — فیبوناچی: پنجرهٔ هم‌جهتی دریفت
VOL_LB = 55                      # منجمد — فیبوناچی: پنجرهٔ سقف حجم
...
CKPT_DIR = os.path.join(ROOT, 'results', '_s916_ckpt')
...
def ignition_signal(df: pd.DataFrame) -> np.ndarray:
    """هجوم سفته‌بازان — آناتومیک محض؛ ورود کندل بعد (بدون نگاه به آینده)."""
    o = df['open'].to_numpy()
    c = df['close'].to_numpy()
    v = df['volume'].to_numpy(dtype=float)
    n = len(c)
    # سقف حجم ۵۵ بار قبلی (شیفت‌خورده — بار جاری داخل پنجره نیست)
    vmax_prev = pd.Series(v).shift(1).rolling(VOL_LB).max().to_numpy()
    ev = np.zeros(n, bool)
    t = np.arange(VOL_LB + TREND_LB + 1, n)
    vshock = v[t] > vmax_prev[t]                # شوک حجم آناتومیک
    bull = c[t] > o[t]                          # بدنهٔ گاوی
    drift = c[t] > c[t - TREND_LB]              # هم‌جهتی دریفت
    ev[t] = vshock & bull & drift
    out = np.zeros(n, bool)
...
def phase_train(tf: str):
    ...
    out = {'tf': tf, 'split_bar': split, 'n_train': len(df_tr),
           'n_sig': int(sig.sum()), 'trend_lb': TREND_LB, 'vol_lb': VOL_LB,
           'max_hold': MAX_HOLD}
