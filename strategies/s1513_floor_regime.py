# -*- coding: utf-8 -*-
"""S1513 — رکوردِ تازهٔ کف (S1511 منجمد) × رژیمِ σ (S605 منجمد، W=233) ⇒ LONG | H6/H8 × {calm,storm}.

پیش‌ثبت: `results/S1513_PREREG_FRESH_FLOOR_SIGMA_REGIME.md` (کامیت 9626d227 — **قبل** از هر عدد).
استفاده: python3 strategies/s1513_floor_regime.py {calm|storm} [CARDS...]
رویداد: S1511 & regime(arm)؛ نول مشروط در فضای drift>0 ∧ regime(arm).
...
n_trials=20. SEED=20260907, K=2000.
تشخیصی: پایهٔ S1511 بی‌گیت روی همان کارت (P1 مرجع).
"""
...
def sig_base(df):  # S1511 عیناً
    ff = ff_state(df)
    return ff & ~ff.shift(1, fill_value=False) & drift_mask(df)


def sig_fresh(df):  # S1513: پایه × رژیم
...
def main():
    ...
    cards = args or CARDS
    print(f'S1513 fresh-floor x sigma-regime[{ARM}] W={W_CALM} | null in drift>0 & regime | '
    ...
    for card in cards:
        try:
            ...
            r['p1_base_s1511'] = diag(df, sig_base, sl_abs, ps, be)
            r['arm'] = ARM
        ...
        print(f'{card}: span={r.get("span_years")}y n={r.get("n_trades")} '
              ...
              f'arm={ARM} base_S1511={r.get("p1_base_s1511")} pass={r.get("pass_rate_pct")}%',
              flush=True)
