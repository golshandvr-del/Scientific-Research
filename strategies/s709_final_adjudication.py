# -*- coding: utf-8 -*-
"""
S709 — داوریِ نهایی (یک آزمون، مسیر C)
سلولِ منجمدشده در جست‌وجو (نیمهٔ اول): XAUUSD-H8 · N=89 · gate=0 · cont · k_sl=1.618
هندسهٔ منجمد: SL = 1.618 × median(ATR55 علّیِ نیمهٔ جست‌وجو) ؛ TP = 1.5×SL
...
_spec = _u.spec_from_file_location(
    's709scan', os.path.join(os.path.dirname(os.path.abspath(__file__)),
                             's709_range_record_scan.py'))
...
def main():
    ...
    ev = build_events(df, N_WIN, GATE)              # رکورد دامنه — همان تابع اسکن
    ev_next = np.zeros_like(ev)
    ev_next[1:] = ev[:-1]
    print(f"[رکورد] n_events(full)={(ev!=0).sum()} first-half={(ev[:half]!=0).sum()} "
          f"second-half={(ev[half:]!=0).sum()}", flush=True)
    ls = pd.Series(ev_next == 1, index=df.index)   # cont: هم‌جهت بدنه
    ...
    null_path = os.path.join(OUT, 'final_null_H8.json')
    ...
    line = (f"S709-H8 OFFICIAL | {res['verdict']} RQS2={res['rqs2_score']:.1f} | "
