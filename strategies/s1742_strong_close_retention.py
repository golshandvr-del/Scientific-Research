...
# -*- coding: utf-8 -*-
"""
S1742 — بستهٔ قوی × رانش دوگانه × ماندگاری بدنه (Strong-Close × Kyle ρ≥0.618) — XAUUSD
================================================================================
پیش‌ثبت: results/S1742_PREREG_STRONG_CLOSE_BODY_RETENTION_PATH_C.md (کامیت قبل از کد)
قاعده: S749/S748-H2 winner_params (از JSON) + گیت ρ=|c−o|/(h−l) ≥ 0.618 (ثابت S965).
بدون شبکه. n_trials=649 رسمی؛ 2000/5176 تنش. کارت داوری H2؛ H4/H6 تأییدی.
P6: هم‌پوشانی با گیت آرامش S1740 گزارش می‌شود.
داده: فقط data/mt5_full (کد src را چک می‌کند و در غیر این صورت خطا می‌دهد).
اجرا: python3 strategies/s1742_strong_close_retention.py <TF>
"""
import os
import sys
import json
...
RHO_THR = 0.618                                                     # ثابت طلایی S965 — قفل
...
OUT_DIR = os.path.join(ROOT, 'results', '_scan_S1742')
PREREG = 'results/S1742_PREREG_STRONG_CLOSE_BODY_RETENTION_PATH_C.md'
...
def run_tf(tf):
    ...
    print(f"\n{'='*88}\n=== S1742 STRONG-CLOSE × RHO :: {ASSET}-{tf} bars={n:,} src={d['src']}\n"
          f"    span={d['first_utc']} → {d['last_utc']} ({d['span_years']:.2f}y) "
          f"split_bar={split} N_TRIALS={N_TRIALS} RHO_THR={RHO_THR}", flush=True)
    ...
    o_ = df['open'].values; h_ = df['high'].values; l_ = df['low'].values; c_ = df['close'].values
    with np.errstate(divide='ignore', invalid='ignore'):
        rho = np.where((h_ - l_) > 0, np.abs(c_ - o_) / (h_ - l_), np.nan)
    rv_ev = rho[sig_all]
    calm_mask = np.isfinite(rv_ev) & (rv_ev >= RHO_THR)       # بازوی داوری: بدنهٔ غالب
    storm_mask = np.isfinite(rv_ev) & (rv_ev < RHO_THR)       # counter: سایه‌دار
    n_ev = len(sig_all)
    ...
    p6 = dict(n_rho=n_calm, n_calm=int(calm_s1740.sum()), n_both=both,
              overlap_pct_of_rho=round(100.0 * both / n_calm, 1) if n_calm else None,
              rho_median_at_events=round(float(np.nanmedian(rv_ev)), 3) if n_ev else None)
    print(f"    events={n_ev} rho>=.618={n_calm} rho<.618={int(storm_mask.sum())} "
          f"pass_rate={pass_rate}%  P6={p6}", flush=True)

    out = dict(strategy='S1742_StrongCloseRetention', asset=ASSET, tf=tf, bars=n,
               src=d['src'], span_years=round(float(d['span_years']), 2),
               split_bar=split, n_trials=N_TRIALS, seed=SEED, k_perm=K_PERM,
               prereg=PREREG, params=dict(**wp, rho_thr=RHO_THR),
    ...
    off = judge(N_TRIALS, f'S1742-{tf} RHO ({N_TRIALS})')
    out['official'] = off
    out['stress'] = {str(nt): judge(nt, f'S1742-{tf} STRESS ({nt})') for nt in N_STRESS}
    ...
    json.dump(out, open(os.path.join(OUT_DIR, f'{tf}.json'), 'w'),
              ensure_ascii=False, indent=1, default=_default)
    print(f"    P1(lift_vs_be rho {rep['lift_vs_be']} > ungated {rep_u['lift_vs_be']}): "
