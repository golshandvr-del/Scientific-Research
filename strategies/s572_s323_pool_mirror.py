# -*- coding: utf-8 -*-
"""
S572 — استخرِ S323 با هندسهٔ آینه‌ای (TP≥SL): XAUUSD M30+H1
================================================================================
پیش‌ثبت: `results/S572_PREREG_s323_pool_mirror_geometry.md` (کامیت 24e0c2a9،
قبل از این اجرا).

تنها تفاوت با S571: برای هر عضو slMult↔tpMult جابه‌جا می‌شود (صفر پارامتر آزاد).
نول‌ها با هندسهٔ جدید بازساخته می‌شوند. بقیه عیناً دستورکارِ S571.

عهدها: n_trials=2802 (تنش 5604) · C5″-ب: lift_pool ≥ +8.0pp وگرنه REJECT-STOP ·
یک داوری؛ حکم عیناً گزارش می‌شود · فقط XAUUSD.
"""
...
N_TRIALS = 2802
N_TRIALS_STRESS = 5604
PERM_K = 2000
SEEDS = (23, 101, 777)
SPLIT_FRAC = 0.60
C5B_MIN_POOL_LIFT = 8.0    # پیش‌ثبت §۳ — آستانهٔ پایین‌تر چون TP دورتر است
OUT = 'results/_s572_mirror'
MEMBER_CARDS = ['XAUUSD-M30', 'XAUUSD-H1']

os.makedirs(OUT, exist_ok=True)


def build_member(card: str) -> dict:
    """بازتولیدِ عضو با سیگنالِ منجمدِ S570 اما هندسهٔ آینه‌ای slMult↔tpMult."""
    asset, tf = card.split('-')
    ...
    # ★ تبدیلِ آینه‌ای — تنها تفاوت با S571:
    sl = round(cfg['tpMult'] * atr_pip_med, 1)   # SL جدید = tpMult قدیم
    tp = round(cfg['slMult'] * atr_pip_med, 1)   # TP جدید = slMult قدیم
    ...
    print(f"[{card}] src={d['src']} bars={len(df)} n={n} WR={wr:.2f} "
          f"null={ref:.2f} lift={lift:+.2f}pp SL={sl} TP={tp} (RR={tp/sl:.2f}) "
          f"mh={mh}", flush=True)
...
def blend_pool_null(members_used, pool_df, seed):
    """نولِ استخر: ترکیبِ وزنی به سهمِ پس-از-FIFO (دستورکارِ S431/S571)."""
...
def main():
    t0 = time.time()
    print(f"S572 — استخرِ آینه‌ای S323 M30+H1 (TP>=SL) | "
          f"n_trials={N_TRIALS}/{N_TRIALS_STRESS} K={PERM_K}", flush=True)
    ...
    if res is None:
        ...
        with open(os.path.join(OUT, 'no_members.json'), 'w') as fh:
            json.dump(dict(status='NO_VALID_MEMBERS'), fh)
        return
    pool = res['pool']
    ...
    # محورِ مشترکِ ۳۰دقیقه‌ای (عیناً S571)
    ...
    for seed in SEEDS:
        ...
        for label, nt in (('honest', N_TRIALS), ('stress', N_TRIALS_STRESS)):
            ...
            if seed == 23 and label == 'honest':
                print('\n' + R2.format_rqs2('S572-MIRROR', r), flush=True)
    ...
    rec = dict(session='S572', variant='mirror_geometry_TPgeSL',
    ...
    with open(os.path.join(OUT, 'S572_mirror_verdict.json'), 'w',
              encoding='utf-8') as fh:
        json.dump(rec, fh, ensure_ascii=False, indent=1, default=str)
    print(f"\n→ wrote {OUT}/S572_mirror_verdict.json  ({rec['elapsed_s']}s)",
          flush=True)
