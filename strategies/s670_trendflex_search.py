...
# -*- coding: utf-8 -*-
"""
S670 — فاز جست‌وجو (فقط نیمهٔ نخستِ داده — مسیر C پیش‌ثبت‌شده در commit b125212d)

قضیهٔ ادعایی: گذر TrendFlex اِهلرز (۲۰۲۰) از آستانهٔ ±θ روی XAUUSD شتاب‌گیریِ روند
را با مهارت فراتر از مبنای بی‌قیدِ اندازه‌گیری‌شده علامت می‌زند.
...
قوانین رعایت‌شده:
  ...
  * خانوادهٔ منجمد: period×θ×side×k_sl×RR = 5×4×3×3×2 = 360 پیکربندی در هر TF
  * TrendFlex از tools/s670_trendflex_fast (برابری بیت‌به‌بیت با بانک اثبات شده)
  * مبنا: اندازه‌گیری‌شده (ورود بی‌قید نمونه‌گیری‌شده با همان هندسه) — هرگز نظری
  * قانون بودجه: RR ≥ 1 (TP ≥ SL) — همیشه
  * قانون اندک‌اندک: خروجی هر TF بلافاصله JSON در results/_scan_S670/
  * بهداشت حافظه (سندباکس ~1GB): آزادسازی فوری df کامل + gc در هر period

اجرا:  python3 strategies/s670_trendflex_search.py M1
"""
import gc
import json
import os
...
PERIODS = (13, 21, 34, 55, 89)           # فیبوناچی غیرگرد — منجمد
THETAS = (1.0, 1.272, 1.618, 2.0)        # منجمد
...
def run_tf(tf):
    ...
    print(f"[S670/{tf}] src={d_src}  n_full={n_full}  search_half={n}  "
    ...
    if len(vidx) < 500:
        print(f"[S670/{tf}] داده ناکافی — رد", flush=True)
        out = {'tf': tf, 'skip': 'insufficient_data', 'n_search': n}
    ...
    for k_sl in K_SLS:
        ...
        for rr in RRS:
            ...
            for side in ('long', 'short'):
                ...
                del tr
    gc.collect()
    print(f"[S670/{tf}] مبنای بی‌قید اندازه‌گیری شد "
    ...
    for period in PERIODS:
        ...
        prev[0] = 0.0
        for th in THETAS:
            # رویداد: گذرِ +θ به بالا → LONG؛ گذرِ −θ به پایین → SHORT (متقارن)
                ...
                for rr in RRS:
                    ...
                    for side in ('long', 'short', 'both'):
                        ...
                        if n_tr == 0:
                            continue
                        wr = wr_of(tr)
                        # مبنای وزنی به سمت (الگوی s431/s432/s620)
                        if side == 'both':
                        ...
                        rows.append(dict(
                            period=period, theta=th, side=side, k_sl=k_sl, rr=rr,
        ...
        del tfx, prev
        gc.collect()
        print(f"[S670/{tf}] period={period} تمام شد ({time.time()-t0:.0f}s)",
              flush=True)
    dt = time.time() - t0
    ...
    print(f"[S670/{tf}] تمام شد: {cfg_i} پیکربندی، {len(rows)} نتیجه، "
          f"{dt:.0f}s → {path}", flush=True)
    if best:
        print(f"[S670/{tf}] بهترین: {best}", flush=True)
    return out
