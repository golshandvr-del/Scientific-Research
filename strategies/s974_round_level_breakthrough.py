...
# -*- coding: utf-8 -*-
"""
S974 — عبورِ تازه از سطحِ رُندِ ۱۰۰دلاری (Fresh Round-Level Breakthrough)
================================================================================
پیش‌ثبت: results/S974_PREREG_round_level_breakthrough.md (پیش از هر PnL)
معیار:   RQS2 v2.6 gates_only. مسیرِ B — یک تعریفِ منجمد، n_trials=24.

تعریف (بندِ ۲ پیش‌ثبت):
  lev[i]=floor(close[i]/100). بارِ عبور: lev[i]≠lev[i−1] (چند سطح ⇒ همه بررسی).
  تازگی: دست‌کم یکی از سطوحِ عبورشده در ۷۲ ساعتِ اخیر لمس نشده باشد (علّی).
  جهت: follow — LONG اگر lev بالا رفت، SHORT اگر پایین. ورود: openِ کندلِ بعدی.

هندسه (بندِ ۲): SL=1.0×median(ATR14)، TP=1.5×SL، hold=6h (کف ۴ کندل).
نال (بندِ ۴): جای‌گشتِ مکانِ ورود، K=2000 هر سمت، بذر 20260830، split=0.70.

فقط XAUUSD. H4 بازنمونه از H1. checkpoint: results/_scan_S974/<TF>.json
اجرا: python3 strategies/s974_round_level_breakthrough.py M1 [--kperm 2000]
"""
import sys
import os
import json
...
GRID = 100.0                  # شبکهٔ سطوحِ رُند (دلار)
FRESH_S = 72 * 3600           # تازگیِ سطح: ۷۲ ساعت بدونِ عبورِ قبلی
ATR_WIN = 14
SL_K = 1.0
RR = 1.5
HOLD_HOURS = 6.0
...
def build_signals(df):
    """(sig_idx, is_long) — عبورِ تازه از سطحِ رُندِ GRID دلاری؛ کاملاً causal.

    حلقه فقط روی بارهای عبور (lev[i]≠lev[i−1]) می‌چرخد — روی M1 هم اندک است
    (~۲۰k روی H1، مشابه روی M1 چون عبورِ close از سطحِ ۱۰۰دلاری نادر است).
    دیکشنری last_cross زمانِ آخرین عبورِ هر سطح را نگه می‌دارد (فقط گذشته).
    ورود در کندلِ بعدی توسطِ queue_rr/barrier انجام می‌شود (sig+1).
    """
    t = df['time'].values.astype(np.int64)
    c = df['close'].values.astype(np.float64)

    lev = np.floor(c / GRID).astype(np.int64)
    cross = np.flatnonzero(np.diff(lev) != 0) + 1           # بارهای عبور

    sig, is_long = [], []
    last_cross = {}                                          # سطح → زمانِ آخرین عبور
    for i in cross:
        lo = int(min(lev[i - 1], lev[i]))
        hi = int(max(lev[i - 1], lev[i]))
        fresh_any = False
        for L in range(lo + 1, hi + 1):                      # سطوحِ عبورشده
            lt = last_cross.get(L)
            if lt is None or (t[i] - lt) >= FRESH_S:
                fresh_any = True
            last_cross[L] = int(t[i])                        # همیشه به‌روزرسانی
        if fresh_any:
            sig.append(int(i))
            is_long.append(bool(lev[i] > lev[i - 1]))        # follow
...
def run_tf(tf, k_perm=K_PERM, seed=SEED):
    ...
    print(f"    fresh round-level signals: {len(sig):,} (L={int(is_long.sum()):,}"
    ...
    print('\n' + rqs2.format_rqs2(f'S974-{tf}', r), flush=True)
