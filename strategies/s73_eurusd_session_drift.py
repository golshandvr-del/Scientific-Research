...
استراتژی (Time-of-Day، ذاتاً forward-safe چون فقط به ساعتِ ساعت-دیوارِ کندل وابسته):
  • ورود Long روی سیگنالِ کندلِ آخرِ ساعتِ 23 UTC (close آن) ⇒ ورود در open کندلِ
    ساعتِ 0 UTC (کندلِ بعد). این دقیقاً «باز شدنِ روزِ اروپا» را می‌گیرد.
  • TP و SL بر پایهٔ ATR (رژیم-آگاه) — پیش‌فرض SL=1.1×ATR، TP=1.4×ATR.
  • فیلترِ اختیاریِ pullback: فقط وقتی وارد شویم که چند کندلِ قبل نزولی بوده باشد.
  • Short اختیاری در ساعتِ 13 UTC (drift نزولیِ پایدار).
...
# --- پارامترهای ثابتِ استراتژی (از پروفایلِ اکتشافی، بدونِ بهینه‌سازیِ overfit) ---
LONG_ENTRY_HOUR = 0      # ورود در open کندلِ ساعتِ 0 UTC (سیگنال روی کندلِ ساعتِ 23)
SHORT_ENTRY_HOUR = 13    # drift نزولیِ پایدار
ATR_PERIOD = 14
SL_ATR = 1.1
TP_ATR = 1.4
MAX_HOLD = 8             # ~2 ساعت؛ افقِ سودآورِ کشف‌شده
...
def build_signals(df):
    ...
    # سیگنالِ Short: کندلی که کندلِ بعدش ساعتِ 13 UTC است
    is_last_before_h13 = np.zeros(n, dtype=bool)
    is_last_before_h13[:-1] = (hour[1:] == SHORT_ENTRY_HOUR) & (hour[:-1] != SHORT_ENTRY_HOUR)
    short_sig = is_last_before_h13 & eval_mask
...
def run_eurusd(cfg=EURUSD_CFG, label="S73 base"):
    ...
    n = len(df)
    atrv = ind.atr(df, ATR_PERIOD).values
    valid = ~np.isnan(atrv)
