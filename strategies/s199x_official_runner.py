...
# ---------------- قواعد منجمد ----------------
CFG = {
 's1990': dict(name='AsianFreshLowFadeLong', tf='H1', side='long', mh=48, nt=220, seed=19901990),
}
...
def rule_s1990(df):
    ...
    cs = pd.Series(c); nl = (cs < cs.shift(1).rolling(90).min()).fillna(False)
    edge = (nl & ~nl.shift(1, fill_value=False)).astype(bool)
    hr = (df['time'].values.astype(np.int64) % 86400)//3600
    gate = pd.Series(hr < 8, index=df.index)
