# -*- coding: utf-8 -*-
"""
S648 — TweezerTopDriftShort — داوریِ نهاییِ holdout (مسیرِ C — فقط یک بار اجرا)
=======================================================================
پیش‌ثبت: results/S648_PREREG_TWEEZER_TOP_DRIFT_SHORT_XAUUSD_PATHC.md (commit b755bc3a)
منطق عیناً جدولِ §۲ پیش‌ثبت — صفر درجهٔ آزادیِ باز:

  • کارت‌ها: {H3, H4, H6, H8, H12}
  • لانگ: وجود ندارد (short-only)
  • شورت: cdl_tweezertop (سقفِ دوقلو، |Δhigh|≤0.05×range، صعودی→نزولی) و درفتِ ۶۰ روزِ تقویمی < 0 · TP = 1.618×SL
...
def run_card(tf, rng):
    ...
    tw_top = np.asarray(ib.compute('cdl_tweezertop', df)) != 0
    ...
    bull = np.zeros(len(cv), bool)          # فقط شورت — لانگ پیش‌ثبت نشده
    bear = tw_top & (drift < 0)             # Tweezer-top هم‌راستا با درفتِ نزولی → شورت
    ...
    sl = max(1.0, round(1.5 * slb, 1)); tp = round(1.618 * sl, 1)
...
def main():
    ...
    print('== S648 — داوریِ نهاییِ holdout · Tweezer-top × drift60d<0 · SHORT-ONLY · RR1.618 · {H2,H3,H4,H6,H8,H12} · '
    ...
    if res is None:
        print('\n[S648] هیچ عضوِ معتبری نماند — استخر تهی. حکم: مرگ در H0.',
              flush=True)
    ...
    print('\n' + R.format_rqs2('S648-TWZ-POOL', r), flush=True)

    out = dict(prereg='S648_PREREG_TWEEZER_TOP_DRIFT_SHORT_XAUUSD_PATHC.md@b755bc3a',
