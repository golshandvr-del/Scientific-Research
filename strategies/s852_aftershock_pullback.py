# -*- coding: utf-8 -*-
"""
S852 — CUSUM Aftershock Pullback (پس‌لرزه‌ی اصلاحی) — XAUUSD فقط
================================================================================
پیش‌ثبت : results/S852_PREREG_CUSUM_AFTERSHOCK_PULLBACK.md  (commit 277a2aa6/c08f063a)
داور    : engine/rqs2.compute_rqs2 — حکم موتور عیناً گزارش می‌شود
مسیر C  : اکتشاف فقط ۶۰٪ نخست؛ یک قضاوت با split_bar. n_trials = 64.

فرضیه (ماندلبرو): پس از شکست ساختاری CUSUM (لبه‌ی اثبات‌شده‌ی S851)،
پس‌لرزه‌های اصلاحی (پولبک f·ATR از اکسترمم پس-رویداد) درون پنجره‌ی W بار،
نقاط ورودِ هم‌جهتِ شکست‌اند: n بیشتر (درمان سقف شیشه‌ای) + ورود موضعاً
ضد-دریفت (پادزهر H10) + قیمت ورود بهتر (هزینه در R کوچک‌تر).

تمایز از S793: شرط‌گذار رویداد نادر CUSUM است، نه رژیم درفت دائمی.
درس‌های S850/S851: سقف null=۱M، سقف uncond=50k، آزادسازی حافظه، np.bool_.
"""
import sys
import os
import json
...
EXPLORE_MIN_N = 30
SL_FLOOR_PIP = 5.0
OUT = 'results/_scan_S852'


def aftershock_signals(z, close, atr, k_drift, h, W, f):
    """ماشین حالت علّی: رویداد CUSUM ⇒ پنجره‌ی W بار ⇒ پولبک f·ATR(رویداد)
    از اکسترمم پس-رویداد ⇒ سیگنال هم‌جهتِ شکست. یک پولبک = یک سیگنال."""
    n = len(z)
    long_sig = np.zeros(n, dtype=np.bool_)
    short_sig = np.zeros(n, dtype=np.bool_)
    sp = 0.0
    sm = 0.0
    dirn = 0            # 0=خاموش، +1=UP، -1=DOWN
    win_end = -1
    atr_ref = 0.0
    ext = 0.0
    for i in range(n):
        ...
        if ev_dn:
            sm = 0.0
        if ev_up and not ev_dn:
            a = atr[i]
            if a == a and a > 0.0:      # isfinite بدون numpy
                dirn = 1
                win_end = i + W
                atr_ref = a
                ext = close[i]
            ...
            if a == a and a > 0.0:
                ...
                dirn = 0
        elif dirn != 0 and i > win_end:
            dirn = 0
        if dirn == 1 and not ev_up:
            if close[i] > ext:
                ext = close[i]
            elif close[i] <= ext - f * atr_ref:
                long_sig[i] = True
                ext = close[i]          # reset ردیاب — یک پولبک، یک سیگنال
        elif dirn == -1 and not ev_dn:
            if close[i] < ext:
                ext = close[i]
            elif close[i] >= ext + f * atr_ref:
                short_sig[i] = True
...
try:
    ...
    aftershock_signals = njit(cache=True)(aftershock_signals)
except Exception:
    pass
...
def run_tf(tf, verbose=True):
    print(f"\n{'='*90}\n=== S852 Aftershock Pullback :: XAUUSD-{tf} ===",
          flush=True)
    ...
    for h in H_THRESH:
        for W in WINDOWS:
            for f_ in F_PULL:
                ls, ss = aftershock_signals(z, close, atr,
                                            K_DRIFT, h, W, f_)
                ls[:warmup] = False
                ss[:warmup] = False
                sig_cache[(h, W, f_)] = (ls, ss)
                for k in K_SL:
                    for rr in RRS:
                        st = combo_trades(df, ls, ss, atr, k, rr,
                                          warmup, hi_explore, pip)
                        row = dict(h=h, W=W, f=f_, k=k, rr=rr,
                                   n=(st['n'] if st else 0),
                        ...
                        grid.append(row)
                        if verbose:
                            print(f"    explore h={h:<4} W={W:<3} f={f_:<4} "
    ...
    win = elig[0]
    h_w, W_w, f_w, k_w, rr_w = win['h'], win['W'], win['f'], win['k'], win['rr']
    print(f"\n    ★ winner (exploration only): h={h_w} W={W_w} f={f_w} "
          f"k={k_w} rr={rr_w}  exp={win['exp']}pip n={win['n']}", flush=True)

    # ---------- ۲) قضاوت: یک بار، کل داده، split_bar ----------
    ls, ss = sig_cache[(h_w, W_w, f_w)]
    ...
    out = dict(tf=tf, src=src, bars=n, split_bar=split,
               ...
               judged=dict(h=h_w, W=W_w, f=f_w, k=k_w, rr=rr_w,
