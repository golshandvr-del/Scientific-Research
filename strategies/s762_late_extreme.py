#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""S762 — «اکسترممِ دیرهنگامِ درون‌کندلی → ادامه» (Late-Extreme Continuation) · XAUUSD

پیش‌ثبت: results/S762_PREREG_LATE_EXTREME_CONTINUATION.md (commit 04f4225b — پیش از هر آزمون)
مسیرِ چندگانگی: C (hold-out) · SPLIT_FRAC=0.60 · SEED=20260904 · K_PERM=2000

τ_hi[t] = موقعیتِ زمانیِ نسبیِ سقفِ کندلِ درشتِ t درونِ زیرکندل‌های ریز (0=آغاز، 1=پایان)؛ τ_lo آینه.
LONG: τ_hi≥τ* ∧ close>open ∧ range≥1×ATR89[t−1] (+ گیتِ درفتِ اختیاری)؛ SHORT آینه. follow.
سیگنال روی t ⇒ ورود openِ t+1 (قراردادِ موتور، forward-safe).

اجرا:
  python3 strategies/s762_late_extreme.py search H8
  python3 strategies/s762_late_extreme.py holdout H8   # فقط پس از انجمادِ کامیت‌شده
"""
import json
import os
import sys
...
# ---- خانوادهٔ منجمدِ پیش‌ثبت: 5 TF × 2 τ* × 2 gate × 2 mode = 40 ----
TFS = ['H4', 'H6', 'H8', 'H12', 'D1']
TAUS = [0.75, 0.875]
GATES = ['none', 'drift']
MODES = ['cont', 'rev']
N_FAMILY = len(TFS) * len(TAUS) * len(GATES) * len(MODES)   # = 40

ATR_P = 89
K_RANGE = 1.0      # range ≥ 1.0×ATR89[t−1]
K_SL = 1.272       # SL = 1.272×ATR89[t]
...
def tau_per_bar(coarse_t, sub_t, sub_h, sub_l):
    """τ_hi, τ_lo, cnt برای هر کندلِ درشت از زیرکندل‌ها (اولین رخداد در تساوی)."""
    idx = np.searchsorted(coarse_t, sub_t, side='right') - 1
    ok = idx >= 0
    idx, sub_h, sub_l = idx[ok], sub_h[ok], sub_l[ok]
    n = len(coarse_t)
    cnt = np.bincount(idx, minlength=n).astype(float)
    # ترتیبِ زیرکندل درونِ میزبان: موقعیت − شروعِ گروه
    starts = np.r_[0, np.cumsum(cnt)[:-1]].astype(np.int64)
    pos = np.arange(len(idx)) - starts[idx]
    tau_hi = np.full(n, np.nan)
    tau_lo = np.full(n, np.nan)
    # argmax پایدار در هر گروه: مرتب‌سازی بر (idx, -h, pos)
    order = np.lexsort((pos, -sub_h, idx))
    first = np.r_[True, idx[order][1:] != idx[order][:-1]]
    g = idx[order][first]
    tau_hi[g] = pos[order][first] / np.maximum(cnt[g] - 1, 1)
    order = np.lexsort((pos, sub_l, idx))
    first = np.r_[True, idx[order][1:] != idx[order][:-1]]
    g = idx[order][first]
    tau_lo[g] = pos[order][first] / np.maximum(cnt[g] - 1, 1)
    return tau_hi, tau_lo, cnt
...
def load_card(tf: str):
    ...
    tau_hi, tau_lo, cnt = tau_per_bar(t, sub['time'].astype(np.int64),
                                      sub['high'].astype(np.float64),
                                      sub['low'].astype(np.float64))
    return df, t, tau_hi, tau_lo, cnt, d['src'], sub['src']


def prepare(tf: str):
    df, t, tau_hi, tau_lo, cnt, src, sub_src = load_card(tf)
    ...
    n = len(df)
    valid_mask = (~np.isnan(atr)) & (np.arange(n) >= ATR_P) & \
                 (cnt >= EXP_SUB[tf] * 0.5) & (~np.isnan(tau_hi))
    quality = rng_pip >= K_RANGE * np.nan_to_num(atr_prev, nan=np.inf)
    c, o = df['close'].values, df['open'].values
    k = DRIFT_K[tf]
    c_lag = np.r_[np.full(k, np.nan), c[:-k]]
    drift_up = c > c_lag
    drift_dn = c < c_lag
    feats = dict(tau_hi=tau_hi, tau_lo=tau_lo, quality=quality,
                 bull=c > o, bear=c < o,
...
def signals(feats, tau_star, gate, mode, n):
    up = (np.nan_to_num(feats['tau_hi'], nan=-1) >= tau_star) & feats['bull'] & feats['quality']
    dn = (np.nan_to_num(feats['tau_lo'], nan=-1) >= tau_star) & feats['bear'] & feats['quality']
    if gate == 'drift':
        up &= feats['drift_up']
        dn &= feats['drift_dn']
    if mode == 'cont':
...
def scan_search(tf: str):
    ...
    for tau_star in TAUS:
        for gate in GATES:
            for mode in MODES:
                ls, ss = signals(fs, tau_star, gate, mode, split)
                ls &= vm_s
                ss &= vm_s
                ...
                for side in ('long', 'short'):
                    ...
                    row = dict(tau=tau_star, gate=gate, mode=mode, side=side, n=int(n),
                               wr=round(wr, 2), exp_pip=round(expp, 3),
                               lift=round(float(lift), 2), score=round(float(score), 2))
                    rows.append(row)
                    # rev فقط برای ابطال — نامزدِ انجماد نمی‌شود
                    if mode == 'cont' and expp > 0 and lift > 0 and (
    ...
    out = dict(tf=tf, src=str(src), sub_src=str(sub_src), n_bars=n_all, split=split,
               ...
               barrier='lift*sqrt(n)>=78 & exp>0 & n>=30 & mode=cont',
...
def judge_holdout(tf: str):
    ...
    ls, ss = signals(feats, cfg['tau'], cfg['gate'], 'cont', n_all)
    ls &= valid_mask
    ss &= valid_mask
    if cfg['side'] == 'long':
    ...
    res = {}
    for label, ntr in (('N1_pathC', 1), ('N40_strict', N_FAMILY)):
