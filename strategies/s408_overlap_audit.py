...
# -*- coding: utf-8 -*-
"""
S408 — ممیزی هم‌پوشانی (تعهد مأموریت ۱، فقط‌خواندنی)
======================================================
مقایسهٔ روزهای ورود S408 (M15 فیل، V ثابت، دادهٔ کامل ۱۵.۶ ساله) با:
...
خروجی: results/_s408_overlap.json
"""
import sys, os, json
import numpy as np
import pandas as pd
...
from strategies import s408_gap_fill_m15_fulldata as s408

ROOT = os.path.join(os.path.dirname(__file__), '..')


def entry_days(tf, runner, **kw):
    df = s408.load_full() if tf == 'M15' else se.load_data(f'data/XAUUSD_{tf}.csv')
...
def main():
    d408, t408 = entry_days('M15', s408.run_layer, q=60, k_sl=2.0, use_v=True)
    ...
    out = {
        'S408_n_days': len(d408),
        'window_note': 'S404/S562 restricted to S408 data window [first,last] for fair comparison',
        'vs_S404_M30_window': cmp(d408, d404w),
        'vs_S562_M15_rolling_window': cmp(d408, d562w),
        'S408_first': d408[0], 'S408_last': d408[-1],
        'vs_S404_M30': cmp(d408, d404),
        'vs_S562_M15_rolling': cmp(d408, d562),
        'note': 'day-level overlap; S562 times from frozen read-only artifact (Leibniz block untouched)',
    }
    print(json.dumps(out, indent=1, ensure_ascii=False))
    with open(os.path.join(ROOT, 'results/_s408_overlap.json'), 'w') as f:
