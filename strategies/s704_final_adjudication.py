# -*- coding: utf-8 -*-
"""
S704 — داوریِ نهایی (یک آزمون، مسیر C)
سلولِ منجمدشده در جست‌وجو (نیمهٔ اول): XAUUSD-H2 · θ=1.6 · cont · k_sl=1.618
هندسهٔ منجمد: SL = 1.618 × median(ATR55 علّیِ نیمهٔ جست‌وجو) ؛ TP = 1.5×SL
بازسازی روی دادهٔ کامل؛ split_bar = n_full//2 ؛ نول K=1000 دوطرفه؛ n_trials=228.
...
def main():
    ...
    ev_next[1:] = ev[:-1]
    ls = pd.Series(ev_next == 1, index=df.index)   # cont: هم‌جهت دریفت
    ...
    null_path = os.path.join(OUT, 'final_null_H2.json')
    ...
    line = (f"S704-H2 OFFICIAL | {res['verdict']} RQS2={res['rqs2_score']:.1f} | "
