# -*- coding: utf-8 -*-
"""
s456_mfe_roundtrip.py — S456 · خروجِ «رفت‌وبرگشتِ سود» (MFE Round-Trip)
================================================================================
پیش‌ثبت: results/S456_PREREG_ADDENDUM_MFE_ROUNDTRIP_EXIT.md (کامیت قبل از این اجرا).

قاعده (قفل‌شده): در close هر کندلِ i بعد از ورود، mfe_i = بیشینهٔ سودِ شناورِ close
تا کنون. اگر mfe_i >= θ×TP_dist و سودِ شناورِ فعلی <= 0 ⇒ خروج (ts/se: open کندلِ
بعد پس از تقدمِ SL/TP/max_hold؛ S382: close همان کندل). معاملهٔ در سود لمس نمی‌شود.
واریانت‌ها: θ ∈ {0.25, 0.50, 0.75}.

بیماران و قراردادهای بازپخش عینِ S453/S454/S455 (بیت‌به‌بیت).
توازی اجباری (vn=None ⇒ pnl پایه، tol 1e-6). مسیرِ C: کالیبراسیون نیمهٔ اول؛
برنده = بهترین بهبودِ maxDD میانِ پاس‌شدگان؛ هولد‌اوت یک بار.

اجرا: python3 strategies/s456_mfe_roundtrip.py <patient>
"""
import os
import sys
import json
...
VARIANTS = ('T025', 'T050', 'T075')  # قفل‌شده در پیش‌ثبت
THETA = {'T025': 0.25, 'T050': 0.50, 'T075': 0.75}


def symbols(df):
    """S456 به الفبای نمادین نیاز ندارد؛ امضا برای سازگاری با قالب نگه داشته شد."""
    return None


def rule_step(vn, sym, i, is_long, losing, state, fl=None, tp_dist=None):
    """ارزیابی در close کندلِ i. state = mfe (بیشینهٔ سودِ شناورِ close تا کنون).

    خروجی: (exit_now, state جدید). fl = سودِ شناورِ جهت‌دار در close کندلِ i."""
    if vn is None:
        return False, state
    mfe = fl if state is None else max(state, fl)
    if mfe >= THETA[vn] * tp_dist and losing:
        return True, mfe
    return False, mfe
...
def replay_ts(trade, arr, sym, vn, max_hold, spec):
    ...
    while i < n - 1:
        ...
        if vn is not None and i > e:
            ...
            if ex:
                return (o[nb] - entry - cost) * contract, nb, 'mfe_exit'
        i += 1
...
def replay_se(trade, arr, sym, vn, max_hold, cfg, is_long, tp_pip):
    ...
    for j in range(eb, end):
        ...
        if pend_exit:
            ...
            g = (xf - fill) if is_long else (fill - xf)
            return g / pip - spread, j, 'mfe_exit'
        if j == eb:
...
def replay_s382(trade, arr, sym, vn, ps, sl_abs, rr):
    ...
    while j < n:
        ...
        if vn is not None:
            ...
            if ex:
                return (c[j] - entry) / ps, j, 'mfe_exit'
        j += 1
...
def _save(name, out):
    ...
    p = os.path.join(ROOT, 'research', 'mgmt', f'S456_{name}.json')
    with open(p, 'w') as f:
