#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""S765 — «عدمِ تعادلِ جریانِ سفارشِ زیرکندلی در شوک» (Sub-bar Order-Flow Imbalance Shock) · XAUUSD

پیش‌ثبت: results/S765_PREREG_SUBBAR_ORDERFLOW_IMBALANCE.md (commit see git — پیش از هر آزمون)
مسیرِ چندگانگی: C (hold-out) · SPLIT_FRAC=0.60 · SEED=20260905 · K_PERM=2000

OFI[t] = Σ_j sgn(c_j−o_j)·vol_j / Σ_j vol_j روی زیرکندل‌های M5 (D1: H1) ∈ [−1,+1].
شوک: range ≥ 1.618×ATR21[t−1]. LONG: شوک ∧ close>open ∧ OFI≥θ؛ SHORT: شوک ∧ close<open ∧ OFI≤−θ. follow. θ=0 = کنترل. سیگنال روی t ⇒ ورود openِ t+1.

اجرا:
  python3 strategies/s765_subbar_ofi.py search H8
  python3 strategies/s765_subbar_ofi.py holdout H8   # فقط پس از انجمادِ کامیت‌شده
"""
import json
import os
import sys
...
# ---- خانوادهٔ منجمدِ پیش‌ثبت: 5 TF × 3 θ (0=کنترل) × 2 gate = 30 ----
TFS = ['H4', 'H6', 'H8', 'H12', 'D1']
ESTARS = [0.0, 0.3, 0.5]   # θ (OFI)
GATES = ['none', 'drift']
...
def ofi_per_bar(coarse_t, sub_t, sub_o, sub_c, sub_v):
    """OFI = Σ sgn(c−o)·vol / Σ vol برای هر کندلِ درشت (قاعدهٔ تیکِ زیرکندلی)؛ cnt = شمارِ زیرکندل."""
    n = len(coarse_t)
    idx = np.searchsorted(coarse_t, sub_t, side='right') - 1
    ok = idx >= 0
    idx, o, c, v = idx[ok], sub_o[ok], sub_c[ok], sub_v[ok].astype(np.float64)
    sgn = np.sign(c - o)
    num = np.zeros(n); den = np.zeros(n)
    np.add.at(num, idx, sgn * v)
    np.add.at(den, idx, v)
    cnt = np.bincount(idx, minlength=n).astype(float)
    with np.errstate(divide='ignore', invalid='ignore'):
        ofi = np.where(den > 0, num / den, np.nan)
    return ofi, den, cnt
...
def load_card(tf: str):
    ...
    ofi, vsum, cnt = ofi_per_bar(t, sub['time'].astype(np.int64),
                                 sub['open'].astype(np.float64),
                                 sub['close'].astype(np.float64),
                                 sub['volume'])
    return df, t, ofi, vsum, cnt, d['src'], sub['src']


def prepare(tf: str):
    df, t, ofi, vsum, cnt, src, sub_src = load_card(tf)
...
def signals(feats, e_star, gate, n):
    up = feats['quality'] & feats['bull'] & (feats['ofi'] >= e_star)
    dn = feats['quality'] & feats['bear'] & (feats['ofi'] <= -e_star)
    if gate == 'drift':
        up &= feats['drift_up']
        dn &= feats['drift_dn']
    return up, dn
...
def scan_search(tf: str):
    ...
    # P4: همبستگیِ اسپیرمنِ |OFI| و ρ روی شوک‌های نیمهٔ جستجو (افزونگی با S965)
    sh = fs['quality'] & vm_s
    a, b = np.abs(fs['ofi'][sh]), fs['rho'][sh]
    ra, rb = pd.Series(a).rank().values, pd.Series(b).rank().values
    p4_spearman = float(np.corrcoef(ra, rb)[0, 1]) if len(a) > 10 else None
    ...
    out = dict(tf=tf, src=str(src), sub_src=str(sub_src), n_bars=n_all, split=split,
               ...
               jaccard_vs_s965=jacc, p4_spearman_absOFI_rho=p4_spearman,
               barrier='lift*sqrt(n)>=78 & exp>0 & n>=30 & theta>0 & P1(beats control)',
    ...
    print(f"[S765 search {tf}] src={os.path.basename(str(src))} bars={n_all} "
...
def judge_holdout(tf: str):
    ...
    res = {}
    for label, ntr in (('N1_pathC', 1), ('N30_strict', N_FAMILY)):
