# -*- coding: utf-8 -*-
"""S1515 — سقفِ تازهٔ ۹۰ (S526 منجمد، بدون ρ) × رژیمِ σ (S605، W=233) ⇒ LONG | H4/H6 × {storm,calm}.

پیش‌ثبت: `results/S1515_PREREG_FRESH_HIGH_STORM_SUBH8.md` (کامیت f4ca0fa4 — **قبل** از هر عدد).
استفاده: python3 strategies/s1515_high_storm.py {calm|storm} [CARDS...]
رویداد: S526 (close>max close90 fresh, drift>0) & regime(arm)؛ نول مشروط در فضای drift>0 ∧ regime(arm).
...
n_trials=8. SEED=20260909, K=2000.
تشخیصی: پایهٔ S526 خام روی همان کارت (P2 مرجع).
"""
...
def regime_mask(df, arm):
    S6 = _mod('strategies/s605_engle_sigma_regime.py', '_s605')
    s = S6.sigma_series(df['close'].values)
    reg = S6.regime_ratio(s, W_CALM)
    ok = np.isfinite(reg)
    sel = ok & (reg <= 1.0) if arm == 'calm' else ok & (reg > 1.0)
    return pd.Series(sel, index=df.index)


def space_mask(df):  # فضای نول: drift>0 ∧ regime(ARM)
    return drift_mask(df) & regime_mask(df, ARM)
...
def sig_base(df):  # S526 عیناً (بدون ρ)
    c = df['close']
    nh = _b(c > c.rolling(LOOKBACK).max().shift(1))
    return nh & ~nh.shift(1, fill_value=False) & drift_mask(df)


def sig_fresh(df):  # S1515: پایه × رژیم
    return sig_base(df) & regime_mask(df, ARM)
...
def main():
    ...
    global ARM
    args = sys.argv[1:]
    if args and args[0] in ('calm', 'storm'):
        ARM = args[0]; args = args[1:]
    cards = args or CARDS
    print(f'S1515 fresh-high(S526 raw) x sigma-regime[{ARM}] W={W_CALM} | null in drift>0 & regime | '
    ...
    for card in cards:
        try:
            ...
            r['p2_base_s526'] = diag(df, sig_base, sl_abs, ps, be)
            r['arm'] = ARM
            r['n_event_base'] = int(sig_base(df).sum())
            r['n_event_arm'] = int(sig_fresh(df).sum())
            r['pass_rate_pct'] = round(100.0 * r['n_event_arm'] / max(1, r['n_event_base']), 1)
        except Exception as e:
            print(f'{card}: ERROR {e}', flush=True)
            continue
        r['lookback'] = LOOKBACK
        with open(f'{OUT}/{card}_{ARM}.json', 'w') as f:
        ...
        print(f'{card}: span={r.get("span_years")}y n={r.get("n_trades")} '
              ...
              f'arm={ARM} base_S526={r.get("p2_base_s526")} pass={r.get("pass_rate_pct")}%',
              flush=True)
