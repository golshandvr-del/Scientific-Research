# -*- coding: utf-8 -*-
"""
S646 — ForceShock — داوریِ نهاییِ holdout (مسیرِ C — فقط یک بار اجرا)
=======================================================================
پیش‌ثبت: results/S646_PREREG_FORCE_SHOCK_XAUUSD_PATHC.md (commit 0af8b61d)
منطق عیناً جدولِ §۲ پیش‌ثبت — صفر درجهٔ آزادیِ باز:

  • کارت‌ها: {H3, H4, H6, H8, H12}
  • لانگ: |vol×Δclose| ≥ 6×median(|f|, 100 کندلِ قبلی) و Δclose>0
  • شورت: آینه — Δclose<0
...
def run_card(tf, rng):
    ...
    dc = np.diff(cv, prepend=cv[0])
    f = vol * dc
    med = pd.Series(np.abs(f)).rolling(100).median().shift(1).values   # فقط گذشته
    with np.errstate(invalid='ignore', divide='ignore'):
        ratio = np.abs(f) / med
    ratio = np.nan_to_num(ratio, nan=0.0, posinf=0.0)
    shock = ratio >= 6.0
    bull = shock & (dc > 0)      # شوکِ نیرویِ خرید → لانگ
    bear = shock & (dc < 0)      # شوکِ نیرویِ فروش → شورت
...
def main():
    ...
    print('== S646 — داوریِ نهاییِ holdout · Force shock K6 · {H3,H4,H6,H8,H12} · '
    ...
    if res is None:
        print('\n[S646] هیچ عضوِ معتبری نماند — استخر تهی. حکم: مرگ در H0.',
              flush=True)
    ...
    print('\n' + R.format_rqs2('S646-FORCE-POOL', r), flush=True)

    out = dict(prereg='S646_PREREG_FORCE_SHOCK_XAUUSD_PATHC.md@0af8b61d',
