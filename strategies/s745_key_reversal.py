...
# -*- coding: utf-8 -*-
"""
S745 — «برگشتِ کلیدی در کرانهٔ N-کندلی» (Key Reversal at Extreme) — XAUUSD
================================================================================
پیش‌ثبت: results/S745_PREREG_KEY_REVERSAL_EXTREME_PATH_C.md (کامیت پیش از این کد)
رویداد: کفِ/سقفِ جدیدِ LOOK-کندلی + کندلِ بیرونی + بستهٔ برگشتی.
شبکه: LOOK∈{21,55} × K_SL∈{1.5,2.5} × RR∈{1.0,1.5} = ۸ · n_trials=8 · مسیر C
اجرا: python3 strategies/s745_key_reversal.py <TF> [--kperm 500]
"""
import sys
import os
import json
...
ASSET = 'XAUUSD'
N_TRIALS = 8                  # خانوادهٔ جدید — 2·2·2
LOOK_GRID = (21, 55)
K_SL_GRID = (1.5, 2.5)
RR_GRID = (1.0, 1.5)
ATR_WIN = 100
...
MIN_PF_DISC = 1.3
SPLIT_FRAC = 0.60
M5_VERDICT_CAP = 'UNPROVEN'   # قرنطینهٔ موروثیِ S740
W1_VERDICT_CAP = 'UNPROVEN'   # holdout W1 در S743 مصرف شده (احتیاطِ حداکثری)

OUT_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                       'results', '_scan_S745')


def _roll_min_prev(a, win):
    """کمینهٔ رولینگِ win کندلِ *پیشین* (بدونِ کندلِ جاری)."""
    import pandas as pd
    s = pd.Series(a)
    return s.rolling(win).min().shift(1).values


def _roll_max_prev(a, win):
    import pandas as pd
    s = pd.Series(a)
    return s.rolling(win).max().shift(1).values


def build_signals(df, atr, look):
    """برگشتِ کلیدی در کرانه. خروجی (sig_idx, is_long)."""
    h = df['high'].values
    l = df['low'].values
    c = df['close'].values
    ...
    lo_prev = _roll_min_prev(l, look)
    hi_prev = _roll_max_prev(h, look)

    valid = (np.isfinite(atr) & (atr > 0) & np.isfinite(c1)
             & np.isfinite(lo_prev) & np.isfinite(hi_prev))
    outside = (h > h1) & (l < l1)

    long_c = valid & outside & (l < lo_prev) & (c > c1)
    short_c = valid & outside & (h > hi_prev) & (c < c1) & ~long_c
...
def discover(df, atr, split, c_pip):
    ...
    df_d = df.iloc[:split]
    atr_d = atr[:split]
    for look in LOOK_GRID:
        sig, is_long = build_signals(df_d, atr_d, look)
        for k_sl in K_SL_GRID:
            for rr in RR_GRID:
                row = dict(look=look, k_sl=k_sl, rr=rr,
...
def run_tf(tf, k_perm=500, seed=745):
    t0 = _time.time()
    ...
    if winner is None:
        ...
        print(f"    هیچ نامزدی واجد نشد (eligible=0 از {len(grid)}).",
              flush=True)
        return out
    out['winner_params'] = {k: winner[k] for k in ('look', 'k_sl', 'rr')}
    ...
    w = winner
    sig, is_long = build_signals(df, atr, w['look'])
    ...
    r = rqs2.compute_rqs2(tr, ASSET, sl_pip=sl_med, tp_pip=tp_med,
                          ...
                          allow_overlap=False)
    print('\n' + rqs2.format_rqs2(f'S745-{tf}', r), flush=True)

    verdict = r['verdict']
    if tf in ('M5', 'W1') and verdict == 'ACCEPT':
        out['verdict_engine'] = verdict
        verdict = M5_VERDICT_CAP if tf == 'M5' else W1_VERDICT_CAP
