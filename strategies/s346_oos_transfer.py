...
# فیلترهای **پیش‌ثبت‌شدهٔ** C1 — عیناً، آستانه‌ها بر حسبِ مقیاسِ D1
C1_FILTERS = [
    dict(col='cg_fib_13',  dir='ge', thr=-0.05637353658676148),
    dict(col='std_fib_55', dir='le', thr=0.8881055116653442),
]
WARMUP_REF = 250          # همان کفِ warmupِ خانواده


def _ind(df, name):
    """محاسبهٔ یک ستونِ اندیکاتور — بی‌نیاز از ساختِ کلِ بانکِ ۴۰۱."""
    return ib.compute(name, df).values.astype('float64')
