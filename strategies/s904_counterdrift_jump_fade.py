"""
S904 — Counter-Drift Jump Fade · XAUUSD · RQS2 v2.6 · Path C
==============================================================
پرشِ BV-سنجیده که **خلاف** درفت ۸۹کندله شلیک شده → فید به سمتِ درفت.
جمعیتِ مکملِ S950 (که پرشِ هم‌راستا را ادامه می‌دهد) — هم‌پوشانی صفر.

پیش‌ثبت: results/S904_PREREGISTRATION.md (کامیت f7313a11، قبل از هر آزمون).
گرید منجمد: k∈{2.6,3.5} × k_sl∈{1.5,2.058} × RR∈{1.0,1.5} × hold∈{13,34} ⇒ N_eff=16.
"""
...
ASSET = 'XAUUSD'
OUT = os.path.join(ROOT, 'results', '_s904')
...
GRID_HOLD = (13, 34)
N_EFF = 16
assert len(GRID_JK)*len(GRID_KSL)*len(GRID_RR)*len(GRID_HOLD) == N_EFF

W = 89          # پنجرهٔ BV/درفت/ATR — ثابت، خارج از گرید (پیش‌ثبت §۲)
WARMUP = 91
SPLIT_FRAC = 0.60
K_PERM = 1000
SEED = 904
...
def build_features(d):
    """σ_BV (jump-robust, causal تا t−1) و درفت W-کندله (causal)."""
    c = d['close']
    r = np.concatenate(([0.0], np.diff(c)))
    absr = np.abs(r)
    bp = absr * np.concatenate(([0.0], absr[:-1]))          # |r_i|·|r_{i−1}|
    # میانگین W-تایی تا i، بعد shift(1) ⇒ فقط تا t−1 (causal)
    bv_mean = pd.Series(bp).rolling(W).mean().shift(1).values
    sigma_bv = np.sqrt((np.pi / 2.0) * bv_mean)
    drift = np.full(len(c), np.nan)
    drift[W + 1:] = c[W:-1] - c[:-W - 1]                      # close[t−1]−close[t−1−W]
    return r, sigma_bv, drift


def build_signals(d, jk, r, sigma_bv, drift):
    """LONG: پرش پایین در درفت صعودی · SHORT: پرش بالا در درفت نزولی."""
    with np.errstate(invalid='ignore'):
        jump_dn = r < -jk * sigma_bv
        jump_up = r > +jk * sigma_bv
        ls = jump_dn & (drift > 0)
        ss = jump_up & (drift < 0)
...
def phase_discover(tf):
    ...
    atr = atr_pip(d1)
    r, sbv, drift = build_features(d1)

    t0 = time.time()
    results = dict(done)
    i = 0
    for jk in GRID_JK:
        ls, ss = build_signals(d1, jk, r, sbv, drift)
        n_sig = int(ls.sum() + ss.sum())
        for k_sl in GRID_KSL:
            for rr in GRID_RR:
                for hold in GRID_HOLD:
                    i += 1
                    key = f'j{jk}_k{k_sl}_rr{rr}_h{hold}'
                    if key in results:
                        continue
...
def phase_final(tf):
    ...
    print(f"FINAL S904 {tf} · locked={locked['best_key']} · "
          f"n_trials={N_EFF} · split_bar={locked['split_bar']}")
    d = fd.load_fast(ASSET, tf)
    assert d['src'] == locked['src'], 'data source changed since lock!'
    atr = atr_pip(d)
    r, sbv, drift = build_features(d)
    ls, ss = build_signals(d, p['jk'], r, sbv, drift)
