# -*- coding: utf-8 -*-
"""S918 — «اشتعال روح حیوانی»: رکورد دامنهٔ ۳۴باره + بدنه + هم‌جهتی دریفت · XAUUSD · Long+Short

پیش‌ثبت: results/S918_PREREG_animal_spirits_ignition.md (کامیت a5f83086)
مسیر C: کارت اصلی H2، جست‌وجو فقط در ۶۰٪ نخست، n_trials=3 (قالب بار سوم در بلوک).

سیگنال (منجمد — محور قیمتی، نه حجمی):
  شوک دامنه: (high[t]-low[t]) > max(rng[t-34..t-1])
...
اجرا:
  python3 strategies/s918_animal_spirits.py --phase train --tf H2
  python3 strategies/s918_animal_spirits.py --phase holdout --tf H2 --n_trials 3
  python3 strategies/s918_animal_spirits.py --phase family
"""
...
RNG_LB = 34                      # منجمد — فیبوناچی: رکورد دامنه (پیش‌ثبت §۱)
...
CKPT_DIR = os.path.join(ROOT, 'results', '_s918_ckpt')
...
def wave_signals(df: pd.DataFrame):
    """اشتعال روح حیوانی دوطرفه — رکورد دامنه، آناتومیک محض؛ ورود کندل بعد."""
    o = df['open'].to_numpy()
    c = df['close'].to_numpy()
    rng = (df['high'] - df['low']).to_numpy(dtype=float)
    n = len(c)
    rmax_prev = pd.Series(rng).shift(1).rolling(RNG_LB).max().to_numpy()
    ev_l = np.zeros(n, bool)
    ev_s = np.zeros(n, bool)
    t = np.arange(RNG_LB + TREND_LB + 1, n)
    vshock = rng[t] > rmax_prev[t]
    d = c[t] - c[t - TREND_LB]
...
def phase_train(tf: str):
    ...
    out = {'tf': tf, 'split_bar': split, 'n_train': len(df_tr),
           ...
           'trend_lb': TREND_LB, 'rng_lb': RNG_LB, 'max_hold': MAX_HOLD}
