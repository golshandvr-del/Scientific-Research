# -*- coding: utf-8 -*-
"""
S784 — آزمون تأییدی یگانه روی hold-out (مسیر C)
================================================
پیکربندی منجمد از پیش‌ثبت (results/S784_PREREG_mfi_saturation_h1.md،
commit cd7e002a) — هیچ پارامتری قابل تغییر نیست.

MFI فرین → تداوم: عبور به بالای 82.7 → long؛ عبور به پایین 17.3 → short.
XAUUSD-H1، SL=2.23×ATRmed(34)، TP=2.68×ATRmed(34)، max_hold=72، تک‌پوزیشن.
hold-out: time >= 1661990400 (2022-09-01) — نخستین و تنها لمس برای این فرضیه.
"""
...
# ── منجمد (از PREREG cd7e002a) ──
ASSET, TF = 'XAUUSD', 'H1'
THR_LO, THR_HI = 17.3, 82.7
SL_ATR, TP_ATR, HOLD_HOURS = 2.23, 2.68, 72
HOLDOUT_EPOCH = 1_661_990_400
N_TRIALS, SEED, K = 1, 784, 1000
...
def main():
    ...
    p = np.roll(mfi, 1); p[0] = np.nan
    up = (p < THR_HI) & (mfi >= THR_HI) & np.isfinite(p)   # اشباع خرید → long
    dn = (p > THR_LO) & (mfi <= THR_LO) & np.isfinite(p)   # اشباع فروش → short
    ...
    if os.path.exists(nm_path):
        ...
        for side, n_sig in (('long', n_sig_long), ('short', n_sig_short)):
            ...
            pb = perm_baseline(df, side, n_sig, sl, tp, mh, K, SEED)
            print(f'  perm[{side}]: mean={pb["mean"]:.2f} sd={pb["sd"]:.2f} '
                  f'max={pb["max"]:.2f} k={pb["k"]}', flush=True)
            null[side] = dict(uncond_wr=u, perm_mean=pb['mean'], perm_sd=pb['sd'],
                              perm_max=pb['max'], perm_k=pb['k'])
            detail[side] = dict(uncond=rows, perm=pb)
    ...
    print()
    print(R.format_rqs2('S784_MfiSaturation_XAUUSD_H1', res))
