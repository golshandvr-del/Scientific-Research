...
# -*- coding: utf-8 -*-
"""
S743 — «بازگشت از سطوحِ رند» (Round-Level Rejection) — XAUUSD
================================================================================
پیش‌ثبت: results/S743_PREREG_ROUND_LEVEL_REJECTION_PATH_C.md (پیش از این کد)
فرضیه: لمسِ سطحِ رند (k·STEP دلار) + بسته‌شدنِ ردکننده ⇒ ادامهٔ بازگشت
(Aggarwal & Lucey 2007: psychological barriers in gold). مسیر C، n_trials=24.
اجرا: python3 strategies/s743_round_level.py <TF> [--kperm 500]
"""
import sys
import os
import json
...
ASSET = 'XAUUSD'
N_TRIALS = 24                 # خانوادهٔ جدید — 3·2·2·2
STEP_GRID = (25.0, 50.0, 100.0)
REJ_K_GRID = (0.25, 0.5)
K_SL_GRID = (1.5, 2.5)
RR_GRID = (1.0, 1.5)
ATR_WIN = 100
...
MIN_PF_DISC = 1.3
SPLIT_FRAC = 0.60
M5_VERDICT_CAP = 'UNPROVEN'   # قرنطینهٔ موروثیِ holdout M5 (S740)

OUT_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                       'results', '_scan_S743')


def build_signals(df, atr, step, rej_k):
    """سیگنالِ ردِ سطحِ رند، رویدادمحور. خروجی (sig_idx, is_long)."""
    h = df['high'].values
    l = df['low'].values
    c = df['close'].values

    prev_c = np.concatenate(([np.nan], c[:-1]))
    valid = np.isfinite(atr) & (atr > 0) & np.isfinite(prev_c)

    # نزدیک‌ترین سطحِ رندِ درونِ بازهٔ کندل (اگر باشد).
    # سطحِ نامزدِ حمایت: بزرگ‌ترین k·step ≤ high؛ باید ≥ low باشد تا لمس شده باشد.
    lvl_dn = np.floor(h / step) * step          # بالاترین سطحِ ≤ high
    touch = (lvl_dn >= l) & (lvl_dn <= h)

    long_c = (valid & touch
              & (prev_c > lvl_dn)                       # رویکرد از بالا
              & (c >= lvl_dn + rej_k * atr))            # بسته‌شدنِ ردکننده

    lvl_up = np.ceil(l / step) * step           # پایین‌ترین سطحِ ≥ low
    touch_u = (lvl_up >= l) & (lvl_up <= h)
    short_c = (valid & touch_u
               & (prev_c < lvl_up)
               & (c <= lvl_up - rej_k * atr) & ~long_c)
...
def discover(df, atr, split, c_pip):
    """جاروبِ ۲۴ پیکربندی فقط روی پنجرهٔ اکتشاف."""
    rows = []
    df_d = df.iloc[:split]
    atr_d = atr[:split]
    for step in STEP_GRID:
        for rej_k in REJ_K_GRID:
            sig, is_long = build_signals(df_d, atr_d, step, rej_k)
            for k_sl in K_SL_GRID:
                for rr in RR_GRID:
                    row = dict(step=step, rej_k=rej_k, k_sl=k_sl, rr=rr,
...
def run_tf(tf, k_perm=500, seed=743):
    t0 = _time.time()
    ...
    w = winner
    sig, is_long = build_signals(df, atr, w['step'], w['rej_k'])
    ...
    r = rqs2.compute_rqs2(tr, ASSET, sl_pip=sl_med, tp_pip=tp_med,
                          ...
                          allow_overlap=False)
    print('\n' + rqs2.format_rqs2(f'S743-{tf}', r), flush=True)

    verdict = r['verdict']
    if tf == 'M5' and verdict == 'ACCEPT':
        out['verdict_engine'] = verdict
        verdict = M5_VERDICT_CAP
        print(f"    ⚠ قرنطینهٔ پیش‌ثبت: M5 از ACCEPT به {verdict} کاسته شد.",
              flush=True)
    out['verdict'] = verdict
