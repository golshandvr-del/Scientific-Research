# -*- coding: utf-8 -*-
"""
S644 — KumoBreakTK — داوریِ نهاییِ holdout (مسیرِ C — فقط یک بار اجرا)
=======================================================================
پیش‌ثبت: results/S644_PREREG_KUMO_BREAK_XAUUSD_PATHC.md (commit 9f89da7e)
منطق عیناً جدولِ §۲ پیش‌ثبت — صفر درجهٔ آزادیِ باز:

  • کارت‌ها: {H3, H4, H6, H8, H12}
  • لانگ: close از سقفِ ابر (9/26/52) خارج شود + Tenkan>Kijun
  • شورت: آینه — خروج از کفِ ابر + Tenkan<Kijun
...
def run_card(tf, rng):
    ...
    tenkan = ((hh.rolling(9).max() + ll.rolling(9).min()) / 2)
    kijun  = ((hh.rolling(26).max() + ll.rolling(26).min()) / 2)
    spanA  = ((tenkan + kijun) / 2).shift(26)
    spanB  = ((hh.rolling(52).max() + ll.rolling(52).min()) / 2).shift(26)
    top = np.maximum(spanA, spanB).values; bot = np.minimum(spanA, spanB).values
    cv = cc.values; pcv = np.roll(cv, 1); pcv[0] = cv[0]
    ptop = np.roll(top, 1); ptop[0] = top[0]; pbot = np.roll(bot, 1); pbot[0] = bot[0]
    tk = tenkan.values; kj = kijun.values
    with np.errstate(invalid='ignore'):
        bull = np.nan_to_num((pcv <= ptop) & (cv > top) & (tk > kj), nan=False).astype(bool)
        bear = np.nan_to_num((pcv >= pbot) & (cv < bot) & (tk < kj), nan=False).astype(bool)
...
def main():
    ...
    print('== S644 — داوریِ نهاییِ holdout · Kumo-break+TK · {H3,H4,H6,H8,H12} · '
    ...
    if res is None:
        print('\n[S644] هیچ عضوِ معتبری نماند — استخر تهی. حکم: مرگ در H0.',
              flush=True)
    ...
    print('\n' + R.format_rqs2('S644-KUMO-POOL', r), flush=True)

    out = dict(prereg='S644_PREREG_KUMO_BREAK_XAUUSD_PATHC.md@9f89da7e',
