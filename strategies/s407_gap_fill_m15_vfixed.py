...
# -*- coding: utf-8 -*-
"""
S407 — گپ‌فیل M15 با فیلتر نوسان علّی ثابت در همهٔ بازوها · XAUUSD M15
==============================================================================
پیش‌ثبت: results/S407_PREREG_GAP_FILL_M15_VFIXED.md (commit قبل از هر اجرا)
والد: S406 (REJECT 29.5 — V-غایب، DD منفجر) · S404 (ACCEPT 96.8) · قانون DD-انتقال (۵ نقطه)
...
حالت‌ها:
  python3 strategies/s407_gap_fill_m15_vfixed.py tune
  python3 strategies/s407_gap_fill_m15_vfixed.py verdict <q> <k_sl>
"""
import sys, os, json
import numpy as np
import pandas as pd
...
TF = 'M15'
SEED = 407
N_TRIALS_CUM = 280                 # 274 + 6 (پیش‌ثبت §۵)
CKPT = os.path.join(os.path.dirname(__file__), '..', 'results', '_s407_tune_ckpt.json')


def run_layer(df, days, atr, q, k_sl, use_v, vflags=None,
              lo_bar=None, hi_bar=None, no_thresh=False, no_dow=False):
    """S407 روی M15 (V ثابت در تیون؛ در null حذف می‌شود). آستانه/DOW/V = مهارت (در null حذف می‌شوند)."""
...
def mode_verdict(q, k_sl, use_v):
    ...
    print(rqs2.format_rqs2(f'S407 {TF} q={q}/k_sl={k_sl}/V=fixed', r), flush=True)
    outp = os.path.join(os.path.dirname(__file__), '..', 'results', '_s407_verdict.json')
