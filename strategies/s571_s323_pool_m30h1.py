# -*- coding: utf-8 -*-
"""
S571 — تجمیعِ S323: استخرِ XAUUSD-M30 + XAUUSD-H1 روی دادهٔ کامل
================================================================================
پیش‌ثبت: `results/S571_PREREG_s323_pool_m30_h1.md` (کامیت 61b1f28a، قبل از این اجرا).

خلاصهٔ عهدها:
  · اعضا ثابت: {M30, H1} با پیکربندیِ منجمدِ S570 — هیچ جست‌وجویی نیست.
  · n_trials = 2801 (ارثِ ۲۸۰۰ + تصمیمِ تجمیع)، تنش = 5602.
  · محورِ مشترک: شبکهٔ ۳۰دقیقه‌ای مصنوعی (BUG-QUANT/BUG-SPAN-safe).
  · نولِ استخر: blend وزنی به سهمِ پس-از-FIFO از نول‌های اندازه‌گیری‌شده (بذر 23؛
    حساسیت با 101 و 777).
  · تقسیم: holdout_mask با صدکِ ۶۰٪ زمانِ ورود (BUG-SPLITDIR-safe).
  · C5′-ب: اگر lift_pool < 10.5pp ⇒ REJECT-STOP حتی با z بالای سد.
  · یک داوری؛ حکم عیناً گزارش می‌شود.
"""
...
N_TRIALS = 2801
N_TRIALS_STRESS = 5602
PERM_K = 2000
SEEDS = (23, 101, 777)
SPLIT_FRAC = 0.60
C5B_MIN_POOL_LIFT = 10.5   # پیش‌ثبت §۳
OUT = 'results/_s571_pool'
MEMBER_CARDS = ['XAUUSD-M30', 'XAUUSD-H1']   # ثابت، از پیش‌ثبت

os.makedirs(OUT, exist_ok=True)


def build_member(card: str) -> dict:
    """بازتولیدِ بیت‌به‌بیتِ معاملاتِ عضو با پیکربندیِ منجمدِ S570."""
    asset, tf = card.split('-')
    ...
    # نولِ اندازه‌گیری‌شدهٔ عضو — همان پروتکلِ S570 (بذر 23 حاکم؛ بقیه حساسیت)
    nulls = {}
    for seed in SEEDS:
    ...
    print(f"[{card}] src={d['src']} bars={len(df)} n={n} WR={wr:.2f} "
          f"null={ref:.2f} lift={lift:+.2f}pp SL={sl} TP={tp} mh={mh}", flush=True)
...
def blend_pool_null(members_used, pool_df, seed):
    """نولِ استخر: ترکیبِ وزنی به سهمِ پس-از-FIFO (دستورکارِ S431)."""
...
def main():
    t0 = time.time()
    print(f"S571 — استخرِ S323 M30+H1 | n_trials={N_TRIALS}/{N_TRIALS_STRESS} "
    ...
    # --- گامِ ۲: pool_cards (هم‌جهتی + همگنی + FIFO تقویمی) ---
    ...
    # WR و lift استخر + قیدِ C5′-ب (پیش‌ثبت §۳)
    ...
    print(f"[استخر] n={len(pool)} WR={wr_pool:.2f} null={null23['long']['uncond_wr']:.2f} "
          f"lift={lift_pool:+.2f}pp", flush=True)
    if lift_pool < C5B_MIN_POOL_LIFT:
        verdict_stop = dict(status='C5B_VIOLATION', lift_pool=round(lift_pool, 2),
        ...
        with open(os.path.join(OUT, 'c5b_violation.json'), 'w', encoding='utf-8') as fh:
            json.dump(verdict_stop, fh, ensure_ascii=False, indent=1)
        print(f"[C5′-ب نقض] lift_pool={lift_pool:.2f} < {C5B_MIN_POOL_LIFT} ⇒ توقف",
              flush=True)
        return

    # --- گامِ ۳: محورِ مشترکِ ۳۰دقیقه‌ای (BUG-QUANT/BUG-SPAN-safe) ---
    ...
    print(f"[محور] ۳۰دقیقه‌ای {axis_dt[0]} → {axis_dt[-1]} · {len(axis_t):,} سطل",
          flush=True)

    # closeِ محور از H1 (کلِ افق) با نگهداشتِ آخرین مقدار — بدونِ آینده‌بینی
    ...
    # --- گامِ ۴: تقسیمِ ۶۰/۴۰ روی صدکِ زمانِ ورود (BUG-SPLITDIR-safe) ---
    te = pool['t_entry'].values.astype(np.int64)
    split_ns = int(np.quantile(te, SPLIT_FRAC))
    holdout = te >= split_ns
    print(f"[تقسیم] مرز={np.datetime64(split_ns, 'ns')} · "
          f"اکتشاف={int((~holdout).sum())} · خارج‌نمونه={int(holdout.sum())}", flush=True)

    # --- گامِ ۵: هندسهٔ وزنی به سهمِ پس-از-FIFO ---
    ...
    print(f"[هندسه] SL={sl_med:.1f} TP={tp_med:.1f} shares={shares}", flush=True)

    # --- گامِ ۶: داوری — بذرِ حاکم 23، حساسیت 101/777 ---
    out_all = {}
    for seed in SEEDS:
        ...
        for label, nt in (('honest', N_TRIALS), ('stress', N_TRIALS_STRESS)):
            ...
            if seed == 23 and label == 'honest':
                print('\n' + R2.format_rqs2('S571-POOL', r), flush=True)
    ...
    rec = dict(session='S571', members=[dict(card=m['card'], n=m['n'],
                                             wr=round(m['wr'], 2),
                                             lift=round(m['lift'], 2),
    ...
    with open(os.path.join(OUT, 'S571_pool_verdict.json'), 'w', encoding='utf-8') as fh:
        json.dump(rec, fh, ensure_ascii=False, indent=1, default=str)
    print(f"\n→ wrote {OUT}/S571_pool_verdict.json  ({rec['elapsed_s']}s)", flush=True)
