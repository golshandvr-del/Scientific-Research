...
# -*- coding: utf-8 -*-
"""
S678 — فاز جست‌وجو (N-bar Overreaction Fade, مسیر B 60/40) (فقط نیمهٔ نخست — مسیر C، PREREG در git قبل از این فایل)
...
معامله در جهتِ مخالفِ شکست. گیتِ درفتِ علّی K=180 (منجمد از S966).
  ...
  * خانواده: N{3,5} × z{2.5,3.5} × gate{none,counter} × k_sl{1,1.5,2} × RR{1,1.5} × side{L,S,both} = 144/TF
  * مبنا: gate=none → کانونیکالِ بی‌قید؛ gate=counter → درفت-شرطی (per side)
...
from strategies.s678_overreact_signals import overreact_signals, drift_masks  # noqa: E402
...
def run_tf(tf):
    ...
    n_full = len(df_full)
    half = int(0.6 * n_full)   # مسیر B: جستجو روی ۶۰٪ اول
    ...
    if 'mt5_full' not in d_src:
        print(f"[S678/{tf}] ⚠️ src={d_src} خارج از mt5_full — E-16 رد شد", flush=True)
        return None
    print(f"[S678/{tf}] src={d_src} n_full={n_full} search_half={n} max_hold={mh}",
          flush=True)
    ...
    pools = {('none', 'long'): np.where(valid)[0],
             ...
             ('counter', 'long'): np.where(valid & drift_up)[0],
             ('counter', 'short'): np.where(valid & drift_dn)[0]}
    uncond = {}
    for (gate, side), pool in pools.items():
        ...
        for k_sl in K_SLS:
            ...
            for rr in RRS:
                ...
                del tr
    gc.collect()
    print(f"[S678/{tf}] مبناها اندازه‌گیری شد ({len(uncond)} سلول، {time.time()-t0:.0f}s)",
          flush=True)

    rows = []
    cfg_i = 0
    for N, z in [(N, z) for N in NS for z in ZS]:
        L_raw, S_raw = overreact_signals(c, N, z)
        L_raw &= valid; S_raw &= valid
        print(f"[S678/{tf}] N={N} z={z} events: long={int(L_raw.sum())} short={int(S_raw.sum())}", flush=True)
        for gate in GATES:
            if gate == 'counter':
                Lg = L_raw & drift_up
                Sg = S_raw & drift_dn
            ...
            for k_sl in K_SLS:
                ...
                for rr in RRS:
                    ...
                    for side in ('long', 'short', 'both'):
                        ...
                        rows.append(dict(
                            N=N, z=z, gate=gate, side=side, k_sl=k_sl, rr=rr,
    ...
    print(f"[S678/{tf}] تمام شد: {cfg_i} پیکربندی، {len(rows)} نتیجه، {dt:.0f}s → {path}",
          flush=True)
    if rows:
        r = {k: v for k, v in rows[0].items() if not k.startswith('yearly')}
        print(f"[S678/{tf}] بهترین: {r}", flush=True)
    return out
