# -*- coding: utf-8 -*-
"""
S845 — شوک استانداردشده با کیفیت درون-کندلی (CLV) به سبک رابرت انگل
=====================================================================
پیش‌ثبت: results/S845_PREREG_engle_shock_quality_clv.md (کامیت 9f80434f پیش از اجرا)
مسیر تعدد: C — جستجو فقط نیمهٔ اول؛ یک آزمون منجمد per-TF روی کل داده.

قاعده: |z_t| ≥ z_thr و CLV هم‌جهت ≥ q ⇒ ورود open کندل t+1 (صفر تأخیر —
درس S844). فقط follow.
"""
import os
import sys
...
N_GRID = len(Z_GRID) * len(Q_GRID) * len(SLK_GRID) * len(RR_GRID)   # = 54


def clv_signals(z, clv_l, clv_s, atr, z_thr, q, warmup, lo=None, hi=None):
    """سیگنال روی کندل شوک t (بسته‌شده)؛ ورود open t+1 توسط queue_frozen."""
    valid = np.isfinite(atr) & (atr > 0) & np.isfinite(z)
    long_m = valid & (z >= z_thr) & (clv_l >= q)
    short_m = valid & (z <= -z_thr) & (clv_s >= q)
...
def discover_is(df, z, clv_l, clv_s, atr, split, warmup, hold, verbose=True):
    c = cost_pip()
    hi = split - hold - 2
    rows = []
    for z_thr in Z_GRID:
        for q in Q_GRID:
            sig, isl = clv_signals(z, clv_l, clv_s, atr, z_thr, q, warmup,
                                   hi=hi)
            if len(sig) < MIN_N_IS:
                continue
            for slk in SLK_GRID:
                ...
                for rr in RR_GRID:
                    ...
                    rows.append(dict(z_thr=z_thr, q=q, sl_k=slk, rr=rr,
    ...
    if verbose:
        ...
        if best:
            print(f"    IS WINNER: z>={best['z_thr']} q>={best['q']} "
...
def run_tf(tf, verbose=True):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        clv_l = np.where(rng > 0, (cl - l) / rng, 0.0)
        clv_s = np.where(rng > 0, (h - cl) / rng, 0.0)
    del h, l, rng

    rows, best = discover_is(df, z, clv_l, clv_s, atr, split, warmup, hold,
                             verbose)
    out['is_grid'] = rows
    if best is None:
    ...
    sig, isl = clv_signals(z, clv_l, clv_s, atr, best['z_thr'], best['q'],
                           warmup)
