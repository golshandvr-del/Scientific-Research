#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""S763 — «کاراییِ مسیرِ درون‌کندلی در شوک» (Intrabar Path Efficiency Shock) · XAUUSD

پیش‌ثبت: results/S763_PREREG_INTRABAR_PATH_EFFICIENCY.md (commit 1bb178e8 — پیش از هر آزمون)
...
E*=0 = بازویِ کنترلِ شوک-تنها (P1 پیش‌ثبت). سیگنال روی t ⇒ ورود openِ t+1.

اجرا:
  python3 strategies/s763_path_efficiency.py search H8
  python3 strategies/s763_path_efficiency.py holdout H8   # فقط پس از انجمادِ کامیت‌شده
"""
import json
import os
import sys
...
# ---- خانوادهٔ منجمدِ پیش‌ثبت: 5 TF × 3 E* (0=کنترل) × 2 gate = 30 ----
TFS = ['H4', 'H6', 'H8', 'H12', 'D1']
ESTARS = [0.0, 0.5, 0.618]
GATES = ['none', 'drift']
...
def prepare(tf: str):
    ...
    drift_up = c > c_lag
    drift_dn = c < c_lag
    feats = dict(eff=eff, quality=quality, s965=s965,
                 bull=c > o, bear=c < o,
...
def signals(feats, e_star, gate, n):
    ok = feats['quality'] & (feats['eff'] >= e_star)
    up = ok & feats['bull']
    dn = ok & feats['bear']
    if gate == 'drift':
        up &= feats['drift_up']
...
def scan_search(tf: str):
    ...
    out = dict(tf=tf, src=str(src), sub_src=str(sub_src), n_bars=n_all, split=split,
               ...
               barrier='lift*sqrt(n)>=78 & exp>0 & n>=30 & E*>0 & P1(beats control)',
    ...
    print(f"[S763 search {tf}] src={os.path.basename(str(src))} bars={n_all} "
