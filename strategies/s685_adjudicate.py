# -*- coding: utf-8 -*-
"""
s685_adjudicate.py — داوریِ یگانهٔ S685 per کارت (مسیرِ C)
================================================================================
پیش‌ثبت: results/S685_PREREG_INTRAWEEK_BAR_GAP_DRIFT.md.
اکتشاف: strategies/s685_bargap_explore.py — فقط نیمهٔ اول.
...
from strategies.s685_bargap_explore import (            # noqa: E402
    bargap_signals, GS, WARM, OUT_DIR as EXPLORE_DIR)

OUT_DIR = os.path.join(ROOT, 'results', '_s685_adjudicate')
K_PERM = 500
N_TRIALS = 289           # پیش‌ثبت §۳ (283 + 6)
SEED = 20260829          # پیش‌ثبت §۵
...
def pick_candidate(cells: list[dict]) -> tuple[dict | None, str]:
    ...
    bykey = {(c['g'], c['rr']): c for c in cells if 'skipped' not in c}

    def has_plateau(c):
        return any(bykey.get((g2, c['rr']), {}).get('lift_uncond', -99) > 0
                   for g2 in GS if g2 != c['g'])
...
def signals_for(df, tf: str, g: float, warm: int = WARM):
    t = df['time'].values.astype(np.int64)
    o = df['open'].values.astype(np.float64)
    h = df['high'].values.astype(np.float64)
    l = df['low'].values.astype(np.float64)
    c = df['close'].values.astype(np.float64)
    long_sig, short_sig, _ = bargap_signals(t, o, h, l, c, tf, g, warm=warm)
...
def adjudicate(tf: str, asset: str = 'XAUUSD', k_perm: int = K_PERM,
    ...
    if verbose:
        print(f'\n═══ داوریِ S685 {asset}-{tf} ═══\n'
              f'  نامزد: g={cand["g"]} rr={cand["rr"]} ({why})\n'
              f'  هندسه: SL={sl} TP={tp} mh={mh} | src={src} '
              f'n_full={n_full:,} split={split:,}', flush=True)

    long_sig, short_sig = signals_for(df, tf, float(cand['g']), warm)
    ...
    res = dict(layer='S685', tf=tf, asset=asset, src=src, n_full=n_full,
...
def main() -> int:
    ...
    for tf in [t.strip() for t in a.tfs.split(',') if t.strip()]:
        ...
        gc.collect()
    print('\n[S685 adjudication done]', flush=True)
    return 0
