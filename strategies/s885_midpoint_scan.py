#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""S885 — جستجوی IS (فقط نیمهٔ اول) — بازآزمایی میانهٔ تکانه.

پیش‌ثبت: results/S885_PREREG_MidpointRetest_PathC.md (commit b335bcf9)
فضا: W∈{34,55} × k∈{21,34} × b∈{1.0,1.618} × hold∈{34,55,89} × side∈{both,long} = 48
usage: python3 strategies/s885_midpoint_scan.py <TF> [<TF> ...]
"""
...
OUT = os.path.join(ROOT, 'results', '_scan_S885')
...
def scan_tf(tf):
    ...
    print(tf, 'configs=', len(out['configs']),
          'best=', (f"W={b['W']} k={b['k']} side={b['side']} n={b['n']} wr={b['wr']} lift={b['lift']} score={b['score']}" if b else None),
