# -*- coding: utf-8 -*-
"""
S618 — Gold/DXY DECOUPLING continuation (cross-asset, simultaneous) — XAUUSD
============================================================================
پیش‌ثبت: results/S618_PREREG_GOLD_DXY_DECOUPLING.md (کامیت 8a41ada3)

سیگنال منجمد (روی H1 هر دو دارایی، lag 0):
  r = ln(close/close[-1]); sd = rolling-STD(r,34) علّی (تا بار قبل); z = r/sd[-1]
  رویداد: sign(z_gold)==sign(z_dxy) ∧ |z_gold|>=1 ∧ |z_dxy|>=1 ؛ جهت = sign(z_gold)
ورود: اولین بار طلا (TF کارت) با open_time >= close_time بار رویداد.
...
GRID_TF = ['H1', 'H4']
GRID_K = [1.272, 2.058]
OUT_DIR = os.path.join(ROOT, 'results', '_s618_decoup')
...
# ---------- events: simultaneous same-sign z on H1 gold & DXY ----------
def causal_z(c):
    n = len(c)
    r = np.full(n, np.nan); r[1:] = np.diff(np.log(c))
    s = pd.Series(r)
    sd_prev = s.rolling(W_SIG).std(ddof=1).shift(1).values  # تا بار قبل
    with np.errstate(invalid='ignore', divide='ignore'):
        z = r / sd_prev
    return z


def build_events(dxy, g1):
    """returns list of (close_time_of_event_bar, gold_direction, z_gold, z_dxy)"""
    zg = pd.Series(causal_z(g1['close']), index=g1['time'])
    zd = pd.Series(causal_z(dxy['close']), index=dxy['time'])
    j = pd.concat([zg.rename('g'), zd.rename('d')], axis=1, join='inner').dropna()
    m = (np.sign(j['g']) == np.sign(j['d'])) & (j['g'].abs() >= Z_EVENT) & (j['d'].abs() >= Z_EVENT)
    ev = [(int(t + 3600), 1 if g > 0 else -1, float(g), float(d))
          for t, g, d in zip(j.index[m], j['g'][m], j['d'][m])]
    return ev
...
def map_to_gold(events, d):
    ...
    gt = d['time']
    out = []
    for (ct, direction, zg, zd) in events:
        j = int(np.searchsorted(gt, ct, side='left'))
        if j - 1 < 1 or j >= len(gt):
            continue
        out.append((j - 1, direction, zg))  # entry at open[j] = open[sig+1]
    return out
...
def stats(rows):
    ...
    isL = np.array([r[1] > 0 for r in rows])
    wrL = 100.0 * w[isL].mean() if isL.any() else float('nan')
    wrS = 100.0 * w[~isL].mean() if (~isL).any() else float('nan')
    return dict(n=len(rows), n_long=nL, n_short=len(rows) - nL,
                wr=100.0 * w.mean(), wr_long_arm=wrL, wr_short_arm=wrS,
                net=float(p.sum()), mirror_wr=100.0 * wm.mean())
...
def phase_explore():
    ...
    if not al['valid']:
        ...
        return
    events = build_events(dxy, g1)
    nL = sum(1 for e in events if e[1] > 0)
    print(f"[EVENTS] simultaneous same-sign |z|>=1: {len(events)} (long={nL}, short={len(events)-nL}) "
          f"P3 range [400,1500] -> {'OK' if 400 <= len(events) <= 1500 else 'OUT'}", flush=True)
    grid = []
    for tf in GRID_TF:
        ...
        for k in GRID_K:
            ...
            print(f"  {tf} k={k}: n={st['n']} (L{st['n_long']}/S{st['n_short']}) WR={st['wr']:.2f} "
                  f"[L{st['wr_long_arm']:.1f}/S{st['wr_short_arm']:.1f}] mirrorWR={st['mirror_wr']:.2f} net={st['net']:.0f} ref={ref:.2f} "
...
def phase_verdict():
    ...
    dxy = load_dxy_h1()
    events = build_events(dxy, load_gold('H1'))
    d = load_gold(tf)
    atr = wilder_atr34(d['high'], d['low'], d['close'])
