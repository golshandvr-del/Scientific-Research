# -*- coding: utf-8 -*-
"""S912 — «انباشت خاموش»: توالی کندل‌های صعودی کم‌دامنه · XAUUSD · Long-only

پیش‌ثبت: results/S912_PREREG_stealth_accumulation.md (کامیت faa8ca5c)
مسیر C: کارت اصلی H1، جست‌وجو فقط در ۶۰٪ نخست، تست نهایی ۱ اجرا (n_trials=1).

سیگنال (منجمد):
  در پنجرهٔ ۸ کندل اخیر: حداقل ۷ صعودی (close>open)
  دامنهٔ هر کندل پنجره < صدک ۶۱.۸ از ۱۴۴ کندل قبلی (غلتان، بدون جاری)
  close[i] > close[i-8] (پیشروی خالص)
  لبه (کندل قبل واجد نبوده) · ورود کندل بعد.

هندسه: TP=q60(MFE_train) · SL=q30(MAE_train) · TP>=SL · max_hold=34

اجرا:
  python3 strategies/s912_stealth_accumulation.py --phase train --tf H1
  python3 strategies/s912_stealth_accumulation.py --phase holdout --tf H1 --n_trials 1
  python3 strategies/s912_stealth_accumulation.py --phase family
"""
...
SEED = 20260814
WIN = 8                          # منجمد — فیبوناچی: پنجرهٔ انباشت
MIN_UP = 7                       # منجمد — حداقل کندل صعودی در پنجره
LOOKBACK = 144                   # منجمد — فیبوناچی: مرجع دامنه
RANGE_PCTL = 61.8                # منجمد — نسبت طلایی: سقف «خاموشی»
...
CKPT_DIR = os.path.join(ROOT, 'results', '_s912_ckpt')
...
def ignition_signal(df: pd.DataFrame) -> np.ndarray:
    """لبهٔ انباشت خاموش؛ ورود کندل بعد (بدون نگاه به آینده)."""
    h = df['high'].to_numpy(); l = df['low'].to_numpy()
    o = df['open'].to_numpy(); c = df['close'].to_numpy()
    n = len(c)
    rng = h - l
    # مرجع خاموشی: صدک ۶۱.۸ از ۱۴۴ کندل قبلی (shift(1) سپس rolling)
    thr = pd.Series(rng).shift(1).rolling(LOOKBACK).quantile(
        RANGE_PCTL / 100.0).to_numpy()
    up = (c > o)
    quiet = (rng < thr) & ~np.isnan(thr)
    # شمارش غلتان در پنجرهٔ WIN: تعداد صعودی و تعداد خاموش
    up_cnt = pd.Series(up.astype(float)).rolling(WIN).sum().to_numpy()
    quiet_cnt = pd.Series(quiet.astype(float)).rolling(WIN).sum().to_numpy()
    # پیشروی خالص: close[i] > close[i-WIN]
    adv = np.zeros(n, bool)
    adv[WIN:] = c[WIN:] > c[:-WIN]
    state = (up_cnt >= MIN_UP) & (quiet_cnt >= WIN) & adv
    state &= ~np.isnan(up_cnt) & ~np.isnan(quiet_cnt)
    warm = LOOKBACK + WIN + 1
    state[:warm] = False
...
def phase_train(tf: str):
    ...
    out = {'tf': tf, 'split_bar': split, 'n_train': len(df_tr),
           'n_sig': int(sig.sum()), 'win': WIN, 'min_up': MIN_UP,
           'lookback': LOOKBACK, 'range_pctl': RANGE_PCTL,
           'max_hold': MAX_HOLD}
