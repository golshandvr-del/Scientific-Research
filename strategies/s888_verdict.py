#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""S888 — آزمونِ نهاییِ Path C روی **نیمهٔ دوم** (hold-out) — یک بار، فقط یک بار.

پیش‌ثبت: results/S888_PREREG_RangeEdgeShock_PathC.md (commit 7edbdec3)
شوک بزرگ (range ≥ θ×ATR21[t−1]) + گیت مکان در دامنهٔ ۵۵ کندل قبل (EDGE: pos≥1−ε long / pos≤ε short)، follow؛ هیچ چندک منجمدی
وجود ندارد؛ فقط sl/tp منجمد از نیمهٔ اول. محور side از نامزد IS منجمد است.
null کانونی K=500 per-side؛ compute_rqs2 با هر ۵ ورودی؛ n_trials=48؛ seed=888.
usage: python3 strategies/s888_verdict.py <TF> [<TF> ...]
"""
...
SCAN = os.path.join(ROOT, 'results', '_scan_S888')
N_TRIALS = 48
N_PERM = 500
...
def judge_tf(tf):
    ...
    th, eps, side, hold = best['theta'], best['eps'], best['side'], best['hold']
    assert best['zone'] == 'edge'
