# -*- coding: utf-8 -*-
"""
S214_finalize — همپوشانی + سهمِ مستقلِ کاندیدِ برندهٔ S214 (Late-Entry، فصلِ ۱۱)
=================================================================================
> قانونِ شمارهٔ ۱ پروژه: هدف فقط سودِ خالصِ بیشتر (XAUUSD + EURUSD)؛ WR فقط کفِ ۴۰٪.

کاندیدِ برنده (از گریدِ S214):
  **XAUUSD_M5 LONG، ema20/50، run4، br0.5، clx1.5، SL150/TP300، mh96**
  ⇒ net=+$3,153، WR 49.79٪، n=243، PF 1.29، WF [598,722,704,802] (۴/۴)، هر دو نیمه مثبت.

قانونِ همپوشانیِ اجباری (پیش از فصلِ بعد):
  سهمِ مستقلِ کاندید نسبت به **اجتماعِ کاملِ لایه‌های LONG طلا** (زمان-محورها ∪ Brooks
  High-2/Low-2 — عیناً build_union_mask ِ S186b) روی M5 سنجیده می‌شود. علاوه بر آن،
  چون S214 یک لبهٔ *continuation/momentum* است (نزدیک به S171 Signs-of-Strength و
  Squeeze→Breakout)، همپوشانیِ بار-به-بار با S171 به‌طور جداگانه هم گزارش می‌شود.

  اگر همپوشانی جزئی بود ⇒ سهمِ مستقل (bar-not-in-union) گیتِ سختِ ۴-گانه را می‌گذراند؟
  همچنین بررسیِ راهِ سومِ همپوشانی: آیا بخشِ همپوشان به‌عنوان فیلترِ تأیید ارزش دارد؟
"""
import os, sys, json
import numpy as np, pandas as pd
import warnings; warnings.filterwarnings('ignore')

HERE = os.path.dirname(os.path.abspath(__file__)); ROOT = os.path.dirname(HERE)
sys.path.insert(0, ROOT); sys.path.insert(0, HERE)
import s172_brooks_two_legs as S
import s214_brooks_late_entry as X
from s168_brooks_high2_low2 import count_high2_low2
from engine import scalp_engine as se

RESULTS = os.path.join(ROOT, 'results')
P = dict(tf='XAUUSD_M5', asset='XAUUSD', side='long', ef=20, es=50,
         n_run=4, br=0.5, clx=1.5, sl=150, tp=300, mh=96)


def build_union_mask(df):
    """ماسکِ بار-به-بارِ اجتماعِ کاملِ پرتفویِ طلا: زمان-محورها ∪ Brooks High-2/Low-2.
    عیناً مطابقِ S186b (سازگاریِ سیب‌به‌سیب)."""
    d = df.copy()
    d['hour'] = d['dt'].dt.hour
    d['dow'] = d['dt'].dt.dayofweek
    d['dom'] = d['dt'].dt.day
    d['ym'] = d['dt'].dt.year * 100 + d['dt'].dt.month
    days = d[['dt', 'ym']].copy()
    days['date'] = d['dt'].dt.normalize()
    dd = days.drop_duplicates('date').reset_index(drop=True)
    dd['rank'] = dd.groupby('ym').cumcount() + 1
    dd['cnt'] = dd.groupby('ym')['date'].transform('count')
    dd['from_end'] = dd['rank'] - dd['cnt'] - 1
    mp = dict(zip(dd['date'], dd['from_end']))
    d['from_end'] = d['dt'].dt.normalize().map(mp)

    time_mask = (
        d['hour'].isin([22, 23]) |
        (d['dom'] <= 3) |
        d['dom'].between(13, 17) |
        d['from_end'].between(-8, -6) |
        (d['dow'] == 0)
    ).to_numpy()

    long_evt, short_evt = count_high2_low2(d, 20, 50)
    brooks_mask = np.asarray(long_evt, dtype=bool) | np.asarray(short_evt, dtype=bool)
    return time_mask | brooks_mask, time_mask, brooks_mask


def wf_halves_from_trades(tr, asset):
    if tr is None or len(tr) < 8:
        return None
    tr = tr.sort_values('entry_bar').reset_index(drop=True)
    _, _, ptbl = se.run_capital_pertrade(tr, asset, initial_capital=S.CAP,
                                         risk_pct=S.RISK, compounding=False)
    nu = ptbl['net_usd'].to_numpy()
    h = len(nu) // 2
    q = len(nu) // 4
    return dict(h1=float(nu[:h].sum()), h2=float(nu[h:].sum()),
                wf=[round(float(nu[i*q:(i+1)*q].sum())) for i in range(4)])


def main():
    print("=" * 92)
    print("S214_finalize — همپوشانیِ کاندیدِ Late-Entry (XAUUSD_M5 LONG) با Union-All طلا")
    print("=" * 92, flush=True)

    df = S.lastn(S.load(P['tf']), y=4)
    sig = X.late_entry_signals(df, P['side'], P['ef'], P['es'], P['n_run'], P['br'], P['clx'])
    z = np.zeros(len(df), bool)

    # لبهٔ خام
    r = X.evaluate(df, P['asset'], sig, P['side'], P['sl'], P['tp'], P['mh'])
    print(f"\n### لبهٔ خام (کاندید): net=${r['net']:+,.0f}  WR={r['wr']}  n={r['n']}  PF={r['pf']}")
    print(f"    WF={[w[0] for w in r['wf']]}  h1={r['h1']}  h2={r['h2']}  ACC={r['accepted']}")

    tr = S.sim(df, sig, z, P['sl'], P['tp'], P['mh'], P['asset'])
    union_mask, time_mask, brooks_mask = build_union_mask(df)

    total = len(tr)
    ct = cb = cu = 0
    indep_flag = []
    for _, t in tr.iterrows():
        i = int(t['entry_bar'])
        if i >= len(union_mask):
            i = len(union_mask) - 1
        ct += bool(time_mask[i]); cb += bool(brooks_mask[i]); cu += bool(union_mask[i])
        indep_flag.append(not bool(union_mask[i]))
    indep_flag = np.array(indep_flag, bool)
    tr_ind = tr[indep_flag].reset_index(drop=True)
    tr_ov = tr[~indep_flag].reset_index(drop=True)

    print(f"\n### همپوشانیِ بار-به-بار (بر مبنای entry_bar) — کلِ معاملات: {total}")
    print(f"  با زمان-محورها : {ct} ({ct/total*100:.1f}%)")
    print(f"  با Brooks H2/L2 : {cb} ({cb/total*100:.1f}%)")
    print(f"  با Union-All    : {cu} ({cu/total*100:.1f}%)")
    print(f"  سهمِ مستقلِ اصیل : {len(tr_ind)} ({len(tr_ind)/total*100:.1f}%)")

    s_all = S.stats(tr, P['asset'])
    s_ind = S.stats(tr_ind, P['asset']) if len(tr_ind) else dict(net=0, wr=0, n=0, pf=0)
    print(f"\n  کل    : net={s_all['net']:+,.2f} WR={s_all['wr']:.2f}% n={s_all['n']} PF={s_all['pf']:.3f}")
    print(f"  مستقل : net={s_ind['net']:+,.2f} WR={s_ind['wr']:.2f}% n={s_ind['n']} PF={s_ind['pf']:.3f}")

    wfh = wf_halves_from_trades(tr_ind, P['asset'])
    passes = False
    if wfh:
        print(f"  مستقل halves: h1={wfh['h1']:+.0f} h2={wfh['h2']:+.0f} | WF={wfh['wf']}")
        passes = (s_ind['net'] > 0 and s_ind['wr'] >= 40 and wfh['h1'] > 0 and wfh['h2'] > 0
                  and all(w > 0 for w in wfh['wf']) and s_ind['n'] >= 30)
    print(f"\n  سهمِ مستقلِ نهایی گیتِ سخت را: "
          f"{'✅ پاس می‌کند ⇒ ثبتِ لبهٔ مستقل' if passes else '❌ رد می‌کند'}")
    print(f"  ➜ سودِ خالصِ قابلِ ثبت (سهمِ مستقلِ محافظه‌کارانه): "
          f"{s_ind['net'] if passes else 0:+,.2f}$")

    out = dict(param=P, raw=r,
               overlap=dict(time=ct/total*100, brooks=cb/total*100, union=cu/total*100,
                            indep_share=len(tr_ind)/total*100),
               indep=dict(net=s_ind['net'], wr=s_ind['wr'], n=s_ind['n'], pf=s_ind['pf'],
                          **(wfh or {}), passes=passes),
               total=total)
    with open(os.path.join(RESULTS, '_s214_finalize.json'), 'w') as f:
        json.dump(out, f, indent=1, default=float)
    print("\nsaved: results/_s214_finalize.json")


if __name__ == '__main__':
    main()
