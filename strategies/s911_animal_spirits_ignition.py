# -*- coding: utf-8 -*-
"""S911 — «اشتعال روحیات حیوانی»: کندل گاوی دامنه-استثنایی · XAUUSD · Long-only

پیش‌ثبت: results/S911_PREREG_animal_spirits_ignition.md (کامیت 4ef792da)
مسیر C: کارت اصلی M30، جست‌وجو فقط در ۶۰٪ نخست، تست نهایی ۱ اجرا (n_trials=1).

سیگنال (منجمد):
  range > صدک ۸۹ از ۲۳۳ کندل قبلی (غلتان، بدون کندل جاری)
  body_pos = (close-low)/range >= 0.786 · گاوی (close>open)
  لبه (کندل قبل اشتعال نبوده) · ورود کندل بعد.

هندسه: TP=q60(MFE_train) · SL=q30(MAE_train) · TP>=SL · max_hold=55

اجرا:
  python3 strategies/s911_animal_spirits_ignition.py --phase train --tf M30
  python3 strategies/s911_animal_spirits_ignition.py --phase holdout --tf M30 --n_trials 1
  python3 strategies/s911_animal_spirits_ignition.py --phase family
"""
...
SEED = 20260814
LOOKBACK = 233                   # منجمد — فیبوناچی
RANGE_PCTL = 89                  # منجمد — فیبوناچی
BODY_POS_MIN = 0.786             # منجمد — نسبت طلایی
MAX_HOLD = 55                    # منجمد — فیبوناچی
TRAIN_FRAC = 0.60
N_PERM = 500
COST_PIP = 3.3
GOLD_TFS = ('M1', 'M3', 'M4', 'M5', 'M6', 'M10', 'M12', 'M15', 'M20', 'M30',
            'H1', 'H2', 'H3', 'H6', 'H8', 'H12', 'D1', 'W1', 'MN1')
CKPT_DIR = os.path.join(ROOT, 'results', '_s911_ckpt')
...
def ignition_signal(df: pd.DataFrame) -> np.ndarray:
    """لبهٔ اشتعال گاوی؛ ورود کندل بعد (بدون نگاه به آینده)."""
    h = df['high'].to_numpy(); l = df['low'].to_numpy()
    o = df['open'].to_numpy(); c = df['close'].to_numpy()
    n = len(c)
    rng = h - l
    # صدک غلتان ۸۹ از ۲۳۳ کندل قبلی (بدون کندل جاری) — shift(1) سپس rolling
    r = pd.Series(rng).shift(1).rolling(LOOKBACK).quantile(RANGE_PCTL / 100.0)
    thr = r.to_numpy()
    with np.errstate(invalid='ignore', divide='ignore'):
        body_pos = np.where(rng > 0, (c - l) / rng, 0.0)
    state = (rng > thr) & (body_pos >= BODY_POS_MIN) & (c > o)
    state &= ~np.isnan(thr)
...
def phase_train(tf: str):
    ...
    out = {'tf': tf, 'split_bar': split, 'n_train': len(df_tr),
           'n_sig': int(sig.sum()), 'lookback': LOOKBACK,
           'range_pctl': RANGE_PCTL, 'body_pos_min': BODY_POS_MIN,
           'max_hold': MAX_HOLD}
