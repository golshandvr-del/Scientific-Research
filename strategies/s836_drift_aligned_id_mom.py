# -*- coding: utf-8 -*-
"""
S836 — لایه «ماندگاریِ بازدهِ درون‌روزی × گیتِ رانشِ علّی ۱۲۰ روزه (Drift-Aligned ID-Momentum)» XAUUSD-H1 — آزمون نهایی مسیر C
=====================================================================================================
پیش‌ثبت: results/S836_PREREG_DRIFT_ALIGNED_ID_MOMENTUM_HOLDOUT.md (کامیت a4c6d72a — قبل از این آزمون)
هندسه‌ی منجمد (هیچ عددی قابل تغییر نیست):
    ID_d = close(last H1 of day) − open(first H1 of day) ; σ_d = median20(daily range)
    z_d = Σ_{10} ID / (σ_d·√10) ; |z|>0.5 پس از ۲۶۰ روز ; drift=close_d−close_{d−120}
    long: z>0 & drift>0 ; short: z<0 & drift<0 (آینه‌ای مشروط) ; n_trials=2 (بدهی S835)
...
فازها:
    --null  : نالِ جای‌گشتیِ جهت K=500 (seed=836836) روی کندل‌های سیگنالِ هولد‌اوت
    --judge : یک (۱) آزمون compute_rqs2 (n_trials=1) ⇒ S836_HOLDOUT_SPENT.lock
اجرا: python3 strategies/s836_er_lock_cont.py --null سپس --judge
"""
...
OUT_DIR = 'results/_scan_S836'
SPLIT_IDX = 54798
N_DAYS, K, WARM_D, G = 10, 0.5, 260, 120
...
def build_features(df):
    ...
    dl = np.where(ev & (z > 0) & (drift > 0))[0]; ds = np.where(ev & (z < 0) & (drift < 0))[0]
...
def phase_null(df, ls, ss, sl_pip, tp_pip):
    ...
    with open(os.path.join(OUT_DIR, 's836_null_holdout.json'), 'w') as f:
...
def phase_judge(df, ls, ss, sl_pip, tp_pip):
    lock = os.path.join(OUT_DIR, 'S836_HOLDOUT_SPENT.lock')
    if os.path.exists(lock):
        print('⛔ هولد‌اوت S836 قبلاً مصرف شده — آزمون دوم ممنوع است (مسیر C).')
        return None
    with open(os.path.join(OUT_DIR, 's836_null_holdout.json')) as f:
        null = json.load(f)
    tr = run_strategy(df, ls, ss, sl_pip, tp_pip)
    ...
    r = rqs2.compute_rqs2(
        ...
        bar_time=df['time'].values, null=null, n_trials=2,
        split_bar=SPLIT_IDX, close=df['close'].values)
    with open(lock, 'w') as f:
        f.write(f'S836 holdout spent — one test only (path C), prereg {PREREG}\n')
    res = dict(layer='S836', card='XAUUSD-H1', prereg=PREREG,
               geometry=dict(n_days=N_DAYS, k=K, drift_days=G, slm_sigma_d=SLM, rr=RR, hold=HOLD, n_trials=2),
               ...
               notes=r.get('notes'))
    with open(os.path.join(OUT_DIR, 's836_judgment_h1.json'), 'w') as f:
