# -*- coding: utf-8 -*-
"""
S649 — UlcerCollapseLong — داوریِ نهاییِ holdout (مسیرِ C — فقط یک بار اجرا)
=======================================================================
پیش‌ثبت: results/S649_PREREG_ULCER_COLLAPSE_LONG_XAUUSD_PATHC.md (commit 094d050b)
منطق عیناً جدولِ §۲ پیش‌ثبت — صفر درجهٔ آزادیِ باز:

  • کارت‌ها: {H3, H4, H6, H8, H12}
  • لانگ: rank100(UI14[t-1]) ≥ 50% و close[t] = max(close[t-13..t]) و درفتِ ۶۰ روزِ تقویمی > 0 · TP = 1.5×SL
  • شورت: وجود ندارد (long-only)
...
PIP = 0.1
MAX_HOLD = 64
# رویدادِ پوشا — تعریفِ بانک، بدونِ پارامتر
CARDS = ['H2', 'H3', 'H4', 'H6', 'H8', 'H12']
...
def run_card(tf, rng):
    ...
    tt = df['time'].values.astype(np.int64)
    ui = np.asarray(ib.compute('ulcer', df), dtype=float)            # UI14 روی close
    pui = np.roll(ui, 1); pui[0] = np.nan
    rank = pd.Series(pui).rolling(100).rank(pct=True).values * 100    # چندکِ UI[t-1] در ۱۰۰ کندلِ منتهی به t-1
    stressed = np.nan_to_num(rank >= 50.0, nan=False).astype(bool)
    hh14 = pd.Series(cv).rolling(14).max().values
    at_high = np.nan_to_num(cv >= hh14 - 1e-9, nan=False).astype(bool)
    recover = stressed & at_high
    # گیتِ درفتِ ۶۰ روزِ تقویمی، علّی: close[t-1] − close[idx(time[t-1] − 60d)]
    pc1 = np.roll(cv, 1); pc1[0] = cv[0]
    pt1 = np.roll(tt, 1); pt1[0] = tt[0]
    j = np.searchsorted(tt, pt1 - DRIFT_SEC, side='left')
    drift = pc1 - cv[np.clip(j, 0, len(cv) - 1)]
    drift[j <= 0] = 0.0                       # تاریخچهٔ ناکافی → نامعتبر
    bull = recover & (drift > 0)            # فروپاشیِ زخم هم‌راستا با درفتِ صعودی → لانگ
    bear = np.zeros(len(cv), bool)          # فقط لانگ — شورت پیش‌ثبت نشده (قانونِ S648)
    ...
    sl = max(1.0, round(1.5 * slb, 1)); tp = round(1.5 * sl, 1)

    long_sig = bull
    short_sig = bear

    tr = sim(df, long_sig, short_sig, sl, tp)
    if tr is not None and len(tr) and 'tp_pip' not in tr.columns:
        tr = tr.copy(); tr['tp_pip'] = float(tp)      # اصلاحِ فنیِ لایهٔ قبل: فرادادهٔ RR درست
...
def main():
    ...
    print('== S649 — داوریِ نهاییِ holdout · Ulcer-collapse × drift60d>0 · LONG-ONLY · RR1.5 · {H2,H3,H4,H6,H8,H12} · '
    ...
    if res is None:
        print('\n[S649] هیچ عضوِ معتبری نماند — استخر تهی. حکم: مرگ در H0.',
              flush=True)
    ...
    print('\n' + R.format_rqs2('S649-ULCER-POOL', r), flush=True)

    out = dict(prereg='S649_PREREG_ULCER_COLLAPSE_LONG_XAUUSD_PATHC.md@094d050b',
