# -*- coding: utf-8 -*-
"""
S652 — Follow-Through Close — آزمونِ نهایی روی نیمهٔ دومِ دست‌نخورده (مسیرِ C) — قفل‌شده طبق PREREG
=============================================================================
⛔ طبقِ `research/S652_PREREG.md` (کامیت b885e9b9) و **پیش از اجرا** کامیت
می‌شود. هیچ پارامتری این‌جا انتخاب نمی‌شود. نیمهٔ دوم هر TF یک‌بار لمس می‌شود.

روش (عیناً الگوی داوریِ S650/S651 — فقط سیگنال و جدولِ قفل متفاوت):
  • معاملات با موتورِ رسمی `engine.scalp_engine.simulate_trades`
  • مدلِ صفر: K=600 زیرمجموعه از رخدادهای غیرشرطیِ هم‌هندسه، بذر 652652
...
from strategies.s652_followthrough_explore import (        # noqa: E402
    signals, GEO_ATR_P, GEO_SL_K, GEO_RR, GEO_HOLD)
from tools import s434_fast_data as fd                     # noqa: E402

OUT = os.path.join(ROOT, 'results', '_scan_S652')
ASSET = 'XAUUSD'
SEED = 652652            # منجمد (PREREG)
PERM_K = 600             # منجمد
N_TRIALS = 17            # منجمد
SPLIT_FRAC = 0.70        # منجمد

# جدولِ قفل‌شدهٔ PREREG — TF → (c, m). تغییرش = نقضِ پیش‌ثبت.
LOCKED = {
    'M1': (3, 0.0), 'M3': (3, 0.618), 'M4': (3, 0.618), 'M5': (3, 0.618),
    'M6': (3, 0.618), 'M10': (2, 0.618), 'M12': (2, 0.618),
    'M15': (1, 0.618), 'M20': (3, 0.618), 'M30': (3, 0.618),
    'H1': (1, 0.618), 'H2': (2, 0.618), 'H3': (3, 0.0), 'H6': (1, 0.618),
    'H8': (3, 0.0), 'H12': (2, 0.0), 'D1': (2, 0.0),
}
...
def judge_tf(tf):
    ...
    if tr is None or len(tr) == 0:
        return dict(layer='S652', tf=tf, verdict='REJECT (no trades)',
                    locked=dict(c=c_, m=m_), src=src)
    ...
    print(rqs2.format_rqs2(f'S652_{tf}', res), flush=True)

    out = dict(layer='S652', tf=tf, asset=ASSET, src=src,
               locked=dict(c=c_, m=m_, atr_p=GEO_ATR_P, sl_k=GEO_SL_K,
                           rr=GEO_RR, hold=GEO_HOLD),
...
def _save_push(tf, out):
    ...
    try:
        ...
        r = subprocess.run(
            ['git', 'commit', '-m',
             f"S652 FINAL {tf}: {out.get('verdict', '?')} "
             f"(hold-out second half, per PREREG b885e9b9)"],
...
def main():
    ...
    print(f"S652 FINAL TEST — path C — locked per PREREG b885e9b9 — TFs: {tfs}",
          flush=True)
    for tf in tfs:
        fp = os.path.join(OUT, f'final_{tf}.json')
        ...
        try:
            ...
            out = dict(layer='S652', tf=tf, verdict='ERROR', error=str(e))
            print(f"    ✖ {tf} ERROR: {e}", flush=True)
        _save_push(tf, out)
    print("\nS652 FINAL — DONE", flush=True)
