# -*- coding: utf-8 -*-
"""S913 — «بازگشت خوش‌بینی»: گذر صعودی RSI(21) از ۵۵ · XAUUSD · Long-only

پیش‌ثبت: results/S913_PREREG_optimism_resumption.md (کامیت f6ee6c31)
مسیر C: کارت اصلی H2، جست‌وجو فقط در ۶۰٪ نخست، تست نهایی ۱ اجرا (n_trials=1).

سیگنال (منجمد — صفر فیلتر):
  RSI(21) وایلدری (engine.indicator_bank.rsi_s)
...
اجرا:
  python3 strategies/s913_optimism_resumption.py --phase train --tf H2
  python3 strategies/s913_optimism_resumption.py --phase holdout --tf H2 --n_trials 1
  python3 strategies/s913_optimism_resumption.py --phase family
"""
...
SEED = 20260814
RSI_P = 21                       # منجمد — فیبوناچی: دورهٔ RSI
...
CKPT_DIR = os.path.join(ROOT, 'results', '_s913_ckpt')
...
def ignition_signal(df: pd.DataFrame) -> np.ndarray:
    """گذر صعودی RSI(21) از ۵۵؛ ورود کندل بعد (بدون نگاه به آینده)."""
    c = df['close']
