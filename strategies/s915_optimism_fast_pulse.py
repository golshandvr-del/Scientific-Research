# -*- coding: utf-8 -*-
"""S915 — «بازگشت خوش‌بینی II — نبض سریع»: گذر صعودی RSI(8) از ۵۵ · XAUUSD · Long-only

پیش‌ثبت: results/S915_PREREG_optimism_resumption_fast_pulse.md (کامیت b2e89897)
مسیر C: کارت اصلی H6 (n_trials=2)، غیراصلی n_trials=20. تنها تفاوت با S913: دورهٔ RSI 21→8.

سیگنال (منجمد — صفر فیلتر):
  RSI(8) وایلدری (engine.indicator_bank.rsi_s)
  گذر صعودی: RSI[i-1] < 55 و RSI[i] >= 55
  ورود کندل بعد (بدون نگاه به آینده).

هندسه: TP=q60(MFE_train) · SL=q30(MAE_train) · TP>=SL · max_hold=55

اجرا:
  python3 strategies/s915_optimism_fast_pulse.py --phase train --tf H6
  python3 strategies/s915_optimism_fast_pulse.py --phase holdout --tf H6 --n_trials 2
  python3 strategies/s915_optimism_fast_pulse.py --phase family
"""
...
from engine.indicator_bank import rsi_s        # noqa: E402
from tools import s434_fast_data as fd         # noqa: E402

SEED = 915
RSI_P = 8                        # منجمد — فیبوناچی: دورهٔ RSI (تنها تغییر vs S913)
RSI_THR = 55.0                   # منجمد — فیبوناچی: آستانهٔ گذر
...
CKPT_DIR = os.path.join(ROOT, 'results', '_s915_ckpt')
...
def ignition_signal(df: pd.DataFrame) -> np.ndarray:
    """گذر صعودی RSI(8) از ۵۵؛ ورود کندل بعد (بدون نگاه به آینده)."""
    c = df['close']
    r = rsi_s(c, RSI_P).to_numpy()
    n = len(r)
    cross = np.zeros(n, bool)
    cross[1:] = (r[:-1] < RSI_THR) & (r[1:] >= RSI_THR)
    cross &= ~np.isnan(r)
    warm = RSI_P * 5              # گرم‌شدن ewm
    cross[:warm] = False
    out = np.zeros(n, bool)
    out[1:] = cross[:-1]          # ورود در کندل بعد
    return out
...
def phase_train(tf: str):
    ...
    out = {'tf': tf, 'split_bar': split, 'n_train': len(df_tr),
           'n_sig': int(sig.sum()), 'rsi_p': RSI_P, 'rsi_thr': RSI_THR,
           'max_hold': MAX_HOLD}
