...
# -*- coding: utf-8 -*-
"""
S971 — آغازِ رژیمِ پایاییِ Roll (Roll-Regime Onset) — دریچهٔ Hasbrouck
================================================================================
پیش‌ثبت: results/S971_PREREG_roll_regime_onset.md (commit f7497255 — پیش از هر PnL)
معیار:   RQS2 v2.6 gates_only. مسیرِ B — یک تعریفِ منجمد، n_trials=24.

تعریف (بندِ ۲ پیش‌ثبت):
  ρ_i = خودهم‌بستگیِ lag-1 بازده روی پنجرهٔ 233 کندلِ منتهی به i (causal)
  onset: ρ_i > 0.10 و ρ_{i−1} ≤ 0.10
  جهت: sign(close_i − close_{i−W})؛ ورود open کندلِ i+1

هندسه (بندِ ۳): SL=1.0×median(ATR14)، TP=1.5×SL، hold=12h ثابت (کف ۴ کندل).
نال (بندِ ۴): جای‌گشتِ مکانِ ورود، K=2000 هر سمت، بذر 20260818، split=0.70.

فقط XAUUSD. H4 بازنمونه از H1. checkpoint: results/_scan_S971/<TF>.json
اجرا: python3 strategies/s971_roll_regime_onset.py M1 [--kperm 2000]
"""
import sys
import os
import json
...
def build_signals(df):
    """(sig_idx, is_long) — onset رژیم؛ سیگنال روی کندلِ i، ورود i+1."""
    c = df['close'].values.astype(np.float64)
    rho = rolling_rho1(c)
    regime = np.where(np.isfinite(rho), rho > RHO_THR, False)
    prev = np.concatenate(([False], regime[:-1]))
    onset = regime & ~prev
    idx = np.flatnonzero(onset)
    idx = idx[idx >= W]                       # پنجرهٔ کامل
    drift = c[idx] - c[idx - W]
    keep = drift != 0
    idx = idx[keep]
    return idx, drift[keep] > 0
...
def run_tf(tf, k_perm=K_PERM, seed=SEED):
    ...
    print(f"    onset: {len(sig):,} (L={int(is_long.sum()):,}"
    ...
    print('\n' + rqs2.format_rqs2(f'S971-{tf}', r), flush=True)
