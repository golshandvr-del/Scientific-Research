# -*- coding: utf-8 -*-
"""
s458_dead_on_arrival.py — S458 · خروجِ «مرده‌به‌دنیاآمده» (غیبتِ پیشرفت تا میانهٔ عمر)
================================================================================
پیش‌ثبت: results/S458_PREREG_ADDENDUM_DEAD_ON_ARRIVAL_EXIT.md (کامیت قبل از اجرا).

قاعده (قفل‌شده): تک‌ارزیابی در close کندلِ e+k. اگر mfe (بیشینهٔ سودِ شناورِ close تا
کنون) < φ×TP_dist و سودِ شناور <= 0 ⇒ خروج (ts/se: open بعد پس از تقدمِ
SL/TP/max_hold؛ S382: close). φ ∈ {0.10, 0.25, 0.50}. k قفل‌شده per-patient.
برنده‌ها لمس نمی‌شوند.

بیماران/قراردادها عینِ S453–S457. توازی اجباری؛ مسیرِ C.
اجرا: python3 strategies/s458_dead_on_arrival.py <patient>
"""
import os
import sys
import json
...
VARIANTS = ('P010', 'P025', 'P050')  # قفل‌شده در پیش‌ثبت
PHI = {'P010': 0.10, 'P025': 0.25, 'P050': 0.50}
K_FIXED = {'S312_M15': 24, 'S312_M30': 18, 'S312_H1': 12,
           'S356_H1': 10, 'S344_M15': 16, 'S382_H4': 30}
_K = {'k': None}  # توسط load_patient پر می‌شود
...
def rule_step(vn, sym, i, is_long, losing, state, fl=None, tp_dist=None, e=None):
    """state = (mfe). ارزیابی فقط در i == e + k."""
    if vn is None:
        return False, state
    mfe = fl if state is None else max(state, fl)
    if i == e + _K['k'] and losing and mfe < PHI[vn] * tp_dist:
        return True, mfe
    return False, mfe
...
def replay_ts(trade, arr, sym, vn, max_hold, spec):
    ...
    while i < n - 1:
        ...
        if vn is not None and i > e:
            fl = c[i] - entry
            ex, st = rule_step(vn, sym, i, True, fl <= 0.0, st, fl, tp - entry, e)
            if ex:
                return (o[nb] - entry - cost) * contract, nb, 'doa_exit'
        i += 1
...
def replay_s382(trade, arr, sym, vn, ps, sl_abs, rr):
    ...
    while j < n:
        ...
        if vn is not None:
            fl = c[j] - entry
            ex, st = rule_step(vn, sym, j, True, fl <= 0.0, st, fl, sl_abs * rr, e)
            if ex:
                return (c[j] - entry) / ps, j, 'doa_exit'
        j += 1
...
def _save(name, out):
    ...
    p = os.path.join(ROOT, 'research', 'mgmt', f'S458_{name}.json')
    with open(p, 'w') as f:
