# -*- coding: utf-8 -*-
"""
S692 — PDH/PDL Close-Breakout Acceptance — XAUUSD فقط
========================================================
پیش‌ثبت: results/S692_PREREG_PDHL_ACCEPTANCE.md (کامیت 631b6493 — قبل از این فایل)

سیگنال (منجمد):
  PDH/PDL = سقف/کف روزِ (دادهٔ) کاملِ قبلی · thL = PDH + b·ATR13 · thS = PDL − b·ATR13
  Long : لبهٔ عبور close از thL (close[t]>thL[t] و close[t−1]≤thL[t−1])
  Short: قرینهٔ کامل روی thS
  Debounce: فقط اولین رخداد هر (روز × سمت)
  ورود: Open کندل t+1 · SL=k_sl×ATR(13) · TP=max(rr×SL,SL) · hold=16h زمانی

خانوادهٔ منجمد: b∈{0,0.236,0.618} × k_sl∈{0.618,1.0,1.618} × rr∈{1.0,1.272,1.618,2.058}
  ⇒ 36 سلول per side · مسیر C: جست‌وجو فقط TRAIN=۶۰٪، حکم از compute_rqs2 (n_trials=36).

درس‌های ارثی S690/S691: رژیم حافظهٔ سخت، کامیت افزایشی per-TF،
...
N_TRIALS   = len(B_GRID) * len(SL_K_GRID) * len(RR_GRID)     # 36
ATR_P      = 13
HOLD_HOURS = 16.0
SPLIT_FRAC = 0.60
N_PERM     = 500
...
def prev_day_levels(d):
    """PDH/PDL[t] = سقف/کف روزِ دادهٔ قبلی نسبت به کندل t (علّی؛ روز اول NaN)."""
    ...
    d_high = np.maximum.reduceat(high, starts)
    d_low = np.minimum.reduceat(low, starts)
    day_idx = np.searchsorted(starts, np.arange(n), side='right') - 1
    pdh = np.full(n, np.nan)
    pdl = np.full(n, np.nan)
    m = day_idx >= 1
    pdh[m] = d_high[day_idx[m] - 1]
    pdl[m] = d_low[day_idx[m] - 1]
    return pdh, pdl, day_idx


def _first_per_day(sig_idx, day_idx):
    if len(sig_idx) == 0:
        return sig_idx
    dd = day_idx[sig_idx]
    keep = np.concatenate(([True], dd[1:] != dd[:-1]))
    return sig_idx[keep]


def acceptance_signals(d, pdh, pdl, day_idx, atr, b, warmup):
    """لبهٔ پذیرش: عبور close از آستانهٔ بافردار + debounce اولینِ روز/سمت."""
    ...
    for side, cond in (('long', close > thL), ('short', close < thS)):
        ...
        edge[:warmup] = False
        out[side] = _first_per_day(np.flatnonzero(edge), day_idx)
    return out
...
def run_tf(tf, do_git=True):
    t0 = _time.time()
    print(f"\n{'='*88}\n=== S692 PDH/PDL-Acceptance :: XAUUSD-{tf} ===", flush=True)
    ...
    out = dict(strategy='S692', concept='pdhl_acceptance', asset=ASSET, tf=tf,
               bars=n, src=src, hold_bars=hold, split_bar=split,
               n_trials=N_TRIALS, family='3x3x4=36/side', cost_pip=c)
    ...
    pdh, pdl, day_idx = prev_day_levels(d)

    # ── ۱) سیگنال‌ها برای ۳ بافر ──
    sig_map = {}
    for b in B_GRID:
        sm = acceptance_signals(d, pdh, pdl, day_idx, atr, b, warmup)
        sig_map[b] = sm
    ...
    print(rqs2.format_rqs2(f'S692 {tf} ', r), flush=True)
...
def _save(tf, out, do_git):
    ...
    if do_git:
        ...
        try:
            subprocess.run(['git', 'add', 'results/s692_runs'], cwd=root, check=True)
            subprocess.run(['git', 'commit', '-m',
                            f"S692 incremental: XAUUSD-{tf} → {out.get('verdict','?')}"],
...
if __name__ == '__main__':
    ...
    for tf in a.tfs:
        ...
        gc.collect()
    print("\n=== S692 sweep complete ===", flush=True)
