...
تقسیم و روی دیسک نوشته می‌شود؛ غربال هر بار یک قطعه را بار می‌کند و
رها می‌کند ⇒ اوجِ حافظه ≈ ۳۰MB به‌جای ۴۸۰MB.

فرمتِ ذخیره: **`.npy` خام با `mmap_mode='r'`** — نه parquet. دو دلیل:
  (الف) این سندباکس `pyarrow`/`fastparquet` ندارد (و نصبشان روی ۹۸۵MB ریسکِ
        بی‌دلیل است)؛ `numpy` هیچ وابستگیِ اضافه نمی‌خواهد.
  (ب) mmap **بهتر** از parquet است: ستون‌ها بدونِ بار شدنِ کلِ آرایه در حافظه،
      مستقیماً در اندیس‌های رویداد (`sb`) نمونه‌گیری می‌شوند ⇒ صفحاتِ لمس‌نشدهٔ
      دیسک هرگز به RAM نمی‌آیند. یعنی حتی از هدفِ اولیه هم کم‌مصرف‌تر شد.
...
def screen401(P, card, man, allow_time=False,
    ...
    vals = {}
    for p, cols in zip(man['parts'], man['part_cols']):
        idx = [j for j, c in enumerate(cols) if c in need]
        if not idx:
            continue                      # ⚡ قطعهٔ بی‌ربط هرگز باز نمی‌شود
        try:
            arr = np.load(p, mmap_mode='r')
        except Exception:
            continue
        for j in idx:
            vals[cols[j]] = np.asarray(arr[sb, j], dtype=np.float64)
        del arr
