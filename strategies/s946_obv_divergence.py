# -*- coding: utf-8 -*-
"""S946 — «واگراییِ OBV در اکسترممِ پنجره» (OBV Divergence Fade).

پیش‌ثبت: results/S946_PREREG_multiplicity_route.md (کامیت 9bcec7c6، پیش از هر بک‌تستی).

فرضیه (Granville 1963): قلهٔ تازهٔ قیمت که OBV (جریانِ تجمعیِ حجمِ جهت‌دار)
تأییدش نکند = صعودِ بی‌پشتوانهٔ تقاضا ⇒ SHORT؛ آینه برای LONG.

رویدادِ آناتومیک: high[i] ≥ max(high[i−w..i−1]) و OBV[i] < max(OBV[i−w..i−1]).
خانوادهٔ منجمد: w∈{55,144} · n_trials=133 (=19×7 تجمعیِ بلوک S94x)
هندسه/صف/مدلِ صفر عیناً S940..S945 (یک نسخهٔ راستی‌آزمایی‌شده).
"""
...
from strategies.s944_wyckoff_no_supply import _rollmin_nb  # noqa: E402
...
def _past_max(x, w):
    """max(x[i−w..i−1]) — بیشینهٔ پنجرهٔ صرفاً-گذشته (شیفتِ rollmax)."""
    r = _rollmax_nb(x, w)
...
def family_dir(high: np.ndarray, low: np.ndarray, close: np.ndarray,
    ...
    """ادغامِ ۲ عضو (OR)؛ اکسترممِ تازهٔ قیمت بدونِ تأییدِ OBV ⇒ فید."""
    n = close.shape[0]
    obv = _obv_nb(close.astype(np.float64), vol.astype(np.float64))
    long_any = np.zeros(n, dtype=np.bool_)
    short_any = np.zeros(n, dtype=np.bool_)
    for w in WINDOWS:
        hmax = _past_max(high.astype(np.float64), w)
        lmin = _past_min(low.astype(np.float64), w)
        omax = _past_max(obv, w)
        omin = _past_min(obv, w)
        sh = np.nan_to_num((high >= hmax) & (obv < omax), nan=False)
        lg = np.nan_to_num((low <= lmin) & (obv > omin), nan=False)
        short_any |= sh
        long_any |= lg
...
def verify_indicators(n=4000, seed=17) -> float:
    """OBV و past-max/min با pandas — پیش از دادهٔ واقعی."""
    import pandas as pd
...
def run_card(tf: str, verbose=True) -> dict:
    ...
    print()
    print(R.format_rqs2(f'S946_ObvDivergence_{ASSET}_{tf}', res), flush=True)
