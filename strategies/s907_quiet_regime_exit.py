"""
S907 — Quiet-Regime Exit · XAUUSD · RQS2 v2.6 · Path C
=========================================================
L کندلِ متوالیِ آرام (range < ATR89[j−1]) و سپس کندل خروج با range ≥ m·ATR89[i−1].
جهت = بدنهٔ کندل خروج. آناتومیک، علّی، شمارشی، یال ذاتی.

پیش‌ثبت: results/S907_PREREGISTRATION.md (قبل از هر آزمون).
گرید منجمد: L∈{8,21} × m∈{1.0,1.5} × k_sl∈{1.272,2.058} × hold∈{13,34}, RR=1.618 ⇒ N_eff=16.
"""
...
ASSET = 'XAUUSD'
OUT = os.path.join(ROOT, 'results', '_s907')
...
GRID_HOLD = (13, 34)
N_EFF = 16
assert len(GRID_L)*len(GRID_TH)*len(GRID_KSL)*len(GRID_HOLD) == N_EFF

W = 89          # پنجرهٔ ATR بلند (پیش‌ثبت §۲) — علّی (shift 1)
WARMUP = 91
SPLIT_FRAC = 0.60
K_PERM = 1000
SEED = 905
...
def build_features(d):
    """ATR21 علّی در واحد قیمت: ATR تا i−1 (شوکِ خودِ کندل، آلوده‌اش نمی‌کند)."""
    h, l, c = d['high'], d['low'], d['close']
    prev_c = np.concatenate(([c[0]], c[:-1]))
    tr = np.maximum(h - l, np.maximum(np.abs(h - prev_c), np.abs(l - prev_c)))
    atr_prev = pd.Series(tr).rolling(W).mean().shift(1).values
    return atr_prev


def shock_mask(d, th, atr_prev):
    with np.errstate(invalid='ignore'):
        s = (d['high'] - d['low']) >= th * atr_prev
    return np.nan_to_num(s, nan=False).astype(bool)


def quiet_mask(d, atr_prev):
    with np.errstate(invalid='ignore'):
        q = (d['high'] - d['low']) < atr_prev
    return np.nan_to_num(q, nan=False).astype(bool)


def build_signals(d, L, th, atr_prev):
    """رویداد: L کندلِ آرامِ متوالی در [i−L, i−1] و کندل i با range ≥ th·ATR[i−1]."""
    quiet = quiet_mask(d, atr_prev)
    exitb = shock_mask(d, th, atr_prev)
    n = len(quiet)
    cs = np.concatenate(([0], np.cumsum(quiet.astype(np.int64))))
    idx = np.arange(n)
    lo = np.maximum(idx - L, 0)
    prev_quiet = cs[idx] - cs[lo]          # آرام‌ها در [i−L, i−1]
    drought = prev_quiet == L
    drought[:L] = False
    ev = exitb & drought
    o, c = d['open'], d['close']
    ls = ev & (c > o)
...
def phase_discover(tf):
    ...
    for L in GRID_L:
      for th in GRID_TH:
        ls, ss = build_signals(d1, L, th, atr_prev)
        n_sig = int(ls.sum() + ss.sum())
        for k_sl in GRID_KSL:
            for rr in (RR_FIXED,):
                for hold in GRID_HOLD:
                    i += 1
                    key = f'L{L}_m{th}_k{k_sl}_rr{rr}_h{hold}'
                    if key in results:
                        continue
...
def phase_final(tf):
    ...
    print(f"FINAL S907 {tf} · locked={locked['best_key']} · "
          f"n_trials={N_EFF} · split_bar={locked['split_bar']}")
    d = fd.load_fast(ASSET, tf)
    assert d['src'] == locked['src'], 'data source changed since lock!'
    atr = atr_pip(d)
    atr_prev = build_features(d)
    ls, ss = build_signals(d, p['L'], p['th'], atr_prev)
