# -*- coding: utf-8 -*-
"""
S647 — AoZeroCrossDrift — داوریِ نهاییِ holdout (مسیرِ C — فقط یک بار اجرا)
=======================================================================
پیش‌ثبت: results/S647_PREREG_AO_ZEROCROSS_DRIFT_XAUUSD_PATHC.md (commit c9027482)
منطق عیناً جدولِ §۲ پیش‌ثبت — صفر درجهٔ آزادیِ باز:

  • کارت‌ها: {H3, H4, H6, H8, H12}
  • لانگ: AO از ≤0 به >0 بگذرد و درفتِ ۶۰ روزِ تقویمی (close[t-1]−close[t-1−60d]) > 0
  • شورت: آینه — گذرِ نزولیِ AO و درفت < 0 · TP = 1.618×SL
...
def run_card(tf, rng):
    ...
    ao = np.asarray(ib.compute('ao', df), dtype=float)
    ...
    bull = up & (drift > 0)      # گذرِ صعودیِ AO هم‌راستا با درفت → لانگ
    bear = dn & (drift < 0)      # گذرِ نزولیِ AO هم‌راستا با درفت → شورت
...
def main():
    ...
    print('== S647 — داوریِ نهاییِ holdout · AO zero-cross × drift60d · RR1.618 · {H2,H3,H4,H6,H8,H12} · '
    ...
    if res is None:
        print('\n[S647] هیچ عضوِ معتبری نماند — استخر تهی. حکم: مرگ در H0.',
              flush=True)
    ...
    print('\n' + R.format_rqs2('S647-AO-POOL', r), flush=True)

    out = dict(prereg='S647_PREREG_AO_ZEROCROSS_DRIFT_XAUUSD_PATHC.md@c9027482',
