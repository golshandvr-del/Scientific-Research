...
# -*- coding: utf-8 -*-
"""
S677 — فاز جست‌وجو (NR-bar → Expansion follow) (فقط نیمهٔ نخست — مسیر C، PREREG در git قبل از این فایل)
...
from strategies.s677_nrexp_signals import nrexp_signals, drift_masks  # noqa: E402
...
def run_tf(tf):
    ...
    if 'mt5_full' not in d_src:
        print(f"[S677/{tf}] ⚠️ src={d_src} خارج از mt5_full — E-16 رد شد", flush=True)
        return None
    print(f"[S677/{tf}] src={d_src} n_full={n_full} search_half={n} max_hold={mh}",
          flush=True)
    ...
    for (gate, side), pool in pools.items():
        ...
        for k_sl in K_SLS:
            ...
            for rr in RRS:
                ...
                del tr
    gc.collect()
    print(f"[S677/{tf}] مبناها اندازه‌گیری شد ({len(uncond)} سلول، {time.time()-t0:.0f}s)",
          flush=True)

    rows = []
    cfg_i = 0
    for q, R in [(q, R) for q in QS for R in RS]:
        L_raw, S_raw = nrexp_signals(o, h, lo, c, q, R)
        L_raw &= valid; S_raw &= valid
        print(f"[S677/{tf}] q={q} R={R} events: long={int(L_raw.sum())} short={int(S_raw.sum())}", flush=True)
        for gate in GATES:
            if gate == 'aligned':
                Lg = L_raw & drift_up
                Sg = S_raw & drift_dn
            ...
            for k_sl in K_SLS:
                ...
                for rr in RRS:
                    ...
                    for side in ('long', 'short', 'both'):
                        ...
                        rows.append(dict(
                            q=q, R=R, gate=gate, side=side, k_sl=k_sl, rr=rr,
    ...
    print(f"[S677/{tf}] تمام شد: {cfg_i} پیکربندی، {len(rows)} نتیجه، {dt:.0f}s → {path}",
          flush=True)
    if rows:
        r = {k: v for k, v in rows[0].items() if not k.startswith('yearly')}
        print(f"[S677/{tf}] بهترین: {r}", flush=True)
    return out
