...
# -*- coding: utf-8 -*-
"""
S747 — «انبساط با بستهٔ قوی، هم‌راستا با رانش» (Strong-Close Drift-Aligned) — XAUUSD
================================================================================
پیش‌ثبت: results/S747_PREREG_STRONG_CLOSE_DRIFT_ALIGNED_PATH_C.md (کامیت پیش از کد)
رویداد: rng≥K_R·ATR + CLV در کرانه + بستهٔ هم‌جهت + هم‌راستایی با SMA(W_D) با shift(1).
شبکه: K_R∈{1.5,2.5} × CLV_K∈{0.8,0.9} × W_D∈{100,200} · K_SL=2.5 · RR=1.5 ثابت = ۸
n_trials=24 (۱۶ خانوادهٔ S746 + ۸ جدید) · مسیر C
قرنطینه: M5(S740)، W1(S743)، H8(S746 — holdout مصرف شد) ⇒ سقف UNPROVEN.
اجرا: python3 strategies/s747_strong_close_drift.py <TF> [--kperm 500]
"""
import sys
import os
import json
...
ASSET = 'XAUUSD'
N_TRIALS = 24                 # صادقانه: ۱۶ (S746) + ۸ (این شبکه)
K_R_GRID = (1.5, 2.5)
CLV_GRID = (0.8, 0.9)
WD_GRID = (100, 200)
K_SL = 2.5                    # منجمد — غالب مطلق S746
RR = 1.5                      # منجمد — غالب مطلق S746
ATR_WIN = 100
HOLD = 16
MIN_N_DISC = 30
MIN_PF_DISC = 1.3
SPLIT_FRAC = 0.60
CAP_TFS = {'M5': 'UNPROVEN', 'W1': 'UNPROVEN', 'H8': 'UNPROVEN'}
...
def build_signals(df, atr, k_r, clv_k, w_d):
    """رویداد گسترش قاطع + هم‌راستایی رانش. خروجی (sig_idx, is_long)."""
    h = df['high'].values
    l = df['low'].values
    c = df['close'].values
    o = df['open'].values

    drift = sma_shift1(c, w_d)
    rng = h - l
    valid = (np.isfinite(atr) & (atr > 0) & (rng > 0) & np.isfinite(drift))
    big = valid & (rng >= k_r * atr)
    with np.errstate(divide='ignore', invalid='ignore'):
        clv = np.where(rng > 0, (c - l) / rng, 0.5)

    long_c = big & (clv >= clv_k) & (c > o) & (c > drift)
    short_c = big & (clv <= 1.0 - clv_k) & (c < o) & (c < drift) & ~long_c
...
def discover(df, atr, split, c_pip):
    ...
    for k_r in K_R_GRID:
        for clv_k in CLV_GRID:
            for w_d in WD_GRID:
                sig, is_long = build_signals(df_d, atr_d, k_r, clv_k, w_d)
                row = dict(k_r=k_r, clv_k=clv_k, w_d=w_d, k_sl=K_SL, rr=RR,
...
def run_tf(tf, k_perm=500, seed=747):
    t0 = _time.time()
    ...
    if n < max(ATR_WIN, max(WD_GRID)) + 200:
    ...
    w = winner
    sig, is_long = build_signals(df, atr, w['k_r'], w['clv_k'], w['w_d'])
    ...
    r = rqs2.compute_rqs2(tr, ASSET, sl_pip=sl_med, tp_pip=tp_med,
                          ...
                          allow_overlap=False)
    print('\n' + rqs2.format_rqs2(f'S747-{tf}', r), flush=True)
