#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""S885 — آزمونِ نهاییِ Path C روی **نیمهٔ دوم** (hold-out) — یک بار، فقط یک بار.

پیش‌ثبت: results/S885_PREREG_MidpointRetest_PathC.md (commit b335bcf9)
رویداد آناتومیک نسبی (تکانهٔ دانچیانی W + بازآزمایی میانهٔ پا در ≤k کندل)؛ هیچ چندک منجمدی
وجود ندارد؛ فقط sl/tp منجمد از نیمهٔ اول. محور side از نامزد IS منجمد است.
null کانونی K=500 per-side؛ compute_rqs2 با هر ۵ ورودی؛ n_trials=48؛ seed=885.
usage: python3 strategies/s885_verdict.py <TF> [<TF> ...]
"""
...
from strategies.s885_feasibility import midpoint_retest_signals
from strategies.s880_entropy_collapse_scan import wr_of

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SCAN = os.path.join(ROOT, 'results', '_scan_S885')
N_TRIALS = 48
N_PERM = 500
...
def judge_tf(tf):
    ...
    W, k, side, hold = best["W"], best["k"], best['side'], best['hold']
