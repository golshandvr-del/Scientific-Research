"""
آموزش و صادرات مدل نهایی ربات — نسخهٔ FULL-FEATURE (بازتولید دقیق استراتژی ۱۴).
=================================================================================
درس نسخه‌های ۱ و ۲: زیرمجموعهٔ کوچکِ feature (۲۴ یا ۳۰ تا) WR را به ۵۸–۵۹٪ افت داد.
یافته: WR=۶۱.۶٪ استراتژی ۱۴ به مجموعهٔ *کامل* build_features (۵۷ feature) و
آستانهٔ ۰.۶۸ وابسته است. پس مدل ربات باید دقیقاً همان مجموعهٔ کامل را بازتولید کند.

این اسکریپت:
1. با build_features کامل + ensemble ۳-seed، WR=۶۱.۶٪ را Walk-Forward بازتولید و تأیید می‌کند.
2. مدل نهایی را روی کل داده آموزش می‌دهد و به ONNX صادر می‌کند.
3. ترتیب کامل feature و آستانه را در model_meta.txt ثبت می‌کند (برای EA).

خروجی: xauusd_s14_model.onnx + model_meta.txt + feature_order.txt
"""
import sys, os
...
# پارامترهای برندهٔ استراتژی ۱۴ (بازتولیدشده: n=1947 WR=61.58% exp=+0.347$)
...
def main():
    print("بارگذاری داده و ساخت featureهای کامل (۵۷ feature)...")
    df = load_data(DATA)
    ...
    print("=" * 64)
    print(f"تأیید نهایی (thr={THR}): n={nt} WR={s['win_rate']:.2f}% "
    ...
    # صادرات ۳ مدل ensemble به‌صورت جداگانه؛ EA میانگین احتمال ۳ مدل را می‌گیرد.
    ...
    for si, sd in enumerate(SEEDS):
        ...
        p = os.path.join(os.path.dirname(__file__), f'xauusd_s14_model_{si}.onnx')
        with open(p, 'wb') as fp:
            fp.write(onx.SerializeToString())
        onnx_paths.append(p)
        print(f"  مدل seed={sd} → {os.path.basename(p)} ({os.path.getsize(p)} bytes)")

    # مدل اصلی (seed=42) به نام استاندارد برای EA تک‌مدلی
    main_path = os.path.join(os.path.dirname(__file__), 'xauusd_s14_model.onnx')
    onx = convert_lightgbm(models[0], initial_types=initial_type, zipmap=False, target_opset=12)
    with open(main_path, 'wb') as fp:
        fp.write(onx.SerializeToString())
    print(f"مدل اصلی ONNX (seed=42): {os.path.basename(main_path)} ({os.path.getsize(main_path)} bytes)")

    # اعتبارسنجی ONNX در برابر LightGBM
    sess = ort.InferenceSession(main_path)
    ...
    fo_path = os.path.join(os.path.dirname(__file__), 'feature_order.txt')
