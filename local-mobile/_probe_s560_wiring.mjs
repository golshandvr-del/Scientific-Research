...
writeFileSync(`${ROOT}/results/_s560_arms/probe_wiring_M5.json`, JSON.stringify({
  ...
  stale_guard: {
    stale_windows_checked: staleChecked, entries_emitted: staleEntries, expected: 0,
    new_event_skips: newEventSkips,
    note: 'skips are day-breaks where bar b+2 opens ANOTHER day (defective broker-feed regions, 17 spots; worst June 2013 with ~4 days of M5 bars missing) — those are new events, not stale continuations',
  },
  sample_entry: sample ? {
    gap_usd: +sample.gap.toFixed(3), thr_usd: sample.thr, weekend: sample.weekend,
    direction: sample.geom.direction, entry: sample.geom.entry,
    sl: sample.geom.sl, tp: sample.geom.tp, rr: sample.geom.rr,
    sl_dist_usd: +sample.geom.slDist.toFixed(3), tp_dist_usd: +sample.geom.tpDist.toFixed(3),
    lots: sample.geom.lots, risk_dollars: sample.geom.riskDollars,
  } : null,
