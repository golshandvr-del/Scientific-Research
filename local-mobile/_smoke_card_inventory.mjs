...
// ───────────────────────────────────────────────────────────────────────────
// ───────────────────────────────────────────────────────────────────────────
// به‌روزرسانیِ S547 (درفتِ پیش‌تعطیلات · Ariel 1990) — احیای موجه، دقیقاً طبقِ
// همان دستورِ بالا: «جدول را به‌روز کن، آزمون را برای سبز شدن تضعیف نکن».
//
// چه چیزی عوض شد: S547 روی **سه** کارت وصل شد ⇒ M15 از ۳ به ۴، M30 از ۲ به ۳،
// H4 از ۱ به ۲. هیچ کارتِ دیگری لمس نشد.
//
// مجوز: results/S547_PreHolidayDrift_Xauusd_MTF_rqs2_89_ACCEPT.md
//   M30 = **89.3** (z=۴.۰۰) · M15 = ۸۴.۷ (z=۳.۴۸) · H4 = ۸۱.۹ (z=۳.۰۹)
//   هر سه ۱۱/۱۱ دروازهٔ RQS2 v2.6 · n=۱۴۶/۱۴۶/۱۴۴
//
// ⚠️ نکتهٔ صداقتی که قرارداد باید حمل کند: این سه کارت **شاهدِ مستقل نیستند**.
//    هر سه همان ۱۴۶ روزِ پیش‌تعطیلات را معامله می‌کنند (رویداد روزانه است، پس
//    n مستقل از تایم‌فریم می‌ماند) ⇒ M30 مرجع است و M15/H4 بدیل‌اند. قید در
//    `CROSS_CARD_ALTERNATES` رجیستری ثبت و با `sameEventFamily` نشانه‌گذاری
//    می‌شود؛ سندِ ACCEPT §۸ + قانونِ S605: هم‌زمان معامله نشوند.
//
// ⚠️ بدهیِ موروثیِ این فایل (کارِ S547 نیست، ولی باید ثبت شود تا آیندگان
//    اشتباه نگیرند): قرارداد در دورانِ «۵ کارت» منجمد شده، در حالی که سایت
//    اکنون **۹** کارت دارد — H6/H8/H12/D1 بعدها با اسنادِ ACCEPT خودشان اضافه
//    شدند و این جدول هرگز همراهشان به‌روز نشد. پیش از S547 این آزمون ۷
//    ناسازگاری می‌داد (اندازه‌گیری‌شده روی کامیتِ bf12f0c1، نه حدس). آن ۷ مورد
//    عمداً اینجا «تعمیر» نمی‌شوند: هر کدام سندِ خودش را می‌خواهد و اصلاحِ
//    نسنجیده‌شان یعنی سبز کردنِ آزمون بدونِ مجوز — همان چیزی که این فایل منع
//    می‌کند. فقط سهمِ S547 اصلاح شد.
